...
class Database:
    ...
    async def init_db(self):
        ...
        try:
            ...
            # await self.movies.create_index("file_id") # Removed to fix index conflict
    ...
    # ----- FIX YAHAN HAI (1/2) -----
    # Renamed 'mongo_search_internal' to 'mongo_primary_search'
    ...
    # ----- FIX YAHAN HAI (2/2) -----
    # Added 'mongo_fallback_search' as an alias
    async def mongo_fallback_search(self, query: str, limit: int = 20) -> List[Dict]:
        """MongoDB text search (fallback)."""
        # Yeh function M1 aur M2 ke liye ek hi logic ka istemal karega.
        # Agar aap M2 ke liye alag (jaise ki sirf regex) logic chahte hain,
        # toh aap yahaan 'mongo_primary_search' se code copy-paste karke 
        # "$text" search waala hissa hata sakte hain.
        # Abhi ke liye, dono search ek jaise hain.
    ...
    async def get_all_movies_for_neon_sync(self) -> List[Dict] | None:
        ...
        if not await self.is_ready(): await self._connect()
        try:
            # --- FIX: Yahan se 'file_unique_id' ka filter hata diya gaya hai ---
            # Ab yeh sabhi movies ko fetch karega.
            cursor = self.movies.find(
                {}, # Filter hata diya
                {
                    "message_id": 1, 
