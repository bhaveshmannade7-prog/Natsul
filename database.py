...
class Database:
    ...
    async def force_rebuild_all_clean_titles(self, clean_title_func: Callable[[str], str], progress_callback: Callable[[int, int], Any] | None = None) -> Tuple[int, int]:
        ...
        try:
            ...
            try:
                # anext() Python 3.10+ में async iterators के लिए built-in है
                # अगर आपका Python version इससे कम है, तो यह async for loop के बराबर ही काम करेगा।
                while True:
                    ...
                    if "title" in movie and movie["title"]:
                        ...
                        if cleaned_title_for_db != raw_title:
                             # Title field को भी clean किया गया title से update करे
