...
# Isme user data aur movie ki full details (file_id, message_id) save hongi.
import logging
import re
import asyncio
...
class Database:
    ...
    async def _connect(self):
        ...
        try:
            ...
            await self.client.admin.command('ping')
            logger.info("MongoDB (Main DB) cluster connection successful (ping ok).")
            ...
            logger.info(f"Connected to MongoDB Atlas (Main DB), selected database: {self.db.name}")
            return True
        except ConnectionFailure as e:
            logger.critical(f"Failed to connect to MongoDB Atlas (Main DB): {e}", exc_info=True)
            self.client = None
            return False
        except Exception as e:
            logger.critical(f"An unexpected error occurred during MongoDB (Main DB) connection: {e}", exc_info=True)
            self.client = None
            return False
    ...
    async def create_mongo_text_index(self):
        ...
        try:
            ...
            logger.info("MongoDB (Main DB) text index created/verified.")
        except OperationFailure as e:
            if "IndexOptionsConflict" in str(e) or "already exists" in str(e):
                 logger.warning(f"MongoDB (Main DB) text index warning (likely harmless): {e}")
            else:
    ...
    async def init_db(self):
        ...
        if not await self._connect():
            raise RuntimeError("Database (Main DB) connection failed on startup. Bot cannot continue.")
        
        try:
            logger.info("Creating (Main DB) database indexes...")
            # User indexes
            ...
            logger.info("(Main DB) Database indexes created/verified.")
        except Exception as e:
            logger.error(f"Error during (Main DB) index creation: {e}", exc_info=True)
            pass 
    ...
    async def mongo_search_internal(self, query: str, limit: int = 20) -> List[Dict]:
        ...
        if not await self.is_ready():
            logger.error("mongo_search_internal (Main DB): DB not ready.")
            return []
        try:
            ...
            if not results and len(clean_query) > 2:
                logger.debug(f"Mongo (Main DB) text search failed for '{clean_query}', trying regex...")
            ...
            logger.error(f"mongo_search_internal (Main DB) failed for '{query}': {e}", exc_info=True)
    ...
    async def get_all_movies_for_sync(self) -> List[Dict] | None:
        """Search DB sync ke liye data nikalta hai."""
        if not await self.is_ready(): await self._connect()
        try:
            pipeline = [
