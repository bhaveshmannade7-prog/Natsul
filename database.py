# database.py
import logging
import re
import asyncio
from datetime import datetime, timedelta, timezone
from typing import List, Dict, Tuple, Any, Literal
from motor.motor_asyncio import AsyncIOMotorClient
import pymongo
from pymongo.errors import ConnectionFailure, DuplicateKeyError, OperationFailure
import certifi  # SSL Fix
from bson import ObjectId

logger = logging.getLogger("bot.database")

def clean_text_for_search(text: str) -> str:
    """Search indexing ke liye text ko clean karta hai."""
    if not text: return ""
    text = text.lower()
    # 's01' 'season 01' jaise patterns hatayein
    text = re.sub(r"\b(s|season)\s*\d{1,2}\b", "", text)
    # Sabhi non-alphanumeric (hindi/english) ko space se replace karein
    text = re.sub(r"[^a-z0-9\s]+", " ", text)
    # Extra spaces hatayein
    text = re.sub(r"\s+", " ", text).strip()
    return text

class Database:
    """MongoDB Atlas ke liye Async Database Interface Class."""
    
    def __init__(self, database_url: str):
        self.database_url = database_url
        self.client: AsyncIOMotorClient | None = None
        self.db: Any = None
        self.users: Any = None
        self.movies: Any = None
        self._is_connecting = asyncio.Lock()

    async def _connect(self) -> bool:
        """
        Database se connect karta hai.
        Agar pehle se connected hai toh ping karke check karta hai.
        """
        if self.client and await self.is_ready():
            logger.debug("MongoDB connection pehle se active hai.")
            return True

        async with self._is_connecting:
            # Lock acquire karne ke baad dobara check karein
            if self.client and await self.is_ready():
                return True
                
            logger.info("MongoDB Atlas se connect kiya ja raha hai...")
            try:
                ca = certifi.where()
                self.client = AsyncIOMotorClient(
                    self.database_url,
                    serverSelectionTimeoutMS=10000,
                    tls=True,
                    tlsCAFile=ca,
                    # High-performance settings
                    maxPoolSize=100,
                    waitQueueTimeoutMS=5000
                )
                await self.client.admin.command('ping')
                
                DATABASE_NAME = "MovieBotDB"
                self.db = self.client[DATABASE_NAME]
                self.users = self.db["users"]
                self.movies = self.db["movies"]
                
                logger.info(f"MongoDB Atlas se connection safal (Database: {self.db.name}).")
                return True
            except (ConnectionFailure, OperationFailure) as e:
                logger.critical(f"MongoDB Atlas se connect nahi ho paya: {e}", exc_info=True)
                self.client = None
                return False
            except Exception as e:
                logger.critical(f"MongoDB connection mein anjaani error: {e}", exc_info=True)
                self.client = None
                return False

    async def init_db(self):
        """
        Database connection banata hai aur zaroori indexes create karta hai.
        """
        if not await self._connect():
            raise RuntimeError("Database connection fail hua. Bot start nahi ho sakta.")
        
        try:
            logger.info("Database indexes banaye ja rahe hain...")
            # User indexes
            await self.users.create_index("user_id", unique=True)
            await self.users.create_index("is_active")
            await self.users.create_index("last_active")
            
            # Movie indexes
            # imdb_id primary unique key hai
            await self.movies.create_index("imdb_id", unique=True)
            # file_unique_id duplicates hatane ke liye
            await self.movies.create_index("file_unique_id")
            # Primary search (regex) ke liye
            await self.movies.create_index("clean_title")
            # Fallback search ($text) ke liye
            await self.movies.create_index(
                [("clean_title", "text"), ("title", "text")],
                name="title_text_index",
                default_language="none"
            )
            # Sorting ke liye
            await self.movies.create_index("added_date")
            
            logger.info("Sabhi Database indexes create/verify ho gaye.")
        except OperationFailure as e:
            if "IndexOptionsConflict" in str(e) or "already exists" in str(e):
                 logger.warning(f"MongoDB index warning (samanya hai): {e}")
            else:
                 logger.error(f"Index create karne mein fail: {e}", exc_info=True)
                 raise
        except Exception as e:
            logger.error(f"Index creation ke dauran error: {e}", exc_info=True)
            pass  # Startup fail na hone dein

    async def is_ready(self) -> bool:
        """Check karta hai ki connection active hai ya nahi."""
        if self.client is None or self.db is None:
            return False
        try:
            await self.client.admin.command('ping')
            return True
        except (ConnectionFailure, OperationFailure):
            logger.warning("DB is_ready check failed (ping error).")
            return False
        except Exception:
            return False

    async def _handle_db_error(self, e: Exception) -> bool:
        """Database errors ko gracefully handle karta hai."""
        if isinstance(e, (ConnectionFailure, asyncio.TimeoutError, OperationFailure)):
             logger.error(f"DB connection error: {type(e).__name__}. Reconnect ki koshish...
", exc_info=False)
             if self.client:
                 self.client.close()
             self.client = None
             return True # Reconnect ki koshish karega
        elif isinstance(e, DuplicateKeyError):
             logger.warning(f"DB DuplicateKeyError: {e.details}")
             return False # Yeh ek logical error hai, connection error nahi
        else:
             logger.error(f"Unhandled DB Exception: {type(e).__name__}: {e}", exc_info=True)
             return False

    # --- Search Methods ---

    async def mongo_primary_search(self, query: str, limit: int = 20) -> List[Dict]:
        """
        MongoDB Primary Search (Optimized Regex on clean_title).
        Yeh 'mongo_fallback_search' se tez hai.
        """
        if not await self.is_ready():
            logger.error("Mongo Primary Search: DB ready nahi hai.")
            return []
            
        try:
            clean_query = clean_text_for_search(query)
            if not clean_query:
                return []
                
            # Indexed regex search (case-insensitive)
            # Yeh 'starts with' (^) ke liye optimized hai
            regex_query = re.compile(f"^{re.escape(clean_query)}", re.IGNORECASE)
            
            cursor = self.movies.find(
                {"clean_title": {"$regex": regex_query}}
            ).limit(limit)

            results = []
            async for movie in cursor:
                results.append({
                    'imdb_id': movie['imdb_id'],
                    'title': movie['title'],
                    'year': movie.get('year')
                })
            
            # Agar 'starts with' se kuch nahi mila, toh 'contains' search karein
            if not results and len(clean_query) > 3:
                logger.debug(f"Mongo primary search (^) fail hua '{clean_query}', ab 'contains' try...
")
                regex_query_contains = re.compile(re.escape(clean_query), re.IGNORECASE)
                cursor_contains = self.movies.find(
                    {"clean_title": {"$regex": regex_query_contains}}
                ).limit(limit)
                async for movie in cursor_contains:
                    results.append({
                        'imdb_id': movie['imdb_id'],
                        'title': movie['title'],
                        'year': movie.get('year')
                    })

            return results
        except Exception as e:
            logger.error(f"mongo_primary_search fail hua '{query}': {e}", exc_info=True)
            await self._handle_db_error(e)
            return []

    async def mongo_fallback_search(self, query: str, limit: int = 20) -> List[Dict]:
        """MongoDB $text search (Fallback)."""
        if not await self.is_ready():
            logger.error("Mongo Fallback Search: DB ready nahi hai.")
            return []
        try:
            clean_query = clean_text_for_search(query)
            if not clean_query:
                return []
                
            # $text search (full-text index ka istemal karta hai)
            cursor = self.movies.find(
                { "$text": { "$search": clean_query } },
                { "score": { "$meta": "textScore" } } 
            ).sort([("score", {"$meta": "textScore"})]).limit(limit)

            results = []
            async for movie in cursor:
                results.append({
                    'imdb_id': movie['imdb_id'],
                    'title': movie['title'],
                    'year': movie.get('year')
                })
            
            return results
        except Exception as e:
            # Agar index nahi bana hai toh yeh fail ho sakta hai
            if "text index required" in str(e):
                logger.error("CRITICAL: MongoDB $text search fail hua. Index nahi bana hai! '/rebuild_index' chalayein.")
            else:
                logger.error(f"mongo_fallback_search fail hua '{query}': {e}", exc_info=True)
            await self._handle_db_error(e)
            return []
    
    # --- User Methods ---
    
    async def add_user(self, user_id: int, username: str | None, first_name: str, last_name: str | None):
        """User ko add ya update karta hai."""
        if not await self.is_ready(): await self._connect()
        try:
            now = datetime.now(timezone.utc)
            await self.users.update_one(
                {"user_id": user_id},
                {"$set": {
                    "username": username,
                    "first_name": first_name,
                    "last_name": last_name,
                    "last_active": now,
                    "is_active": True
                },
                "$setOnInsert": {
                    "joined_date": now
                }},
                upsert=True
            )
        except Exception as e:
            logger.error(f"add_user fail hua {user_id}: {e}", exc_info=False)
            await self._handle_db_error(e)

    async def deactivate_user(self, user_id: int):
        """User ko 'blocked' (inactive) mark karta hai."""
        if not await self.is_ready(): await self._connect()
        try:
            await self.users.update_one(
                {"user_id": user_id},
                {"$set": {"is_active": False}}
            )
            logger.info(f"User {user_id} ko deactivate kiya.")
        except Exception as e:
            logger.error(f"deactivate_user fail hua {user_id}: {e}", exc_info=False)
            await self._handle_db_error(e)

    async def get_concurrent_user_count(self, minutes: int) -> int:
        """Active users ka count nikalta hai."""
        if not await self.is_ready(): await self._connect()
        try:
            cutoff = datetime.now(timezone.utc) - timedelta(minutes=minutes)
            count = await self.users.count_documents({
                "last_active": {"$gte": cutoff},
                "is_active": True
            })
            return count
        except Exception as e:
            logger.error(f"get_concurrent_user_count error: {e}", exc_info=False)
            await self._handle_db_error(e)
            return 9999 # Error ke case mein high value, taaki limit trigger ho

    async def get_user_count(self) -> int:
        """Total active users ka count."""
        if not await self.is_ready(): await self._connect()
        try:
            count = await self.users.count_documents({"is_active": True})
            return count
        except Exception as e:
            logger.error(f"get_user_count error: {e}", exc_info=False)
            await self._handle_db_error(e)
            return -1

    async def cleanup_inactive_users(self, days: int = 30) -> int:
        """Purane (inactive) users ko deactivate karta hai."""
        if not await self.is_ready(): await self._connect()
        try:
            cutoff = datetime.now(timezone.utc) - timedelta(days=days)
            filter_query = {
                "last_active": {"$lt": cutoff},
                "is_active": True
            }
            result = await self.users.update_many(
                filter_query,
                {"$set": {"is_active": False}}
            )
            deactivated_count = result.modified_count
            if deactivated_count > 0:
                logger.info(f"{deactivated_count} inactive users ko deactivate kiya.")
            return deactivated_count
        except Exception as e:
            logger.error(f"cleanup_inactive_users error: {e}", exc_info=False)
            await self._handle_db_error(e)
            return 0

    async def get_all_users(self) -> List[int]:
        """Broadcast ke liye sabhi active users ki ID list."""
        if not await self.is_ready(): await self._connect()
        try:
            users_cursor = self.users.find(
                {"is_active": True},
                {"user_id": 1, "_id": 0}
            )
            return [user["user_id"] async for user in users_cursor]
        except Exception as e:
            logger.error(f"get_all_users error: {e}", exc_info=False)
            await self._handle_db_error(e)
            return []

    async def get_user_info(self, user_id: int) -> Dict | None:
        """Ek user ki jaankari nikalta hai."""
        if not await self.is_ready(): await self._connect()
        try:
            user = await self.users.find_one({"user_id": user_id})
            return user
        except Exception as e:
            logger.error(f"get_user_info error {user_id}: {e}", exc_info=False)
            await self._handle_db_error(e)
            return None

    # --- Movie Methods ---

    async def get_movie_count(self) -> int:
        """Total movies ka count."""
        if not await self.is_ready(): await self._connect()
        try:
            count = await self.movies.count_documents({})
            return count
        except Exception as e:
            logger.error(f"get_movie_count error: {e}", exc_info=False)
            await self._handle_db_error(e)
            return -1

    async def get_movie_by_imdb(self, imdb_id: str) -> Dict | None:
        """
        IMDB ID se movie nikalta hai.
        Hamesha sabse nayi entry (by added_date) nikalta hai.
        """
        if not await self.is_ready(): await self._connect()
        try:
            # Sabse nayi entry ko prefer karein
            movie = await self.movies.find_one(
                {"imdb_id": imdb_id},
                sort=[("added_date", pymongo.DESCENDING)]
            )
            return self._format_movie_doc(movie) if movie else None
        except Exception as e:
            logger.error(f"get_movie_by_imdb error {imdb_id}: {e}", exc_info=False)
            await self._handle_db_error(e)
            return None

    def _format_movie_doc(self, movie_doc: Dict | None) -> Dict | None:
        """Internal helper: Bot ke liye movie data format karta hai."""
        if not movie_doc: 
            return None
        return {
            'imdb_id': movie_doc.get("imdb_id"),
            'title': movie_doc.get("title"),
            'year': movie_doc.get("year"),
            'file_id': movie_doc.get("file_id"),
            'channel_id': movie_doc.get("channel_id"),
            'message_id': movie_doc.get("message_id"),
            # Caption `copy_message` ke saath automatically aa jayega
        }

    async def add_movie(self, imdb_id: str, title: str, year: str | None, file_id: str, message_id: int, channel_id: int, clean_title: str, file_unique_id: str) -> Literal[True, "updated", "duplicate", False]:
        """
        Database mein movie add ya update karta hai.
        imdb_id ko unique key maankar 'upsert' karta hai.
        """
        if not await self.is_ready(): await self._connect()
        
        movie_doc = {
            "imdb_id": imdb_id,
            "title": title,
            "clean_title": clean_title,
            "year": year,
            "file_id": file_id,
            "file_unique_id": file_unique_id,
            "channel_id": channel_id,
            "message_id": message_id,
            "added_date": datetime.now(timezone.utc)
        }
        
        try:
            # Hum 'imdb_id' par upsert kar rahe hain
            result = await self.movies.update_one(
                {"imdb_id": imdb_id},
                {"$set": movie_doc},
                upsert=True
            )
            
            if result.upserted_id:
                return True  # Naya add hua
            elif result.modified_count > 0:
                return "updated" # Purana update hua
            else:
                # Na add hua, na update (matlab data same tha)
                return "duplicate" 
            
        except DuplicateKeyError as e:
            logger.warning(f"add_movie DuplicateKeyError: {title} ({imdb_id}). Error: {e.details}")
            return "duplicate"
        except Exception as e:
            logger.error(f"add_movie fail hua {title} ({imdb_id}): {e}", exc_info=False)
            await self._handle_db_error(e)
            return False

    async def remove_movie_by_imdb(self, imdb_id: str) -> bool:
        """IMDB ID se judi sabhi entries ko delete karta hai."""
        if not await self.is_ready(): await self._connect()
        try:
            result = await self.movies.delete_many({"imdb_id": imdb_id})
            return result.deleted_count > 0
        except Exception as e:
            logger.error(f"remove_movie_by_imdb error {imdb_id}: {e}", exc_info=False)
            await self._handle_db_error(e)
            return False

    async def cleanup_mongo_duplicates(self, batch_limit: int = 100) -> Tuple[int, int]:
        """
        `imdb_id` ke basis par duplicates hatata hai.
        Sabse nayi (by added_date) entry rakhta hai.
        """
        if not await self.is_ready(): await self._connect()
        
        pipeline = [
            {"$sort": {"added_date": pymongo.DESCENDING}},
            {"$group": {
                "_id": "$imdb_id", 
                "count": {"$sum": 1},
                "doc_ids_to_delete": {"$push": "$_id"} # Sabhi ID push karein
            }},
            {"$match": {"count": {"$gt": 1}}},
            # Pehli ID (jo sabse nayi thi) ko chhodkar baaki sab delete list mein daalein
            {"$project": {
                "ids_to_delete": {"$slice": ["$doc_ids_to_delete", 1, {"$size": "$doc_ids_to_delete"}]},
                "duplicate_count_found": {"$subtract": ["$count", 1]}
            }}
        ]
        
        total_duplicates_found = 0
        all_ids_to_delete = []
        
        try:
            async for group in self.movies.aggregate(pipeline):
                total_duplicates_found += group['duplicate_count_found']
                all_ids_to_delete.extend(group['ids_to_delete'])
                
                if len(all_ids_to_delete) >= batch_limit:
                    break
            
            if not all_ids_to_delete:
                return (0, 0) # Koi duplicates nahi mile
            
            ids_for_this_batch = all_ids_to_delete[:batch_limit]
            
            result = await self.movies.delete_many({"_id": {"$in": ids_for_this_batch}})
            
            deleted_count = result.deleted_count
            logger.info(f"{deleted_count} Mongo duplicates (by imdb_id) delete kiye.")
            
            return (deleted_count, total_duplicates_found)
        
        except Exception as e:
            logger.error(f"cleanup_mongo_duplicates error: {e}", exc_info=True)
            await self._handle_db_error(e)
            return (0, 0)

    async def rebuild_clean_titles(self, clean_title_func) -> Tuple[int, int]:
        """
        Sabhi movies ke liye 'clean_title' field ko dobara banata hai.
        """
        if not await self.is_ready(): await self._connect()
        updated_count, total_count = 0, 0
        try:
            total_count = await self.movies.count_documents({})
            if total_count == 0:
                return (0, 0)
                
            # Un sabhi ko process karein jinka clean_title ya toh nahi hai ya khaali hai
            cursor = self.movies.find(
                {"$or": [{"clean_title": {"$exists": False}}, {"clean_title": ""}, {"clean_title": None}]},
                {"title": 1, "_id": 1}
            )
            
            bulk_ops = []
            async for movie in cursor:
                if "title" in movie and movie["title"]:
                    new_clean_title = clean_title_func(movie["title"])
                    bulk_ops.append(
                        pymongo.UpdateOne(
                            {"_id": movie["_id"]},
                            {"$set": {"clean_title": new_clean_title}}
                        )
                    )
                
                # Performance ke liye batching
                if len(bulk_ops) >= 500:
                    result = await self.movies.bulk_write(bulk_ops, ordered=False)
                    updated_count += result.modified_count
                    bulk_ops = []
                    logger.info(f"rebuild_clean_titles: {updated_count} titles update kiye...")

            # Bacha hua batch
            if bulk_ops:
                result = await self.movies.bulk_write(bulk_ops, ordered=False)
                updated_count = result.modified_count
            
            logger.info(f"rebuild_clean_titles: Kul {updated_count} titles update kiye.")
            return (updated_count, total_count)
            
        except Exception as e:
            logger.error(f"rebuild_clean_titles error: {e}", exc_info=False)
            await self._handle_db_error(e)
            return (updated_count, total_count)

    async def get_all_movies_for_neon_sync(self) -> List[Dict] | None:
        """
        NeonDB sync ke liye MongoDB se *sabhi* entries nikalta hai.
        NeonDB duplicates ko khud handle karega.
        """
        if not await self.is_ready(): await self._connect()
        try:
            # Sabhi entries, NeonDB (message_id, channel_id) par conflict check karega
            cursor = self.movies.find(
                {}, 
                {
                    "message_id": 1, 
                    "channel_id": 1, 
                    "file_id": 1, 
                    "file_unique_id": 1, 
                    "imdb_id": 1, 
                    "title": 1, 
                    "_id": 0
                }
            )
            # .to_list(length=None) poora cursor memory mein load karta hai
            # Badi databases ke liye yeh slow ho sakta hai, par sync ke liye zaroori hai
            movies = await cursor.to_list(length=None) 
            return movies
        except Exception as e:
            logger.error(f"get_all_movies_for_neon_sync error: {e}", exc_info=False)
            await self._handle_db_error(e)
            return None
