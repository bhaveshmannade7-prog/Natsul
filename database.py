...
# --- ADD Redis Import ---
try:
    # Rule: Only ADD new layers/wrappers. We are importing the layer here.
...
class Database:
    ...
    async def mongo_primary_search(self, query: str, limit: int = 10) -> List[Dict]:
        ...
        # NOTE: Bot.py ka search ab Fuzzy Cache se hota hai. 
        # Yeh function sirf backup ya diagnostics ke liye use hoga.
        clean_query = query
        if not clean_query:
            return []
    ...
    async def get_all_movies_for_fuzzy_cache(self) -> List[Dict]:
        ...
        # --- FIX: Yahaan se extra 'await' hata diya gaya hai ---
        if not await self.is_ready(): 
            return []
        # --- END FIX ---
