...
class Database:
    ...
    # ----- YEH FUNCTION FIX KIYA GAYA HAI -----
    async def add_movie(self, imdb_id: str, title: str, year: str | None, file_id: str, message_id: int, channel_id: int, file_unique_id: str) -> Literal[True, "updated", "duplicate", False]:
        # **kwargs ko hata diya gaya hai kyonki ab clean_title ki zaroorat nahi
        if not await self.is_ready(): await self._connect()
        
        movie_doc = {
            "imdb_id": imdb_id,
            "title": title, # Atlas Search isi field ko search karega
            "year": year,
            "file_id": file_id,
