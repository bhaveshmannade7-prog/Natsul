...
# =======================================================
# +++++ MONGO FALLBACK SEARCH (NEW) +++++
# =======================================================
async def mongo_search(query: str, limit: int = 20) -> List[Dict]:
    """MongoDB text search (fallback)."""
    # Note: Yeh function class ke bahar hai taaki ise 'db' instance ki zaroorat na ho
    # Ise 'bot.py' mein seedha 'db.movies' collection ke saath istemal kiya jayega
    
    # Yeh function ab 'bot.py' mein seedha call nahi hoga,
    # balki 'bot.py' mein logic ko 'db.mongo_search_internal' call karna chahiye.
    # Isse behtar hai ki ise class ke andar hi rakhein.
    
    # Neeche diye gaye 'Database' class mein 'mongo_search_internal' dekhein.
    # 'bot.py' ko 'db.mongo_search_internal' call karna chahiye.
    
    # --- Confusion se bachne ke liye, main ise class ke andar move kar raha hoon ---
    # Is file se 'mongo_search' ko import na karein.
    pass
...
class Database:
    ...
    async def create_mongo_text_index(self):
        ...
        try:
            # 'clean_title' aur 'title' par text index banayein
            await self.movies.create_index(
                [("clean_title", "text"), ("title", "text")],
                name="title_text_index",
                default_language="none" # Stop words ko disable karein
            )
    ...
    async def init_db(self):
        ...
        try:
            ...
            # --- NEW: Text index (re)create karein ---
            await self.create_mongo_text_index()
            
            logger.info("Database indexes created/verified.")
        except Exception as e:
            # Errors ko create_mongo_text_index() handle kar lega
            logger.error(f"Error during index creation: {e}", exc_info=True)
            pass # Fail mat ho, bas log karein
    ...
    async def mongo_search_internal(self, query: str, limit: int = 20) -> List[Dict]:
        ...
        try:
            ...
            cursor = self.movies.find(
                { "$text": { "$search": clean_query } }, # Text search
                { "score": { "$meta": "textScore" } } # Relevancy score
            ).sort([("score", {"$meta": "textScore"})]).limit(limit) # Score ke hisab se sort karein
            ...
            # Agar text search se kuch na mile, toh 'like' (regex) se try karein
    ...
    async def get_all_movies_for_sync(self) -> List[Dict] | None:
        ...
        try:
            ...
            async for m in cursor:
                ...
                movies.append({
                    'imdb_id': m["imdb_id"], # 'id' field Typesense client mein add hoga
                    'title': title,
                    'year': m.get("year"),
