...
class Database:
    ...
    async def cleanup_movie_titles(self) -> Tuple[int, int]:
        ...
        try:
            ...
            async for movie in cursor:
                ...
                # Agar title mein junk nahi hai, lekin clean_title field missing hai, to use rebuild_clean_titles mein handle kiya jayega.
                
            if bulk_ops:
                result = await self.movies.bulk_write(bulk_ops, ordered=False)
                updated_count = result.modified_count
                logger.info(f"cleanup_movie_titles: Bulk updated {updated_count} titles (junk removed)।")
    ...
    async def force_rebuild_all_clean_titles(self, clean_title_func) -> Tuple[int, int]:
        ...
        try:
            ...
            # Search Bug Fix: Missing clean_title ko force rebuild karte waqt Title ko bhi clean karein.
            cursor = self.movies.find({}, {"title": 1})
            
            bulk_ops = []
            async for movie in cursor:
                if "title" in movie and movie["title"]:
                    # Clean the raw title first (though this should be done on migration/cleanup)
                    ...
                    # Agar original title में भी कोई junk है, तो उसे भी अपडेट कर रहे हैं
                    if cleaned_title_for_db != raw_title:
                         bulk_ops.append(
                            pymongo.UpdateOne(
                                {"_id": movie["_id"]},
                                {"$set": {"clean_title": new_clean_title, "title": cleaned_title_for_db}}
                            )
                        )
                    else:
                        bulk_ops.append(
