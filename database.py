...
class Database:
    def __init__(self, database_url: str):
        ...
        self.bans = None # ADDED: Ban collection
    ...
    async def _connect(self):
        ...
        try:
            ...
            self.bans = self.db["bans"] # ADDED: Ban collection
    ...
    async def init_db(self):
        ...
        try:
            ...
            await self.bans.create_index("is_banned")
    ...
    # ADDED: Feature 1 (User Export)
    async def export_all_users_for_json(self) -> List[Dict]:
        """User ID aur Username nikalta hai JSON export ke liye।"""
        if not await self.is_ready(): await self._connect()
        try:
            ...
            # List mein users ko dict {user_id: ID, username: Name} format mein store karein
            users_list = []
            async for user in cursor:
                users_list.append({
                    "user_id": user.get("user_id"),
                    "username": user.get("username") or "" # Agar username nahi hai to empty string
                })
            return users_list
        except Exception as e:
    ...
    # ADDED: Feature 2 (Ban Logic)
    async def is_user_banned(self, user_id: int) -> bool:
        """Check karta hai ki user banned hai ya nahi।"""
        if not await self.is_ready(): return False
        try:
            # Check for active ban (is_banned: True)
            doc = await self.bans.find_one(
    ...
    async def set_user_ban_status(self, user_id: int, is_banned: bool) -> bool:
        """User ko ban ya unban karta hai।"""
        if not await self.is_ready(): await self._connect()
        try:
            if is_banned:
                # Ban/Update ban status
                await self.bans.update_one(
                    {"user_id": user_id},
                    {"$set": {"is_banned": True, "banned_at": datetime.now(timezone.utc)}},
                    upsert=True
                )
                logger.info(f"User ID {user_id} BANNED.")
            else:
                # Unban/Remove ban record
                result = await self.bans.delete_one(
                    {"user_id": user_id, "is_banned": True}
                )
                if result.deleted_count > 0:
                     logger.info(f"User ID {user_id} UNBANNED.")
            return True
        except Exception as e:
            logger.error(f"set_user_ban_status failed for {user_id}: {e}", exc_info=False)
            await self._handle_db_error(e)
            return False
    # END ADDED
