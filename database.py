...
class Database:
    def __init__(self, database_url: str):
        ...
        self.banned_users = None # NAYA: Banned Users collection
    ...
    async def _connect(self):
        ...
        try:
            ...
            self.banned_users = self.db["banned_users"] # NAYA: Banned Users collection
    ...
    async def get_concurrent_user_count(self, minutes: int) -> int:
        ...
        if redis_cache.is_ready():
            cached_count = await redis_cache.get_concurrent_user_count()
    ...
    # --- NAYA FUNCTION: Admin Export Users ---
    async def get_all_users_for_export(self) -> List[Dict]:
        """Sare users ko export ke liye nikalta hai।"""
        if not await self.is_ready(): return []
        try:
            # Projection: Sirf zaroori fields nikalna
            ...
            ).sort("joined_date", pymongo.ASCENDING)
            
            # Date objects ko ISO format mein convert karna
            users = []
            async for user in cursor:
                if 'joined_date' in user and isinstance(user['joined_date'], datetime):
                    user['joined_date'] = user['joined_date'].isoformat()
                users.append(user)
            
            return users
        except Exception as e:
            logger.error(f"get_all_users_for_export error: {e}", exc_info=True)
            await self._handle_db_error(e)
            return []
    # --- END NAYA FUNCTION ---
    ...
    # --- NAYE FUNCTIONS: Ban System ---
    async def ban_user(self, user_id: int, reason: str, admin_id: int) -> Tuple[bool, bool]:
        """User ko database se ban karta hai (True, True/False) = (is_new_ban, was_user_active_deleted)"""
        if not await self.is_ready(): return False, False
        
        try:
            # 1. Ban record insert/update karna
            ...
            is_new_ban = result.upserted_id is not None
            
            # 2. User ko main 'users' collection se deactivate karna
            deactivate_result = await self.users.update_one(
                {"user_id": user_id, "is_active": True},
                {"$set": {"is_active": False, "ban_reason": reason}}
            )
            was_active_deleted = deactivate_result.modified_count > 0
            
            # 3. Redis se activity cache clear karna
            if redis_cache.is_ready():
                 await redis_cache.remove_user_activity(user_id)
            
            return is_new_ban, was_active_deleted
            
        except Exception as e:
            logger.error(f"ban_user failed for {user_id}: {e}", exc_info=True)
            await self._handle_db_error(e)
            return False, False
            
    async def unban_user(self, user_id: int, admin_id: int) -> bool:
        """User ko ban list se hatata hai।"""
        if not await self.is_ready(): return False
        try:
            result = await self.banned_users.delete_one({"user_id": user_id})
            # Note: Unban karne par 'users' collection mein 'is_active' ko True nahi kiya jaata, unhe /start karna padega
            return result.deleted_count > 0
        except Exception as e:
            logger.error(f"unban_user failed for {user_id}: {e}", exc_info=True)
            await self._handle_db_error(e)
            return False

    async def is_user_banned(self, user_id: int) -> Tuple[bool, Dict | None]:
        """Check karta hai ki user banned hai ya nahi।"""
        if not await self.is_ready(): return False, None
        try:
            ban_doc = await self.banned_users.find_one({"user_id": user_id})
            if ban_doc:
                return True, {"reason": ban_doc.get("reason"), "banned_at": ban_doc.get("banned_at")}
            return False, None
        except Exception as e:
            logger.error(f"is_user_banned failed for {user_id}: {e}", exc_info=True)
            await self._handle_db_error(e)
            return False, None
    # --- END NAYE FUNCTIONS: Ban System ---
    ...
    # ==================================================
    #
    async def force_rebuild_all_clean_titles(self, clean_title_func) -> Tuple[int, int]:
        """
        ...
        try:
            ...
            cursor = self.movies.find({}, {"title": 1})
            ...
            if bulk_ops:
                logger.info(f"force_rebuild: {len(bulk_ops)} operations taiyaar kiye ja rahe hain...")
                result = await self.movies.bulk_write(bulk_ops, ordered=False)
                updated_count = result.modified_count
                logger.info(f"force_rebuild: Bulk updated {updated_count} titles।")
            
            return (updated_count, total_count)
        except Exception as e:
            logger.error(f"force_rebuild_all_clean_titles error: {e}", exc_info=False)
            await self._handle_db_error(e)
            return (updated_count, total_count)
