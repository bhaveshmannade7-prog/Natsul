...
class Database:
    ...
    async def _connect(self) -> bool:
        ...
        async with self._is_connecting:
            ...
            try:
                ...
                # Har Mongo DB mein `users` aur `movies` collection honge
                # Bhale hi `db_fallback` `users` ka istemal na kare
    ...
    async def init_db(self):
        ...
        try:
            ...
            # User indexes (Yeh sirf db_primary mein kaam aayenge)
            ...
            logger.info(f"DB ({self.db.name}) sabhi indexes create/verify ho gaye.")
...
")
        ...
        try:
            ...
            async for movie in cursor:
                ...
                if len(bulk_ops) >= 500:
                    ...
                    bulk_ops = []
                    logger.info(f"rebuild_clean_titles ({self.db.name}): {updated_count} titles update kiye...")

            if bulk_ops:
                result = await self.movies.bulk_write(bulk_ops, ordered=False)
                updated_count = result.modified_count
            
            logger.info(f"rebuild_clean_titles ({self.db.name}): Kul {updated_count} titles update kiye.")
    ...
    async def get_all_movies_for_mongo_sync(self) -> List[Dict] | None:
        """Mongo-to-Mongo sync ke liye data nikalta hai."""
        if not await self.is_ready(): await self._connect()
        try:
            # Sabhi fields lein taaki doosre DB mein poori copy ban sake
    ...
    async def sync_from_mongo_bulk(self, movies_data: List[Dict]) -> int:
        """Doosre Mongo DB se data ko bulk-upsert karta hai."""
        if not await self.is_ready() or not movies_data:
            return 0
        
        bulk_ops = []
        for movie in movies_data:
            # imdb_id ko key maankar upsert karein
            bulk_ops.append(
                pymongo.UpdateOne(
                    {"imdb_id": movie["imdb_id"]},
                    {"$set": movie},
