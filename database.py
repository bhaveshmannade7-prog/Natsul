...
class Database:
    ...
    # --- NAYA FUNCTION: 'rapidfuzz' ke liye data load karega ---
    ...
    async def force_rebuild_all_clean_titles(self, clean_title_func) -> Tuple[int, int]:
        ...
        Yeh 'rebuild_clean_titles' se alag hai, jo sirf missing waalon ko update karta hai.
        """
        if not await self.is_ready(): await self._connect()
        updated_count, total_count = 0, 0
        ...
        try:
            ...
            # Sabhi documents ko find karein, sirf missing waalon ko nahi
            cursor = self.movies.find(
                {},  # <-- YEH HAI BADLAAV
                {"title": 1}
            )
