...
class Database:
    def __init__(self, database_url: str):
        ...
        self.banned_users = None # NAYA: Banned Users collection for Feature 2
    ...
    async def init_db(self):
        ...
        try:
            ...
            # NAYA: Banned Users Index (Feature 2)
    ...
    async def acquire_cross_process_lock(self, lock_name: str, timeout_sec: int) -> bool:
        ...
        if not await await self.is_ready(): return False
    ...
    #
    # ==================================================
    # +++++ FEATURE 1: EXPORT USERS +++++
    # ==================================================
    #
    async def export_all_users_for_json(self) -> List[Dict]:
        """Feature 1: Saare active users ke user_id aur username export karta hai।"""
        if not await self.is_ready(): await self._connect()
        try:
            ...
            async for user in cursor:
                users_list.append({
                    ...
                    # username agar None ho to empty string bhejein (JSON compliance)
                    "username": user.get("username", "") or "" 
                })
            return users_list
        except Exception as e:
            logger.error(f"export_all_users_for_json error: {e}", exc_info=False)
            await self._handle_db_error(e)
            return []

    #
    # ==================================================
    # +++++ FEATURE 2: BAN USER LOGIC +++++
    # ==================================================
    #
    async def is_user_banned(self, user_id: int) -> bool:
        """Feature 2: Check karta hai ki user permanently banned hai ya nahi।"""
        if not await self.is_ready(): await self._connect()
        try:
            # Simple check, banned_at field ka hona hi ban ko darshata hai
            banned_doc = await self.banned_users.find_one({"user_id": user_id}, {"_id": 1})
    ...
    async def ban_user(self, user_id: int, admin_id: int, reason: str = "") -> bool:
        """Feature 2: User ko permanently ban karta hai।"""
        if not await self.is_ready(): await self._connect()
        try:
            ...
            # Agar pehle se exist karta hai to True return karein
            return True
        except Exception as e:
    ...
    async def unban_user(self, user_id: int) -> bool:
        """Feature 2: User ka permanent ban hatata hai।"""
        if not await await self.is_ready(): await self._connect()
        try:
