...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Hybrid (Mongo1-Exact + Python-Fuzzy)")
...
# --- FIX: IMPROVED CLEANING LOGIC ---
def clean_text_for_search(text: str) -> str:
    """Cleans text for search indexing (Improved)."""
    if not text: return ""
    text = text.lower()
...
async def load_fuzzy_cache(db: Database):
    ...
    Cache format: Dict[clean_title, movie_dict]
    """
        ...
        try:
            ...
            if movies_list:
                for movie_dict in movies_list:
                    clean_title = movie_dict.get('clean_title')
                    if clean_title:
                        # Duplicate clean_title hone par, hum pehle waale ko rakhenge
                        # (DB query pehle se hi sort karke la rahi hai)
                        if clean_title not in temp_cache:
                            temp_cache[clean_title] = movie_dict
...
#
# ==================================================
# +++++ FUZZY SEARCH (Scorer = WRatio) +++++
# ==================================================
#
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    `rapidfuzz` ka istemal karke in-memory cache mein search karta hai.
    Yeh ek CPU-bound function hai, isse `to_thread` mein chalana chahiye.
    FIX: Cutoff reverted to 75 and new logic prioritizes quality over quantity.
    """
    if not fuzzy_movie_cache:
        logger.warning("python_fuzzy_search: Cache khaali hai.")
        return []

    try:
        # Behtar performance ke liye keys ko pehle ek list mein daalein
        choices = list(fuzzy_movie_cache.keys())
        
        # NAYA FIX: Process.extract ke bajay, ab hum token_sort_ratio istemal karenge 
        # aur cutoff ko 75 pe rakhenge.
        results = process.extract(
            query,
            choices, 
            scorer=fuzz.WRatio, # WRatio (Weighted Ratio) ko hi rehne denge, yeh acha hai.
            limit=limit,
            score_cutoff=75, # <-- Fix: Cutoff ko 60 se 75 kiya gaya
            processor=clean_text_for_search, # <-- Title ko phir se clean karein
            force_ascii=True # <-- Zaroori
        )
        
        final_list = []
        for (matched_clean_title, score, index) in results:
            # Matched string se poori movie dict haasil karein
            movie_dict = fuzzy_movie_cache[matched_clean_title]
        ...
        # NAYA FIX: Agar exact match (clean_title mein) 100% hai lekin score 75 se kam aaya, 
        # toh usko force-add karein. (eg: 'Avtar' -> 'Avatar' (clean_title 'avatar') jab 'avatar' cache mein ho)
        query_cleaned = clean_text_for_search(query)
        if query_cleaned in fuzzy_movie_cache:
            # Dekhein ki kya yeh match pehle se hi high-score ke saath list mein hai?
            is_already_in_list = any(r['title'] == fuzzy_movie_cache[query_cleaned]['title'] for r in final_list)
            
            # Agar exact match nahi hai ya score bahut kam hai (WRatio can be tricky)
            # Toh hum exact match ko force add karenge.
            if not is_already_in_list:
                 movie_dict = fuzzy_movie_cache[query_cleaned]
                 # Agar query aur cache key ka clean title match hota hai, toh yeh 100% exact match hai.
                 # Ise list ke shuru mein daalein.
                 final_list.insert(0, {
                    'imdb_id': movie_dict['imdb_id'],
                    'title': movie_dict['title'],
                    'year': movie_dict.get('year'),
                    'score': 100.0 # Force 100% score for exact match
                 })
                 final_list = final_list[:limit] # Limit ko maintain karein

        # NAYA FIX: High-score results ko pehle dikhayein
...
async def health_check():
    ...
    return {
        ...
        "search_logic": "Hybrid (Mongo1-Exact + Python-Fuzzy)",
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        search_status = f"⚡️ Hybrid (Mongo-Exact + Python-Fuzzy)"
        ...
        admin_message = (
            ...
            f"<b>⚙️ Search Configuration (NAYA)</b>\n"
    ...
    if is_member:
        welcome_text = (
            ...
            f"Example: <code>Kantara</code> ya <code>Avegers</code>"
        )
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- NAYA HYBRID PARALLEL SEARCH (Mongo-Exact + Python-Fuzzy) ---
    ...
    loop = asyncio.get_running_loop()
    fuzzy_hits_task = loop.run_in_executor(executor, python_fuzzy_search, original_query, 10) # <-- ORIGINAL QUERY DEIN
    
    # 2. Dono ke results ka intezaar karein
    exact_hits, fuzzy_hits_raw = await asyncio.gather(exact_hits_task, fuzzy_hits_task)
    
    if exact_hits is None: exact_hits = []
    if fuzzy_hits_raw is None: 
        logger.error("Fuzzy search (run_in_executor) se None result mila.")
        fuzzy_hits = []
    else:
        fuzzy_hits = fuzzy_hits_raw
    ...
    if fuzzy_hits:
        ...
        # NAYA FIX: Fuzzy hits ko score ke hisaab se sort karke daalein
        fuzzy_hits.sort(key=lambda x: x['score'], reverse=True)
        for movie in fuzzy_hits:
            if movie.get('imdb_id') and movie['imdb_id'] not in unique_movies:
                # Fuzzy search se aaye hue score ko retain karein
                movie_to_add = movie.copy()
                if not movie_to_add.get('score'): movie_to_add['score'] = 0 
                unique_movies[movie['imdb_id']] = movie_to_add
    ...
    # NAYA FIX: Final results ko bhi score ke hisaab se sort karein 
    ...
    for movie in final_results[:max_buttons]:
        ...
        # Debugging ke liye score dikhayein (agar fuzzy se aaya hai)
        score_str = f" ({int(movie.get('score', 0))}%)" if movie.get('score', 0) > 0 else ""
        
        buttons.append([InlineKeyboardButton(text=f"🎬 {display_title}{year_str}{score_str}", callback_data=f"get_{movie['imdb_id']}")])
...
async def search_switch_command(message: types.Message):
    await safe_tg_call(message.answer("ℹ️ Search switch ki ab zaroorat nahi hai.\nBot ab hamesha <b>Hybrid (Mongo-Exact + Python-Fuzzy)</b> ka istemal karta hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def remove_dead_movie_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if db1_del:
        async with FUZZY_CACHE_LOCK:
            global fuzzy_movie_cache
            # Cache se remove karein (naya Dict logic)
            key_to_delete = None
...
async def rebuild_clean_titles_m1_command(message: types.Message, db_primary: Database):
    ...
    # --- NAYA: Fuzzy cache ko reload karein ---
    await load_fuzzy_cache(db_primary)
...
@handler_timeout(900) # (15 minutes timeout, yeh lamba chalega)
async def force_rebuild_m1_command(message: types.Message, db_primary: Database):
    ...
    # Naya 'force' function call karein
...
# --- NAYA COMMAND: Fuzzy Cache Reload ---
