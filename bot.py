...
from typesense_client import (
    initialize_typesense, is_typesense_ready, # <--- is_typesense_ready ab ASYNC hai
...
async def lifespan(app: FastAPI):
    ...
    try:
        # --- FIX: `is_typesense_ready` ko call karein (jo `initialize` ko call karega) ---
...
async def health_check():
    ...
    # --- FIX: `is_typesense_ready` ab async hai, isliye `await` karein ---
    ts_check = await is_typesense_ready()
...
async def start_command(message: types.Message):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        # --- FIX: `is_typesense_ready` ab async hai, isliye `await` karein ---
        ts_ready = await is_typesense_ready()
...
async def search_movie_handler(message: types.Message):
    ...
    if CURRENT_SEARCH_MODE == "typesense":
        if not await is_typesense_ready(): # <--- YAHAN AWAIT ADD KIYA
...
async def sync_typesense_command(message: types.Message):
    # --- FIX: `is_typesense_ready` ab async hai, isliye `await` karein ---
    if not await is_typesense_ready():
        await safe_tg_call(message.answer("❌ Typesense is not connected. Check logs."))
        return
...
async def main():
    ...
    try:
        await db.init_db()
        await is_typesense_ready() # Pehla connection attempt
