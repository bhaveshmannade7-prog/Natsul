...
WEBHOOK_SEMAPHORE = asyncio.Semaphore(1) # NAYA: Webhook Flood Control
# ============ WEBHOOK URL ============
...
async def safe_tg_call(coro, timeout=TG_OP_TIMEOUT, semaphore: asyncio.Semaphore | None = None):
    ...
    try:
        async with semaphore_to_use:
            # Existing sleep logic maintained
        ...
        if "bot was blocked" in error_msg or "user is deactivated" in error_msg:
            ...
            # Predict future error: Flood limit
...
async def check_user_membership(user_id: int, current_bot: Bot) -> bool:
    ...
    try:
        tasks_to_run = []
        if check_channel:
            tasks_to_run.append(safe_tg_call(current_bot.get_chat_member(chat_id=f"@{JOIN_CHANNEL_USERNAME}", user_id=user_id), timeout=5))
        if check_group:
            tasks_to_run.append(safe_tg_call(current_bot.get_chat_member(chat_id=f"@{USER_GROUP_USERNAME}", user_id=user_id), timeout=5))
...
def clean_text_for_search(text: str) -> str:
    """Strict cleaning for Sequence Match (Used for Fuzzy Cache Keys and Exact Match Anchor)."""
    if not text: return ""
    text = text.lower()
    text = re.sub(r"[._\-]+", " ", text) # DB Clean logic se synchronize kiya gaya (spaces zaroori hain)
    text = re.sub(r"\b(s|season)\s*\d{1,2}\b", "", text)
    text = re.sub(r"[^a-z0-9\s]+", "", text) # Sirf a-z, 0-9, aur space rakhein
    text = re.sub(r"\s+", " ", text).strip() # Extra spaces hatayein
    return text
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    ...
    1. Check for 100% Exact Match in Clean Titles (Anchor Score: 1000).
    ...
    try:
        ...
        # Clean query for exact anchoring (uses the new unified clean_text_for_search)
        ...
        # --- NEW: EXACT MATCH ANCHOR (PRIORITY 1000) ---
        if q_anchor in fuzzy_movie_cache:
            data = fuzzy_movie_cache[q_anchor]
            candidates.append({
                'imdb_id': data['imdb_id'],
                'title': data['title'],
                'year': data.get('year'),
                'score': 1000, # MAX Score for 100% Match
                'match_type': 'exact_anchor'
            })
            seen_imdb.add(data['imdb_id'])
            logger.debug(f"Exact match found for '{query}'")
            # If an exact match is found, we continue to fuzzy search, but the final sort will prioritize it.
        ...
        for clean_title_key, fuzz_score, _ in pre_filtered:
            ...
            # Smart Score Calculation (Intent Score > 50 is a strong indicator)
            intent_score = get_smart_match_score(q_anchor, t_tight)
...
async def lifespan(app: FastAPI):
    ...
    for bot_instance in bot_manager.get_all_bots():
        ...
        # Webhook URL ko token ke hisaab se update karein: /bot/{token}
        ...
        if webhook_url_for_token:
            async def set_webhook_safely(bot_instance: Bot, url: str):
                # Rule: Predict possible future errors (rate-limit)
                async with WEBHOOK_SEMAPHORE: # Ek baar mein sirf ek webhook call
                    # FLOOD WAIT Fix: Guaranteed 1-second delay for setting webhook on any token
                    await asyncio.sleep(1.0) 
                    try:
                        # Use safe_tg_call for get_webhook_info only
                        ...
                        if not is_webhook_set:
                             ...
                             # Use the raw Telegram method and rely on aiogram/safe_tg_call for retry/error handling
...
async def search_movie_handler(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    loop = asyncio.get_running_loop()
    fuzzy_hits_task = loop.run_in_executor(executor, python_fuzzy_search, original_query, 15) # original_query pass karein
    ...
    for movie in fuzzy_hits_raw:
        # Exact match (score 1000) ko pehle hi dal denge
