...
CURRENT_SEARCH_MODE_INDEX = 0 # Default is Mongo 1
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    global CURRENT_SEARCH_MODE # Naya: Global mode ko padhein
    user = message.from_user
    if not user: return
    user_id = user.id
    ...
    # 2. Preferred mode (Admin dwara set) se try karein
    ...
    # 3. Agar preferred mode fail hua (ya result nahi mila), toh baaki failover try karein
...
async def search_switch_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # Naye mode ka status check karein
