...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Smart V5 Search (FIXED):
    1. Priority Pass: Force-add any title containing the query string (Sub-string check).
    2. Fuzzy Fill: Use RapidFuzz for the rest if needed.
    3. Re-Rank: Apply Intent Engine V5.
    """
    ...
    try:
        ...
        # --- FIXED STEP: Priority Pass (Exact Substring Match) ---
        # "Avengers" search karne par "The Avengers" ko force-add karo
        priority_candidates = []
        remaining_titles = []
        
        # Split database into Priority (Must Have) and Others
        # This guarantees that if spelling is correct, it IS included.
        for t in all_titles:
            if q_tight in t:
                # Fake score 100, V5 engine will fix ranking later
                priority_candidates.append((t, 100, 0))
            else:
                remaining_titles.append(t)
        
        # --- STEP 2: RapidFuzz Broad Fetch (Fill the rest) ---
        # Agar priority list mein kam items hain, tabhi fuzzy use karo
        needed = 1000 - len(priority_candidates)
        if needed > 0 and remaining_titles:
            ...
            priority_candidates.extend(fuzzy_candidates)

        candidates = []
        
        # --- STEP 3: INTENT ENGINE V5 RE-RANKING ---
        for clean_title_key, fuzz_score, _ in priority_candidates:
            ...
            # Boost score for Priority Pass items (Substring matches)
            if intent_score > 50: 
