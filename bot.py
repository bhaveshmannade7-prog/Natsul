...
def clean_text_for_search(text: str) -> str:
    ...
    if not text: return ""
    text = text.lower()
    # Separators like dot, underscore, parenthesis ko space mein badle
    text = re.sub(r"[._\-()\[\]]+", " ", text) # FIX: [] aur () added
    # Sirf a-z aur 0-9 rakhein
    text = re.sub(r"[^a-z0-9\s]+", "", text)
    # Season info remove karein
    text = re.sub(r"\b(s|season)\s*\d{1,2}\b", "", text)
    # Year info remove karein (1980-2039)
    text = re.sub(r"\b(19[89]\d|20[0-3]\d)\b", "", text)
    # Common Tags remove karein (baad mein isko chote tags se hata kar dobara check karenge)
    # Yeh logic parse_filename jaisa hona chahiye, taki search query saaf rahe.
    common_tags = r"\b(web|rip|org|hindi|dd|5|1|english|480p|720p|1080p|web|dl|hdrip|bluray|dual|audio|esub|full|hd|mkv|mp4)\b"
    text = re.sub(common_tags, "", text)
...
#
# ==================================================
# +++++ FUZZY SEARCH (Scorer = QRatio) +++++
# ==================================================
#
def python_fuzzy_search(query: str, limit: int = 12) -> List[Dict]: # FIX: limit 12 kiya gaya
    """
    `rapidfuzz` ka istemal karke in-memory cache mein search karta hai.
    FIX: Switched to QRatio and Cutoff lowered to 55 for aggressive typo tolerance.
    """
    ...
    try:
        ...
        results = process.extract(
            query,
            choices, 
            scorer=fuzz.QRatio, # FIX: QRatio (Substring matching) ka istemal
            limit=limit,
            score_cutoff=55 # <-- FIX: 55 kiya gaya for more typo tolerance
        )
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    buttons = []
    max_buttons = 20 # FIX: 12 results ke liye 20 buttons rakhe
