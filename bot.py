...
async def ensure_capacity_or_inform(
    ...
    if active >= CURRENT_CONC_LIMIT:
        ...
        if isinstance(message_or_callback, types.CallbackQuery):
            # NAYA: safe_tg_call ka istemal karein
            await safe_tg_call(callback.answer("Server busy, kripya thodi der baad try karein.", show_alert=True))
        return False
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        await safe_tg_call(bot_instance.answer(message.chat.id, admin_message), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCache):
    ...
    if len(original_query) < 2:
        await safe_tg_call(bot_instance.answer(message.chat.id, "🤔 Query bahut chhoti hai. Kam se kam 2 characters likhein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
        
    clean_query = clean_text_for_search(original_query) # "katra"
    if not clean_query:
        await safe_tg_call(bot_instance.answer(message.chat.id, "🤔 Query mein search karne laayak kuch nahi mila."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
        
    if not fuzzy_movie_cache:
        logger.error(f"FATAL: User {user_id} ne search kiya, lekin fuzzy cache khaali hai!")
        if user.id == ADMIN_USER_ID:
            await safe_tg_call(bot_instance.answer(message.chat.id, "⚠️ ADMIN WARNING: Fuzzy cache khaali hai. /reload_fuzzy_cache chalayein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        else:
            await safe_tg_call(bot_instance.answer(message.chat.id, "⚠️ Bot abhi initialize ho raha hai, kripya 30 second baad try karein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return

    searching_msg = await safe_tg_call(bot_instance.answer(message.chat.id, f"🔎 <b>{original_query}</b> search ho raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def migration_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if not message.forward_from_chat or message.forward_from_chat.id != LIBRARY_CHANNEL_ID:
        if LIBRARY_CHANNEL_ID == 0: await safe_tg_call(bot_instance.answer(message.chat.id, "❌ Migration Error: `LIBRARY_CHANNEL_ID` set nahi hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        else: await safe_tg_call(bot_instance.answer(message.chat.id, f"Migration ke liye, files ko seedha apne `LIBRARY_CHANNEL` (ID: `{LIBRARY_CHANNEL_ID}`) se forward karein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    if not info or not info.get("title"):
        ...
        await safe_tg_call(bot_instance.answer(message.chat.id, f"❌ Migration Skipped: MessageID `{message.forward_from_message_id}` ka caption parse nahi kar paya."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
...
async def stats_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    # NAYA: token_manager.get_current_bot() ka istemal karein
    bot_instance = token_manager.get_current_bot()
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, "📊 System Telemetry fetch ki ja رہی ہے..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def search_switch_command(message: types.Message):
    # NAYA: token_manager.get_current_bot() ka istemal karein
    bot_instance = token_manager.get_current_bot()
    await safe_tg_call(bot_instance.answer(message.chat.id, "ℹ️ Search switch ki ab zaroorat nahi hai.\nBot ab hamesha <b>Hybrid</b> ka istemal karta hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def broadcast_command(message: types.Message, db_primary: Database):
    ...
    if not message.reply_to_message:
        await safe_tg_call(bot_instance.answer(message.chat.id, "❌ Broadcast karne ke liye kisi message ko reply karein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    users = await safe_db_call(db_primary.get_all_users(), timeout=60, default=[])
    if not users:
        await safe_tg_call(bot_instance.answer(message.chat.id, "❌ Database (Mongo 1) mein koi active users nahi mile."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
        
    total = len(users); msg = await safe_tg_call(bot_instance.answer(message.chat.id, f"📤 Broadcast shuru... {total:,} users ko target kiya gaya."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def cleanup_users_command(message: types.Message, db_primary: Database):
    # NAYA: token_manager.get_current_bot() ka istemal karein
    bot_instance = token_manager.get_current_bot()
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, "🧹 (Mongo 1) 30 din se purane inactive users ko clean kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def get_user_command(message: types.Message, db_primary: Database):
    ...
    if len(args) < 2 or not args[1].isdigit():
        await safe_tg_call(bot_instance.answer(message.chat.id, "❌ Istemal: /get_user `USER_ID`"), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    user_id_to_find = int(args[1])
    user_data = await safe_db_call(db_primary.get_user_info(user_id_to_find))
    if not user_data:
        await safe_tg_call(bot_instance.answer(message.chat.id, f"❌ User <code>{user_id_to_find}</code> database (Mongo 1) mein nahi mila."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    await safe_tg_call(bot_instance.answer(message.chat.id, user_text), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if not message.reply_to_message or not message.reply_to_message.document:
        await safe_tg_call(bot_instance.answer(message.chat.id, "❌ Kripya .json file ko reply karke command dein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    doc = message.reply_to_message.document
    if not doc.file_name or (not doc.file_name.lower().endswith(".json") and doc.mime_type != "application/json"):
        await safe_tg_call(bot_instance.answer(message.chat.id, "❌ Yeh .json file nahi hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, f"⏳ `{doc.file_name}` download ho raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    try:
    ...
    await load_fuzzy_cache(db_primary)
    await safe_tg_call(bot_instance.answer(message.chat.id, "✅ Fuzzy cache import ke baad reload ho gaya hai."))
...
async def remove_dead_movie_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    args = message.text.split(maxsplit=1)
    if len(args) < 2: await safe_tg_call(bot_instance.answer(message.chat.id, "❌ Istemal: /remove_dead_movie `IMDB_ID`"), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    imdb_id = args[1].strip()
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, f"⏳ <code>{imdb_id}</code> ko teeno DBs se hataya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def cleanup_mongo_1_command(message: types.Message, db_primary: Database):
    ...
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, "⏳ (Mongo 1) mein `imdb_id` duplicates dhoondhe ja rahe hain (Batch: 100)..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def cleanup_mongo_2_command(message: types.Message, db_fallback: Database):
    ...
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, "⏳ (Mongo 2) mein `imdb_id` duplicates dhoondhe ja rahe hain (Batch: 100)..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def remove_library_duplicates_command(message: types.Message, db_neon: NeonDB):
    ...
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, "⏳ NeonDB se `file_unique_id` duplicates dhoondhe ja rahe hain... (Batch: 100)"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def backup_channel_command(message: types.Message, db_neon: NeonDB):
    ...
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        await safe_tg_call(bot_instance.answer(message.chat.id, "❌ Istemal: /backup_channel `BACKUP_CHANNEL_ID_OR_USERNAME`"), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    try:
        ...
        await safe_tg_call(bot_instance.answer(message.chat.id, f"❌ Error: {e}"), semaphore=TELEGRAM_COPY_SEMAPHORE); return

    msg = await safe_tg_call(bot_instance.answer(message.chat.id, f"⏳ NeonDB se unique files ki list fetch ki ja rahi hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def sync_mongo_1_to_neon_command(message: types.Message, db_primary: Database, db_neon: NeonDB):
    ...
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, "⏳ (Mongo 1) se sabhi movies fetch ki ja rahi hain..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def rebuild_clean_titles_m1_command(message: types.Message, db_primary: Database):
    ...
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, "⏳ (Mongo 1) mein `clean_title` field ko rebuild kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    ...
    await load_fuzzy_cache(db_primary)
    await safe_tg_call(bot_instance.answer(message.chat.id, "✅ Fuzzy cache bhi reload ho gaya hai."))
...
async def rebuild_clean_titles_m2_command(message: types.Message, db_fallback: Database):
    ...
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, "⏳ (Mongo 2) mein `clean_title` field ko rebuild kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def set_limit_command(message: types.Message):
    ...
    if len(args)<2 or not args[1].isdigit(): await safe_tg_call(bot_instance.answer(message.chat.id, f"Istemal: /set_limit N (Abhi: {CURRENT_CONC_LIMIT})"), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    try:
        val = int(args[1]); assert 5 <= val <= 5000 
        CURRENT_CONC_LIMIT = val; await safe_tg_call(bot_instance.answer(message.chat.id, f"✅ Concurrency limit ab {CURRENT_CONC_LIMIT} par set hai."), semaphore=TELEGRAM_COPY_SEMAPHORE); logger.info(f"Concurrency limit admin ne {CURRENT_CONC_LIMIT} kar diya hai.")
    except (ValueError, AssertionError): await safe_tg_call(bot_instance.answer(message.chat.id, "❌ Limit 5 aur 5000 ke beech ek number hona chahiye."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def rebuild_neon_vectors_command(message: types.Message, db_neon: NeonDB):
    ...
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, "⏳ (NeonDB Backup) mein `clean_title` aur FTS Data ko rebuild kiya ja raha hai... (Jo NULL hain)"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def reload_fuzzy_cache_command(message: types.Message, db_primary: Database):
    ...
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, "🧠 In-Memory Fuzzy Cache ko Mongo se reload kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    await load_fuzzy_cache(db_primary)
...
async def check_db_command(message: types.Message, db_primary: Database, db_neon: NeonDB):
    ...
    msg = await safe_tg_call(bot_instance.answer(message.chat.id, "🕵️‍♂️ Diagnostics run ki ja rahi hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
