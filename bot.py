...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Hybrid (Mongo1-Exact + Python-Sequence-Priority)")
...
#
# ==================================================
# +++++ FIX: STRICT SEQUENCE MATCH PRIORITY ++++++++
# ==================================================
#
def is_subsequence(query: str, target: str) -> bool:
    """
    Checks if letters of query appear in target in same order.
    Ex: 'Katra' -> 'Kantara' = True
    Ex: 'Katra' -> 'Khatra' = True (But length differs)
    Ex: 'Katra' -> 'Kajana' = False (Missing t, r)
    """
    it = iter(target)
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Smart Hybrid Search (Prioritizes SEQUENCE over Fuzzy Score):
    1. Scans ALL titles for strict subsequence match (K-a-t-r-a).
    2. If sequence found: Give massive score boost (1000+) minus length penalty.
    3. If not, falls back to standard fuzzy match.
    """
    ...
    try:
        ...
        # --- STEP 1: Strict Sequence Scan (Priority High) ---
        # User requirement: "Katra" -> "Kantara" (Sequence match must win)
        sequence_candidates = []
        
        # Since choices is usually < 50,000, this loop is very fast in Python
        for title in choices:
            if is_subsequence(query, title):
                # Calculate Score: 
                # Base 1000 (Guarantees top slot vs fuzzy which is max 100)
                # Penalty: Difference in length. "Kantara" (diff 2) beats "K...a...t...r...a..." (diff 20)
                len_diff = len(title) - len(query)
                score = 1000 - len_diff
                
                # Extra bonus if starts with same letter
                if title.startswith(query[0]):
                    score += 10
                
                sequence_candidates.append({
                    'title': title,
                    'score': score,
                    'data': fuzzy_movie_cache[title]
                })

        # --- STEP 2: Standard Fuzzy (Fallback/Correction) ---
        # RapidFuzz to catch typos like "Kantra" -> "Kantara" (where strict sequence fails)
        # Use lower cutoff to ensure we get *something*
        ...
        # 1. Add Sequence matches first
        for item in sequence_candidates:
            final_map[item['title']] = {
        ...
        # 2. Add Fuzzy matches (if not already present or if better - though unlikely to beat 1000)
        for (matched_title, base_score, _) in fuzzy_raw:
            # Only add if not present (Sequence logic is king here)
            if matched_title not in final_map:
                movie_dict = fuzzy_movie_cache[matched_title]
                final_map[matched_title] = {
                    'imdb_id': movie_dict['imdb_id'],
                    'title': movie_dict['title'],
                    'year': movie_dict.get('year'),
                    'score': base_score # Max 100
                }

        # Convert to list
        final_results = list(final_map.values())
        
        # Sort by Score Descending (Sequence 1000+ will stay at top)
...
async def health_check():
    ...
    return {
        ...
        "search_logic": "Hybrid (Sequence-Priority + Fuzzy)",
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        search_status = f"⚡️ Hybrid (Sequence-Priority + Fuzzy)"
        ...
        admin_message = (
            ...
            f"<b>⚙️ Search Configuration (SEQUENCE FIXED)</b>\n"
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- NAYA HYBRID PARALLEL SEARCH (Mongo-Exact + Python-Priority-Sequence) ---
    ...
    # Priority Sort: Agar Sequence Match (High Score) hai to usse upar rakhein
    ...
    for movie in final_results[:max_buttons]:
        ...
        # Debugging ke liye score dikhayein (agar fuzzy/seq se aaya hai)
        score_display = ""
        if 'score' in movie and movie['score'] > 100:
            score_display = "⚡️" # Bolt icon for sequence match
        
        buttons.append([InlineKeyboardButton(text=f"🎬 {display_title}{year_str} {score_display}", callback_data=f"get_{movie['imdb_id']}")])
