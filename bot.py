...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    ...
    try:
        # --- YEH HAI AAPKA NAYA, BEHTAR LOGIC ---
        ...
        results = process.extract(
            query,
            choices, 
            scorer=fuzz.WRatio, # <-- BEHTAR SCORER (WRatio) ka istemal karein
            limit=limit,
            score_cutoff=75 # WRatio ke liye 75 ek achha cutoff hai (Aveger->80, Katra->83, Trap->90)
        )
        # --- END NAYA LOGIC ---
        
        # Results (choice, score, index) format mein aate hain
        # Yahaan 'choice' ek string (matched clean_title) hai
        final_list = []
