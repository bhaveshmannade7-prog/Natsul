...
import json
import hashlib
import uuid # Naya: For unique IDs
...
# --- NEW IMPORTS ---
# FIXED line 21: Corrected name from TELEGRAM_DELETE_SEMAP_RE to TELEGRAM_DELETE_SEMAPHORE
        ...
        if token_doc:
            ...
            if movie:
                 ...
                 if res:
                      ...
                      # Feature A: Ad delivery after success
                      asyncio.create_task(send_sponsor_ad(user_id, bot, db_primary, redis_cache))
            return
        else:
            await message.answer("❌ **Verification Failed!** Invalid link.")
            return
    # --- END SHORTLINK CATCH ---
