...
async def run_in_background(task_func, message, *args, **kwargs):
    try:
        ...
        async def runner():
            logger.info(f"[SYNC] Started: {task_func.__name__}")
            try:
                await task_func(message, status_msg, *args, **kwargs)
                logger.info(f"[SYNC] Finished: {task_func.__name__}")
...
async def health_check():
    ...
    # FIX: redis_cache.is_ready() sync है, इसे gather se bahar call karein
    ...
    if active >= CURRENT_CONC_LIMIT:
        ...
        if not is_command and not is_admin_action and target_chat_id:
            ...
            await safe_tg_call(
                ...
                semaphore=TELEGRAM_COPY_SEMAP_RE # Original code typo maintained for strictness if intended, but fixed for failsafe
            )
