...
from core_utils import safe_tg_call, safe_db_call, DB_SEMAPHORE, TELEGRAM_DELETE_SEMAP_RE, TELEGRAM_DELETE_SEMAPHORE, TELEGRAM_COPY_SEMAPHORE, TELEGRAM_BROADCAST_SEMAPHORE, WEBHOOK_SEMAP_RE, WEBHOOK_SEMAPHORE, TG_OP_TIMEOUT, DB_OP_TIMEOUT
from redis_cache import redis_cache, RedisCacheLayer
from queue_wrapper import priority_queue, PriorityQueueWrapper, QUEUE_CONCURRENCY, PRIORITY_ADMIN, PRIORITY_BACKGROUND
from smart_watchdog import SmartWatchdog, WATCHDOG_ENABLED 

# --- NEW FEATURE IMPORTS ---
# Note: Since the user wants to preserve every line, we keep the original imports.
try:
    from ad_manager import send_sponsor_ad
except ImportError:
    # Fallback if ad_manager.py is not in the same dir but logic expects it
    async def send_sponsor_ad(user_id, bot, db, redis): pass
...
# ============ REDIS COORDINATION (BUG 1, 2, 5 FIX) ============

async def acquire_global_sync_lock(lock_key: str, timeout: int = 1800) -> bool:
    """Redis-based global lock to coordinate sync across 4 Gunicorn workers."""
    if not redis_cache.is_ready():
        # Fallback to Mongo based lock if Redis is not ready, to maintain multi-worker safety
        return await safe_db_call(db_primary.acquire_cross_process_lock(f"sync_lock:{lock_key}", timeout), default=False)
    
    # Standard Redis Distributed Lock pattern
    try:
        lock_val = f"worker_{os.getpid()}_{uuid.uuid4().hex[:6]}"
        # Set if not exists with expiration
        success = await redis_cache.redis.set(f"bot_sync_lock:{lock_key}", lock_val, ex=timeout, nx=True)
        return bool(success)
    except Exception as e:
        logger.error(f"Redis Lock Acquire Error: {e}")
        return False

async def release_global_sync_lock(lock_key: str):
    """Release the lock so other workers can perform sync if needed."""
    if redis_cache.is_ready():
        try:
            await redis_cache.redis.delete(f"bot_sync_lock:{lock_key}")
        except Exception: pass
    else:
        await safe_db_call(db_primary.release_cross_process_lock(f"sync_lock:{lock_key}"))
...
async def run_in_background(task_func, message: types.Message, *args, **kwargs):
    ...
    Uses Redis coordination to prevent multi-worker collisions.
    """
    lock_key = task_func.__name__
    
    # Check if another worker is already running this specific sync
    if not await acquire_global_sync_lock(lock_key):
        await safe_tg_call(message.answer("⚠️ **Process Conflict**: Another worker is currently executing this sync. Please wait for it to complete."))
        return

    try:
        msg = await message.answer("⚙️ **Distributed Background Task Started.**\nBot responsive rahega. Task progress monitor karein.")
        
        # Heavy sync functions are wrapped to ensure the lock is released even on failure
        async def sync_wrapper():
            try:
                # Add a timeout protection for the entire sync operation (Bug 8)
                await asyncio.wait_for(task_func(message, msg, *args, **kwargs), timeout=1800)
            except asyncio.TimeoutError:
                logger.error(f"Global Timeout for sync task {lock_key}")
                await safe_tg_call(msg.edit_text(f"❌ **Task Timeout**: {lock_key} process exceeds 30m limit."))
            except Exception as e:
                logger.error(f"Sync task {lock_key} failed: {e}", exc_info=True)
                await safe_tg_call(msg.edit_text(f"❌ **Task Failed**: {str(e)}"))
            finally:
                await release_global_sync_lock(lock_key)
                logger.info(f"Global sync lock released for {lock_key}")

        # Run in a truly non-blocking way
...
async def start_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    if is_member:
        ...
        if join_markup:
            ...
            await safe_tg_call(message.answer("⚠️ Configuration Error: Please contact Admin."), semaphore=TELEGRAM_COPY_SEMAP_RE)
...
# ============ BOT HANDLERS: ADMIN COMMANDS + SYNC ============
...
async def force_rebuild_freeze_fix(message: types.Message, db_primary: Database):
    await run_in_background(force_rebuild_m1_command, message, db_primary)
...
async def sync_mongo_1_to_2_command(message: types.Message, db_primary: Database, db_fallback: Database):
    ...
    for movie in mongo_movies_full:
        ...
        # F.I.X: task ko safe_db_call se banayein
        ...
        # F.I.X: Progress update ko niyantrit tareeke se bhejein
    ...
    # F.I.X: Sabhi tasks ko ek saath chalaein aur await karein
    await safe_tg_call(msg.edit_text(f"⏳ **Finalizing Sync**...\nProcessing final {len(all_sync_tasks):,} tasks."))
    
    # F.I.X: MAIN BLOCKING AWAIT
    await asyncio.gather(*all_sync_tasks) # Saare tasks ek saath threadpool mein chalenge
    ...
    async def progress_callback(processed_count: int, total_count: int):
        ...
        try:
            await safe_tg_call(
                ...
                # Telegram API rate limit se bachne ke liye COPY_SEMAPHORE ka upyog
                semaphore=TELEGRAM_COPY_SEMAPHORE 
            )
        except TelegramRetryAfter as e:
            logger.warning(f"Telegram rate limited during progress update. Waiting {e.retry_after}s.")
            # Yahan hum intezaar nahi karenge kyunki DB ka kaam chal raha hai, bas skip karenge.
            pass 
