...
if LIBRARY_CHANNEL_ID == 0:
    logger.warning("LIBRARY_CHANNEL_ID is not set. Auto-indexing and Migration will not work.") # <-- Update
...
async def lifespan(app: FastAPI):
    ...
    try:
        ts_ok = await is_typesense_ready() # is_typesense_ready ab async hai
        if ts_ok: logger.info("Typesense initialization successful.")
        else: logger.critical("TYPESENSE INITIALIZATION FAILED. Bot 'cold start' par fail ho sakta hai, lekin search par retry karega.")
...
async def health_check():
    ...
    ts_check = await is_typesense_ready() # Async check
...
async def start_command(message: types.Message):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        ts_ready = await is_typesense_ready() # Async check
        ...
        admin_message = (
            ...
            f"<b>/cleanup_json</b> ⭐️ (Dead JSON data saaf karein)\n"
...
async def search_movie_handler(message: types.Message):
    ...
    if CURRENT_SEARCH_MODE == "typesense":
        if not await is_typesense_ready(): # Async check
            logger.error(f"User {user_id} search: Typesense not ready! Failing over to MongoDB.")
            CURRENT_SEARCH_MODE = "mongodb" # Fallback
...
async def migration_handler(message: types.Message):
    ...
    toh yeh unhe database mein add/update kar deta hai.
    Yeh 'file_id' problem ko fix karne ka sabse aasan tareeka hai.
    """
    ...
    db_res = await safe_db_call(db.add_movie(imdb_id, title, year, file_id, message_id, channel_id, clean_title_val))

    db_map = {True: "✅ Migrated (Added)", "updated": "✅ Migrated (Updated)", "duplicate": "ℹ️ Migrated (Skipped)", False: "❌ DB Error"}
    status = db_map.get(db_res, "❌ DB Error")

    # Admin ko feedback dein
    await safe_tg_call(message.answer(f"{status}: <b>{title}</b>"))
    
    # Agar yeh update tha, toh Typesense ko bhi update karein
...
@handler_timeout(1800)
async def sync_typesense_command(message: types.Message, from_user_id: int = None): # <-- Naya parameter
    """Typesense ko DB se poori tarah sync karta hai."""
    ...
    msg = None
    if not from_user_id: # Agar yeh command seedha call hua hai, toh message bhejein
    ...
    try:
        ...
        sync_msg_text = f"✅ Fetched {db_count:,} movies from DB.\n⏳ Syncing to Typesense (replace all)..."
