...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Hybrid Optimized (RapidFuzz C++ -> Smart Sequence)")
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Optimized Hybrid Search: RapidFuzz (C++ Pre-filter) -> Smart Sequence (Python Re-rank)
    """
    ...
    try:
        ...
        # 2. RAPIDFUZZ C++ EXTRACT (The CPU Saver)
        # Extract top 50 candidates using C++ optimized matching
        all_titles = list(fuzzy_movie_cache.keys())
        
        # scorer=fuzz.WRatio handles partial matches and different lengths well
        ...
        for clean_title_key, fuzz_score, _ in pre_filtered:
            ...
            if seq_score > 60:
                # Sequence match gets priority (boost)
                final_score = 200 + seq_score 
                match_type = "sequence"
                
                # Penalty for huge length difference (e.g. "Kantara" vs "Kantara Chapter 1")
                len_diff = abs(len(t_tight) - len(q_tight))
                final_score -= len_diff
            else:
                # Fallback to the RapidFuzz score we already calculated
