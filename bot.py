...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Intent Engine V4 (Smart Context + First Letter Priority)")
...
#
# ==================================================
# +++++ NEW: SMART INTENT ENGINE V4 ++++++++++++++++
# ==================================================
#
def get_smart_match_score(query: str, target: str) -> int:
    """
    Intent Engine Scoring:
    1. First Letter Match: Huge Bonus (Fixes 'Gangers' vs 'Avengers')
    2. Sequence Check: 'Aveger' fits inside 'Avengers' (Fixes typos)
    3. Word Start Check: 'Aveng' matches 'The Avengers' (Fixes Context)
    """
    if not query or not target: return 0
    
    score = 0
    
    # --- RULE 1: FIRST LETTER PRIORITY (The "Gangers" Killer) ---
    # If user types 'A', result MUST start with 'A' (or contain word starting with 'A')
    # We check strict first letter first.
    if query[0] == target[0]:
        score += 150
    
    # --- RULE 2: WORD BOUNDARY MATCH (The "The Avengers" Fix) ---
    # User: "Aveng" -> Matches "The Avengers" because 'Avengers' starts with 'Aveng'
    # We add a space to simulate word boundaries
    if (" " + query) in (" " + target):
        score += 200 # Massive bonus for exact word prefix match
    
    # --- RULE 3: ROBUST SEQUENCE (The "Aveger" -> "Avengers" Fix) ---
    # Check if all query chars exist in target IN ORDER
    last_idx = -1
    matches = 0
    broken = False
    ...
    if not broken:
        score += 200 # Sequence Complete Bonus
        # Tie-breaker: How close are they in length?
        len_diff = abs(len(target) - len(query))
        score -= (len_diff * 5) # Penalty for extra junk chars
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Smart V4 Search:
    1. RapidFuzz (Broad Net - Top 1000).
    2. Intent Engine (Start Letter + Sequence + Word Boundary).
    """
    ...
    try:
        # 1. Clean inputs
        q_tight = clean_text_for_search(query) # "aveger"
        q_fuzzy = clean_text_for_fuzzy(query)  # "aveger"
        
        if not q_tight: return []

        # 2. STEP 1: RAPIDFUZZ BROAD FETCH
        # Get 1000 candidates to ensure we don't miss the target due to bad fuzzy score
        ...
        # 3. STEP 2: INTENT ENGINE RE-RANKING
        # Loop is fast for 1000 items (milliseconds)
        for clean_title_key, fuzz_score, _ in pre_filtered:
            ...
            # Hybrid Scoring:
            # If Intent Score is high (>0), it overrides simple Fuzzy
            if intent_score > 0:
                final_score = 500 + intent_score # Boost above everything else
                match_type = "intent"
            else:
                # Fallback to Fuzzy score if no smart match found
