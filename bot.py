...
async def load_fuzzy_cache(db: Database):
    ...
    async with FUZZY_CACHE_LOCK:
        ...
        try:
            movies = await safe_db_call(db.get_all_movies_for_fuzzy_cache(), timeout=300, default=[])
            if movies:
                fuzzy_movie_cache = movies
                logger.info(f"✅ In-Memory Fuzzy Cache {len(fuzzy_movie_cache):,} titles ke saath loaded.")
            else:
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    ...
    # `process.extract` ko list of choices (clean_title) aur query chahiye.
    # Hum use poora dict pass nahi kar sakte, isliye hum 'clean_title' ki ek list banayenge.
    # `rapidfuzz` string matching ke liye banaya gaya hai.
    
    # Behtar Tareeka: `process.extract` choices ke roop mein poori list le sakta hai,
    # aur hum use batate hain ki kis key (processor) par score karna hai.
    
    try:
        # scorer=fuzz.WAnalog_token_sort_ratio # Yeh complex hai, simple rakhte hain
        
        # `process.extract` `choices` (poori list), `query` (user ki query),
        # `processor` (har item par konsa function lagayein), aur `limit` leta hai.
        results = process.extract(
            query,
            fuzzy_movie_cache,
            processor=lambda movie: movie.get('clean_title', ''),
            scorer=fuzz.WRatio, # Yeh typos ke liye 'token_set_ratio' se behtar hai
            limit=limit,
            score_cutoff=75 # Sirf 75% se zyaada match waale results dein
        )
        
        # Results (choice, score, index) format mein aate hain
        final_list = []
        for (movie_dict, score, index) in results:
            final_list.append({
                'imdb_id': movie_dict['imdb_id'],
                'title': movie_dict['title'],
                'year': movie_dict.get('year'),
                'score': score # Debugging ke liye score add karein
            })
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if not fuzzy_movie_cache:
        # Agar cache kisi wajah se load nahi hua, toh admin ko batayein aur user ko error dein
    ...
    # Task 1: Mongo Exact Search (Async)
    exact_hits_task = safe_db_call(db_primary.mongo_primary_search(clean_query, limit=10), default=[])
    
    # Task 2: Python Fuzzy Search (CPU-bound, Thread mein chalayein)
    loop = asyncio.get_running_loop()
    fuzzy_hits_task = loop.run_in_executor(executor, python_fuzzy_search, clean_query, 10)
    ...
    # Step 3b: Ab Fuzzy hits (Python) daalein (jo pehle se nahi hain)
    if fuzzy_hits:
...
async def get_movie_callback(callback: types.CallbackQuery, db_primary: Database, db_fallback: Database):
    ...
    # --- Extra caption hatane ke liye message edit nahi karenge ---
...
async def migration_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- NAYA: Nayi movie ko fuzzy cache mein bhi add karein ---
    if db1_res is True:
        async with FUZZY_CACHE_LOCK:
            fuzzy_movie_cache.append({
                "imdb_id": imdb_id,
                "title": title,
                "year": year,
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- NAYA: Import ke baad cache ko reload karein ---
    await load_fuzzy_cache(db_primary)
...
async def remove_dead_movie_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- NAYA: Cache se bhi hatayein ---
    if db1_del:
        async with FUZZY_CACHE_LOCK:
            global fuzzy_movie_cache
            fuzzy_movie_cache = [m for m in fuzzy_movie_cache if m['imdb_id'] != imdb_id]
...
async def check_db_command(message: types.Message, db_primary: Database, db_neon: NeonDB):
    ...
    reply_text = (
        ...
        f"<b>Node 1: Mongo (Exact + Fuzzy Source)</b>\n"
