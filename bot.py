...
# --- NEW IMPORTS ---
from core_utils import safe_tg_call, safe_db_call, DB_SEMAPHORE, TELEGRAM_DELETE_SEMAP_RE, TELEGRAM_COPY_SEMAPHORE, TELEGRAM_BROADCAST_SEMAPHORE, WEBHOOK_SEMAPHORE, TG_OP_TIMEOUT, DB_OP_TIMEOUT
...
async def show_admin_panel(message: types.Message, is_edit: bool = False):
    panel_text = (
        ...
        "• <code>/listads</code> - List/Manage Ads\n"
...
async def sync_m12_freeze_fix(message: types.Message, db_primary: Database, db_fallback: Database):
    # Launch as background task to prevent worker timeout
...
# ==========================================
# ORIGINAL ADMIN LOGIC (PRESERVED)
# ==========================================
...
async def remove_dead_movie_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    args = message.text.split(maxsplit=1)
    if len(args) < 2: await safe_tg_call(message.answer("⚠️ **Usage**: /remove_dead_movie `IMDB_ID`"), semaphore=TELEGRAM_COPY_SEMAP_RE); return
