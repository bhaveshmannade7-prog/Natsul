...
# --- NAYA FUNCTION: Rule D Compliant User Input Cleaner (Ban System ke liye) ---
def clean_user_input_for_ban(user_input: str) -> str:
    """Removes telegram handles, links, and non-numeric characters for clean user ID extraction."""
    if not user_input: return ""
    # 1. Remove Telegram prefixes and usernames
    user_input = re.sub(r'https?://t\.me/', '', user_input, flags=re.IGNORECASE)
    user_input = user_input.lstrip('@')
    # 2. Only keep digits and leading minus sign
    if user_input.startswith('-'):
        numeric_part = re.sub(r'[^0-9]+', '', user_input[1:])
...
def get_join_keyboard() -> InlineKeyboardMarkup | None:
    ...
    # FIX: Check karein ki agar value numeric ID hai, toh URL na set karein (private invite link bot nahi bana sakta)
    def is_numeric_id_string(identifier):
        if not identifier: return False
        # Checks if it is numeric string or negative numeric string
        return identifier.isdigit() or (identifier.startswith('-') and identifier[1:].isdigit())
        
    def get_clean_identifier(identifier):
        if not identifier: return None
        # Remove URL and @ sign for display purposes (yeh username display ke liye hai)
        identifier = re.sub(r'https?://t\.me/', '', identifier, flags=re.IGNORECASE)
        return identifier.lstrip('@')
        
    if JOIN_CHANNEL_USERNAME:
        clean_name = get_clean_identifier(JOIN_CHANNEL_USERNAME)
        # Check karte hain ki kya yeh numeric ID hai (jis par URL nahi ban sakta)
        if is_numeric_id_string(clean_name):
             # Numeric ID hai (private channel/group), URL set nahi hoga.
             buttons.append([InlineKeyboardButton(text="📢 Channel Join Karein (Private)", callback_data="no_url_join")])
        else:
             # Username hai, URL set hoga
             # FIX: URL mein https://t.me/ use karein
             buttons.append([InlineKeyboardButton(text="📢 Channel Join Karein", url=f"https://t.me/{clean_name}")])

    if USER_GROUP_USERNAME:
        clean_name = get_clean_identifier(USER_GROUP_USERNAME)
        if is_numeric_id_string(clean_name):
             # Numeric ID hai (private channel/group), URL set nahi hoga.
             buttons.append([InlineKeyboardButton(text="👥 Group Join Karein (Private)", callback_data="no_url_join")])
        else:
             # Username hai, URL set hoga
             # FIX: URL mein https://t.me/ use karein
             buttons.append([InlineKeyboardButton(text="👥 Group Join Karein", url=f"https://t.me/{clean_name}")])
...
async def ensure_capacity_or_inform(
    ...
    # --- NAYA: BAN CHECK ---
    is_banned, ban_info = await safe_db_call(db_primary.is_user_banned(user.id), default=(False, None))
    if is_banned:
        ban_text = f"🚫 <b>Aapko ban kar diya gaya hai.</b>"
        if ban_info and ban_info.get('reason'):
            ban_text += f"\nKaaran: <code>{ban_info['reason']}</code>"
        
        if target_chat_id:
             await safe_tg_call(current_bot.send_message(target_chat_id, ban_text, parse_mode=ParseMode.HTML), semaphore=TELEGRAM_COPY_SEMAPHORE)
        if isinstance(message_or_callback, types.CallbackQuery):
             await safe_tg_call(message_or_callback.answer("🚫 Banned User", show_alert=True))
        logger.warning(f"Banned user {user.id} ne bot use karne ki koshish ki।")
        return False
    # --- END BAN CHECK ---
...
async def start_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        admin_message = (
            ...
            f"  - <b>/export_users</b> 💾 (Users ki list export karein)\n"
            f"  - <b>/ban_user</b> `ID/Reply` | <b>/unban_user</b> `ID`\n"
...
async def no_url_join_callback(callback: types.CallbackQuery):
    # FIX: Yah message tab aata hai jab user Private Join button dabata hai.
    await safe_tg_call(callback.answer("⚠️ Yeh ek Private Chat hai. Kripya Admins ke through ya invite link se manually join karein, aur phir 'Verify' button dabayen।", show_alert=True))
...
async def stats_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    await safe_tg_call(msg.edit_text(admin_message), semaphore=TELEGRAM_COPY_SEMAPHORE)
    # Note: /stats command admin panel mein hai, isliye iske baad koi next step nahi.
...
async def get_user_command(message: types.Message, db_primary: Database):
    ...
    is_banned, ban_info = await safe_db_call(db_primary.is_user_banned(user_id_to_find), default=(False, None))

    if not user_data:
        # FIX: Incomplete ternary operator ko poora kiya
        ban_status = f"🚫 Banned: Yes (Reason: {ban_info['reason']})" if is_banned and ban_info else "✅ Not Found"
        await safe_tg_call(message.answer(f"❌ User <code>{user_id_to_find}</code> database (Mongo 1) mein nahi mila।\nBan Status: {ban_status}"), semaphore=TELEGRAM_COPY_SEMAPHORE); return

    def format_dt(dt): return dt.strftime('%Y-%m-%d %H:%M:%S UTC') if dt else 'N/A'
    
    ban_line = f"🚫 Banned: Yes (Reason: {ban_info['reason']})" if is_banned and ban_info else "✅ Banned: No"
    ...
    user_text = (
        ...
        f"<b>Is Active (Bot):</b> {user_data.get('is_active', True)}\n"
        f"{ban_line}"
    )
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    for i, item in enumerate(mlist):
        ...
        if processed_count % 100 == 0 or (now - start_import_time).total_seconds() > 10 or processed_count == total:
            ...
            db1_tasks, db2_tasks, neon_tasks = [], [], []
            try: await safe_tg_call(msg.edit_text(f"⏳ Processed: {processed_count}/{total:,} | Skipped: {s:,} | Failed: {f:,}"))
            except TelegramBadRequest: pass
            last_update_time = now
...
# --- NAYA FUNCTION: Admin Export Users to JSON ---
async def export_users_to_json(db_primary: Database, bot: Bot, chat_id: int):
    """Users ki list Mongo se fetch karke JSON file mein bhejta hai।"""
    start_time_export = datetime.now(timezone.utc)
    
    msg = await safe_tg_call(bot.send_message(chat_id, "⏳ Users ka data Mongo se fetch kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    
    # Rule: DO NOT add ANY blocking I/O or long waits. Using safe_db_call with default.
    user_list = await safe_db_call(db_primary.get_all_users_for_export(), timeout=120, default=None)
    
    if user_list is None:
        await safe_tg_call(msg.edit_text("❌ Data fetch nahi ho paya ya DB down hai।"))
        return
        
    total_count = len(user_list)
    
    await safe_tg_call(msg.edit_text(f"✅ {total_count:,} user records mile। JSON file banayi ja rahi hai..."))
    
    # JSON serialisation is CPU-bound, use ThreadPoolExecutor
    loop = asyncio.get_running_loop()
    try:
        json_data = await loop.run_in_executor(executor, lambda: json.dumps(user_list, indent=2, ensure_ascii=False))
        file_buffer = io.BytesIO(json_data.encode('utf-8'))
        
        filename = f"user_export_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}_{total_count}.json"
        
        # Telegram Document bhejte waqt safe_tg_call aur Semaphore ka istemal karein
        await safe_tg_call(
            bot.send_document(
                chat_id=chat_id,
                document=BufferedInputFile(file_buffer.getvalue(), filename=filename),
                caption=f"✅ User Data Export (Mongo 1)\nTotal Users: {total_count:,}\nTime Taken: {(datetime.now(timezone.utc) - start_time_export).total_seconds():.2f}s"
            ),
            semaphore=TELEGRAM_COPY_SEMAPHORE,
            timeout=120 
        )
        await safe_tg_call(msg.delete()) # Purana message delete karein
        
    except Exception as e:
        logger.error(f"User Export error: {e}", exc_info=True)
        await safe_tg_call(msg.edit_text(f"❌ User data export karte waqt error: {e}"))

@dp.message(Command("export_users"), AdminFilter())
@handler_timeout(180)
async def export_users_command(message: types.Message, db_primary: Database, bot: Bot):
    # Rule: DO NOT convert async to sync or add ANY blocking I/O
    asyncio.create_task(export_users_to_json(db_primary, bot, message.chat.id))


# --- NAYA FUNCTION: Admin Ban User Command ---
@dp.message(Command("ban_user"), AdminFilter())
@handler_timeout(10)
async def ban_user_command(message: types.Message, db_primary: Database):
    # 1. User ID aur Reason extract karein
    ...
    if message.reply_to_message and message.reply_to_message.from_user:
        user_id_str = str(message.reply_to_message.from_user.id)
        if len(args) > 1: reason = args[1].strip()
    
    # Argument se ID aur Reason lena
    elif len(args) >= 2:
        # Rule D compliant cleaning
        cleaned_input = clean_user_input_for_ban(args[1].strip())
        if cleaned_input.isdigit() or (cleaned_input.startswith('-') and cleaned_input[1:].isdigit()):
            user_id_str = cleaned_input
            if len(args) == 3: reason = args[2].strip()
        else:
             # Agar pehla argument number nahi hai to Invalid syntax
             await safe_tg_call(message.answer("❌ Istemal: /ban_user `USER_ID` [Reason] ya Reply karke /ban_user [Reason]"), semaphore=TELEGRAM_COPY_SEMAPHORE)
             return
    else:
        await safe_tg_call(message.answer("❌ Istemal: /ban_user `USER_ID` [Reason] ya Reply karke /ban_user [Reason]"), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    try:
        ...
        if user_id_to_ban == ADMIN_USER_ID:
            await safe_tg_call(message.answer("❌ Aap khud ko ban nahi kar sakte।"), semaphore=TELEGRAM_COPY_SEMAPHORE)
            return

        # 2. Ban operation
        is_new_ban, was_active_deleted = await safe_db_call(
            db_primary.ban_user(user_id_to_ban, reason, message.from_user.id)
        )
        
        if is_new_ban or not is_new_ban: # Ban operation safal hua
            status_msg = "✅ Banned and Deactivated" if was_active_deleted else "✅ Banned (Already inactive/existing ban updated)"
            
            # User ko notify karne ki koshish (Agar bot us user ko pehle se jaanta hai)
            user_data = await safe_db_call(db_primary.get_user_info(user_id_to_ban))
            username_tag = f"@{user_data['username']}" if user_data and user_data.get('username') else f"<code>{user_id_to_ban}</code>"
            ...
            # Agar reply message tha, to usko delete kar dein taki user ko pata chale
            if message.reply_to_message:
                 await safe_tg_call(message.reply_to_message.delete(), semaphore=TELEGRAM_DELETE_SEMAPHORE)
            
        else:
            await safe_tg_call(message.answer(f"❌ User <code>{user_id_to_ban}</code> ko ban karne mein error aaya।"), semaphore=TELEGRAM_COPY_SEMAPHORE)
            
    except ValueError:
        await safe_tg_call(message.answer("❌ User ID ek valid number hona chahiye।"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    except Exception as e:
        logger.error(f"Ban command error: {e}")
        await safe_tg_call(message.answer(f"❌ Unexpected Error: {e}"), semaphore=TELEGRAM_COPY_SEMAPHORE)

# --- NAYA FUNCTION: Admin Unban User Command ---
@dp.message(Command("unban_user"), AdminFilter())
@handler_timeout(10)
async def unban_user_command(message: types.Message, db_primary: Database):
    ...
    if not user_id_str:
        await safe_tg_call(message.answer("❌ Istemal: /unban_user `USER_ID` ya Reply karke /unban_user"), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    try:
        ...
        # Unban operation
        success = await safe_db_call(
            db_primary.unban_user(user_id_to_unban, message.from_user.id)
        )
        
        if success:
            user_data = await safe_db_call(db_primary.get_user_info(user_id_to_unban))
            username_tag = f"@{user_data['username']}" if user_data and user_data.get('username') else f"<code>{user_id_to_unban}</code>"
            
            await safe_tg_call(message.answer(
                f"✅ User Unbanned!\nUser {username_tag} (ID: <code>{user_id_to_unban}</code>) ko ban list se hata diya gaya hai।"
            ), semaphore=TELEGRAM_COPY_SEMAPHORE)
            await safe_tg_call(message.answer("ℹ️ Note: User ko bot use karne ke liye /start dobara bhejna hoga।"), semaphore=TELEGRAM_COPY_SEMAPHORE)
        else:
             await safe_tg_call(message.answer(f"❌ User <code>{user_id_to_unban}</code> ban list mein nahi mila।"), semaphore=TELEGRAM_COPY_SEMAPHORE)
        ...
        await safe_tg_call(message.answer(f"❌ Unexpected Error: {e}"), semaphore=TELEGRAM_COPY_SEMAPHORE)
# --- END NAYE FUNCTIONS: Admin Ban User Command ---
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    if not message.reply_to_message or not message.reply_to_message.document:
        await safe_tg_call(message.answer("❌ Kripya .json file ko reply k करके command dein।"), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    doc = message.reply_to_message.document
    if not doc.file_name or (not doc.file_name.lower().endswith(".json") and doc.mime_type != "application/json"):
        await safe_tg_call(message.answer
