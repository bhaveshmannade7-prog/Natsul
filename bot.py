...
async def lifespan(app: FastAPI):
    ...
    dp.update.outer_middleware.append(BanCheckMiddleware()) 
...
async def start_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        admin_message = (
            ...
            f"  - <b>/export_users</b> 📄 (Sare users ko JSON me export karein)\n"
            f"  - <b>/ban_user</b> `ID` (User ko ban/unban karein)\n"
        )
...
async def get_user_command(message: types.Message, db_primary: Database):
    ...
    is_banned = await safe_db_call(db_primary.is_user_banned(user_id_to_find), default=False)
    user_text += f"\n<b>Is Banned:</b> {'🔴 YES' if is_banned else '🟢 NO'}"
    # --- END ADDED ---
