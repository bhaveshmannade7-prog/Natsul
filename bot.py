...
# --- FIX 1: MemoryStorage ko RedisStorage se replace karein ---
from aiogram.fsm.storage.memory import MemoryStorage 
from aiogram.fsm.storage.redis import RedisStorage # Naya import
...
try:
    ...
    # --- FIX 2: Redis URL ke aadhar par Storage set karein ---
    if REDIS_URL and "redis" in REDIS_URL:
        # aiogram 3.x ke liye, hum Redis URL directly RedisStorage ko pass karte hain
        storage = RedisStorage.from_url(REDIS_URL)
        logger.info("✅ RedisStorage (State) initialize ho gaya.")
    else:
        # Fallback to MemoryStorage if Redis not configured
        storage = MemoryStorage()
        logger.warning("⚠️ RedisStorage set nahi hua, MemoryStorage istemal hoga (Multi-worker fail ho sakta hai).")
    ...
    # --- NEW: Init Managers ---
    token_manager = TokenManager(bot)
    ...
    logger.info("Bot, Dispatcher, 3 Database objects, TokenManager aur RedisCache initialize ho gaye.")
...
NEON_INIT_LOCK_KEY = "neondb:init:lock"
...
async def shutdown_procedure():
    ...
    # NEW: Stop Managers
    # Redis lock ko shutdown par release karna zaroori hai
    try:
        if redis_cache.enabled:
            await redis_cache.release_lock(NEON_INIT_LOCK_KEY)
            logger.info(f"Released Redis lock: {NEON_INIT_LOCK_KEY}")
    except Exception as e:
        logger.error(f"Error releasing Redis lock during shutdown: {e}")
...
# --- FIX 3: load_fuzzy_cache ko ab lifespan ke bahar define kiya gaya hai,
# taki ise background task mein call kiya ja sake. ---
async def load_fuzzy_cache(db: Database):
    ...
    try:
        # DB call ko Timeout mein wrap karna zaroori hai
...
async def lifespan(app: FastAPI):
    ...
    # --- NEW: Start Managers (Redis Cache sabse pehle zaroori hai) ---
    await token_manager.start(BOT_TOKENS_RAW)
    ...
    # --- FIX 4: NeonDB Init ko Redis Lock se synchronize karein ---
    lock_acquired = False
    if redis_cache.enabled:
        # Lock timeout 5 minute rakhein
        lock_acquired = await redis_cache.acquire_lock(NEON_INIT_LOCK_KEY, timeout=300) 
        if lock_acquired:
            logger.info(f"✅ Redis Lock '{NEON_INIT_LOCK_KEY}' acquired for NeonDB init.")
        else:
            logger.warning(f"⚠️ Redis Lock '{NEON_INIT_LOCK_KEY}' nahi mila, NeonDB init skip (doosra worker kar raha hoga).")

    try:
        # Agar lock mila ya Redis enabled nahi hai, to DB init karein
        if not redis_cache.enabled or lock_acquired: 
            await db_neon.init_db()
            logger.info("Database 3 (NeonDB/Postgres Backup) initialization safal.")
        elif redis_cache.enabled and not lock_acquired:
             # Wait karein agar doosra worker DB init kar raha ho
             logger.info("Waiting for NeonDB init lock to release (Max 60s)...")
             if not await redis_cache.wait_for_lock_release(NEON_INIT_LOCK_KEY, timeout=60):
                 logger.warning("NeonDB init lock release nahi hua, continue kar rahe hain (Potential DB not ready).")
             else:
                 logger.info("NeonDB init lock released, proceeding.")
        ...
        if redis_cache.enabled and lock_acquired:
             await redis_cache.release_lock(NEON_INIT_LOCK_KEY)
             logger.info(f"✅ Redis Lock '{NEON_INIT_LOCK_KEY}' released.")

    # --- FIX 5: Fuzzy Cache Load ko Background Task mein bhej dein ---
    # Taaki webhook jaldi se response de sake
    global monitor_task
    monitor_task = asyncio.create_task(monitor_event_loop())
    asyncio.create_task(load_fuzzy_cache(db_primary)) # Background mein load hoga
    logger.info("Event loop monitor aur Fuzzy Cache load background mein start ho gaya.")
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        admin_message = (
            ...
            f"  - 🤖 Active Tokens: {len(token_manager.bots)}\n"
            f"  - 🧲 Redis Cache: {'Active' if redis_cache.enabled else 'Disabled'}\n"
            f"  - 💾 Bot Storage: {type(dp.storage).__name__}\n\n" # FIX: Storage type dikhayein
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- Fuzzy Cache ko check karein (Agar startup mein load nahi hua) ---
    if not fuzzy_movie_cache:
        logger.warning(f"Fuzzy cache khaali hai, turant load karne ki koshish (user: {user_id}).")
        # Ise background mein rakha tha, ab dobara try karte hain.
        await load_fuzzy_cache(db_primary) 
        if not fuzzy_movie_cache:
            if user.id == ADMIN_USER_ID:
                await safe_tg_call(message.answer("⚠️ ADMIN WARNING: Fuzzy cache ab bhi khaali hai. /reload_fuzzy_cache chalayein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
            else:
    ...
    # --- NAYA HYBRID SEARCH (Smart Sequence > Fuzzy) + REDIS CACHE ---
    
    logger.info(f"User {user_id} searching for '{clean_query}'")
    
    # NEW: Try Redis First
    fuzzy_hits_raw = await redis_cache.get_search_results(clean_query)
    
    if not fuzzy_hits_raw:
        loop = asyncio.get_running_loop()
        # FIX: Ensure executor is not None before running in executor
        if executor:
            fuzzy_hits_task = loop.run_in_executor(executor, python_fuzzy_search, clean_query, 15)
            fuzzy_hits_raw = await fuzzy_hits_task
        else:
            logger.error("Executor is None, running search synchronously (performance hit).")
            fuzzy_hits_raw = python_fuzzy_search(clean_query, 15)
        
        # NEW: Save to Redis
        if fuzzy_hits_raw:
             await redis_cache.set_search_results(clean_query, fuzzy_hits_raw)
...
async def get_movie_callback(callback: types.CallbackQuery, db_primary: Database, db_fallback: Database):
    ...
    try:
        ...
        if is_valid_for_copy:
            # --- UPDATED: Use TokenManager for Load Balancing ---
            copy_result = await token_manager.smart_copy_message(
                chat_id=user.id,
            ...
            elif copy_result is False: error_detail = "Bot blocked, not started by user, or copy forbidden."
            else: error_detail = "Copying fail (timeout, or message deleted)."
        else:
        ...
        if not success:
            ...
            if not movie.get("file_id"):
                ...
                # --- UPDATED: Use TokenManager for Load Balancing ---
                send_result = await token_manager.smart_send_document(
                    chat_id=user.id,
...
async def migration_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    db3_task = safe_db_call(db_neon.add_movie(message_id, channel_id, file_id, file_unique_id, imdb_id, title))
    
    db1_res, db2_res, db3_res = await asyncio.gather(db1_task, db2_task, db3_task)
    
    def get_status(res):
        # NeonDB.add_movie returns boolean, so we check for True
        if res is True and not isinstance(db_primary, bool): return "✅ Added" 
        if res == "updated": return "🔄 Updated"
        if res == "duplicate": return "ℹ️ Skipped"
        if res is True and isinstance(db_primary, bool): return "✅ Synced" # NeonDB
        return "❌ FAILED"

    db1_status = get_status(db1_res)
    db2_status = get_status(db2_res)
    neon_status = get_status(db3_res)
...
async def auto_index_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    db3_task = db_neon.add_movie(message.message_id, message.chat.id, file_id, file_unique_id, imdb_id, title)
    
    async def run_tasks():
        res = await safe_db_call(db1_task)
        await safe_db_call(db2_task)
        await safe_db_call(db3_task)
...
async def stats_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    stats_msg = (
        ...
        f"  - 🤖 Active Worker Bots: {len(token_manager.bots)}\n\n"
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    try:
        ...
        loop = asyncio.get_running_loop()
        # FIX: JSON loading ko threadpool mein chalayein
    ...
    for i, item in enumerate(mlist):
        try:
            ...
            db3_tasks.append(db_neon.add_movie(message_id, channel_id, fid_str, file_unique_id, imdb, title))
        ...
        if (i + 1) % 100 == 0 or (now - start_import_time).total_seconds() > 10 or (i+1) == total:
            await asyncio.gather(
                ...
                *[safe_db_call(task) for task in db3_tasks]
            )
            db1_tasks, db2_tasks, db3_tasks = [], [], []
...
async def remove_dead_movie_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    db3_task = safe_db_call(db_neon.remove_movie_by_imdb(imdb_id))
    ...
    neon_stat = "✅ Neon" if db3_del else "❌ Neon"
...
async def rebuild_clean_titles_m1_command(message: types.Message, db_primary: Database):
    ...
    # MongoDB Text Index ki zaroorat nahi hai agar pure fuzzy search hai
    await safe_tg_call(msg.edit_text(f"✅ (Mongo 1) Rebuild Done!\n{updated:,} titles fix kiye gaye. Total: {total:,}.\n(Text index ki ab zaroorat nahi hai)."))
...
async def force_rebuild_m1_command(message: types.Message, db_primary: Database):
    ...
    # Text Index ko ab rebuild karne ki zaroorat nahi hai
...
async def main_polling():
    ...
    try:
        ...
        # --- NEW: Start Managers (Needed for Redis Lock) ---
        await token_manager.start(BOT_TOKENS_RAW)
        await redis_cache.connect(REDIS_URL)

        # --- FIX: Use Redis Lock for NeonDB initialization (Polling mode ke liye) ---
        lock_acquired = False
        if redis_cache.enabled:
            lock_acquired = await redis_cache.acquire_lock(NEON_INIT_LOCK_KEY, timeout=120)
            if lock_acquired: logger.info(f"✅ Redis Lock '{NEON_INIT_LOCK_KEY}' acquired for NeonDB init.")
