...
def setup_signal_handlers():
    ...
    logger.info("Signal handlers (SIGTERM, SIGINT) set हो गए।")
...
async def wrapped_safe_tg_call(
    ...
    for attempt in range(token_manager.pool_size):
        ...
        try:
            ...
            if attempt == token_manager.pool_size - 1:
                ...
                return None # No wait here, let the next incoming update retry
    ...
    # NAYA PATCH: Agar loop khatam ho gaya aur koi result nahi mila, to log karein
    logger.error("ALL TOKENS FAILED/BLOCKED: Wrapped TG call could not complete after all attempts.")
    return None
...
async def search_movie_handler(
    ...
    if user.id != ADMIN_USER_ID:
        # NAYA PATCH: Membership check se pehle chhota delay
        await asyncio.sleep(0.1) 
        if not await check_user_membership(user.id, token_manager):
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB, token_manager: TokenManager):
    ...
    await load_fuzzy_cache(db_primary)
    await wrapped_safe_tg_call(lambda b: b.send_message(message.chat.id, "✅ Fuzzy cache import ke baad reload ho gaya है।"), token_manager=token_manager)
...
async def rebuild_clean_titles_m1_command(message: types.Message, db_primary: Database, token_manager: TokenManager):
    ...
    await load_fuzzy_cache(db_primary)
    await wrapped_safe_tg_call(lambda b: b.send_message(message.chat.id, "✅ Fuzzy cache bhi reload ho gaya है।"), token_manager=token_manager)
