...
from priority_queue import PriorityQueueWrapper, priority_task_wrapper, Priority
...
try:
    ...
    # --- NAYA: MULTI-TOKEN CONFIG ---
    ALTERNATE_BOT_TOKENS_RAW = os.getenv("ALTERNATE_BOT_TOKENS", "") 
    # FIX APPLIED HERE: Use ALTERNATE_BOT_TOKENS_RAW for the condition.
    ALTERNATE_BOT_TOKENS = [t.strip() for t in ALTERNATE_BOT_TOKENS_RAW.split(',') if t.strip()] if ALTERNATE_BOT_TOKENS_RAW else []
    ...
    # --- NAYA: QUEUE CONCURRENCY ---
    # Fix: Ensure correct env var access for queue size
    QUEUE_MAX_CONCURRENT = int(os.getenv("QUEUE_MAX_CONCURRENT", "200"))
...
try:
    # --- NAYA: Token Manager Init ---
    token_manager = TokenManager(BOT_TOKEN, ALTERNATE_BOT_TOKENS)
    # Primary bot instance ab manager se milta hai
    bot = token_manager.get_current_bot() 
    ...
    # --- NAYA: Redis Cache Init ---
    redis_cache = RedisCache(REDIS_URL)

    # --- NAYA: Priority Queue Init ---
    # Fix: Use the correct concurrency limit setting for the queue.
    queue_wrapper = PriorityQueueWrapper(QUEUE_MAX_CONCURRENT) 
    ...
    dp = Dispatcher(
        ...
        db_neon=db_neon,
        redis_cache=redis_cache, # NAYA
        queue_wrapper=queue_wrapper, # NAYA
        token_manager=token_manager # NAYA
    )
...
async def shutdown_procedure():
    ...
    if WEBHOOK_URL:
        try:
            # Delete webhook for the *primary* bot
            await token_manager.get_current_bot().delete_webhook(drop_pending_updates=True) 
    ...
    # --- NAYA: Token Manager sessions close karein ---
    await token_manager.close_sessions()
...
def handler_timeout(timeout: int = HANDLER_TIMEOUT):
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            try:
                # NAYA: Task wrapper ko seedha await karein agar woh task return karta hai
                result = func(*args, **kwargs)
                if asyncio.iscoroutine(result):
                    result = await asyncio.wait_for(result, timeout=timeout)
                elif isinstance(result, asyncio.Task):
                    # Agar yeh priority_task_wrapper se aaya hai, toh task ka wait karein
                ...
                if target_chat_id:
                    # NAYA: token_manager ka istemal karein
                    await token_manager.safe_tg_call(token_manager.get_current_bot().send_message(target_chat_id, "⚠️ Server request time out ho gayi. Kripya dobara koshish karein."))
                if callback_query:
                    # NAYA: token_manager ka istemal karein
                    await token_manager.safe_tg_call(callback_query.answer("Timeout", show_alert=False))
            except Exception as e:
...
async def safe_db_call(coro: Awaitable[Any], timeout=DB_OP_TIMEOUT, default=None):
    try:
...
# NAYA: Original safe_tg_call ko rename/hataya gaya hai aur iski jagah TokenManager ka wrapper use hoga.
async def safe_tg_call(coro: Awaitable[Any], timeout=TG_OP_TIMEOUT, semaphore: asyncio.Semaphore | None = None) -> Any:
    """
    Ab yeh TokenManager ke wrapper ko call karta hai, jo rotation aur flood control handle karta hai।
    Original coroutine ko token_manager.safe_tg_call mein pass kiya jata hai।
    """
    if semaphore:
        async with semaphore:
            if semaphore != asyncio.Semaphore(1): await asyncio.sleep(0.1) 
            return await token_manager.safe_tg_call(asyncio.wait_for(coro, timeout=timeout))
    else:
        # Default TG calls ke liye timeout aur rotation ka istemal karein
        return await token_manager.safe_tg_call(asyncio.wait_for(coro, timeout=timeout))
...
async def check_user_membership(user_id: int) -> bool:
    ...
    try:
        tasks_to_run = []
        # NAYA: Bot instance TokenManager se lein
        bot_instance = token_manager.get_current_bot()
        if check_channel:
            # safe_tg_call ka istemal karein
            tasks_to_run.append(safe_tg_call(bot_instance.get_chat_member(chat_id=f"@{JOIN_CHANNEL_USERNAME}", user_id=user_id), timeout=5))
        if check_group:
            tasks_to_run.append(safe_tg_call(bot_instance.get_chat_member(chat_id=f"@{USER_GROUP_USERNAME}", user_id=user_id), timeout=5))
...
async def load_fuzzy_cache(db: Database):
    ...
    async with FUZZY_CACHE_LOCK:
        ...
        try:
            # Timeout DB call ke liye zyada rakhein
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Redis Fallback ki wajah se yahan bot ko rokna zaroori hai (agar DB1 main data source ho)
    ...
    try:
        ...
        logger.warning(f"WARNING: Database 2 (MongoDB Fallback) initialize nahi ho paya: {e}", exc_info=True)
        # Fallback hai, toh bot chalta rahega
    ...
    try:
        ...
        logger.warning(f"WARNING: Database 3 (NeonDB/Postgres Backup) initialize nahi ho paya: {e}", exc_info=True)
        # Backup hai, toh bot chalta rahega
    ...
    if WEBHOOK_URL:
        try:
            ...
            if not current_webhook or current_webhook.url != WEBHOOK_URL:
                 ...
                 # Webhook set karne ke liye primary bot ka istemal karein
        ...
        # FIX: Agar SetWebhook par Flood control exceeded error aata hai, toh ignore karein.
        except TelegramRetryAfter as e:
            logger.error(f"Webhook setup mein error: {e}", exc_info=False)
            if "SetWebhook" in str(e):
                logger.warning("Telegram SetWebhook rate limit. Ignoring for now, hoping one worker succeeded.")
            else:
                raise
        except Exception as e:
            logger.error(f"Webhook setup mein unexpected error: {e}", exc_info=True)
...
async def _process_update_safe(update_obj: Update, bot_instance: Bot, token_manager: TokenManager):
    try:
        # NAYA: Dispatcher ko token_manager pass karein
        await dp.feed_update(
            ...
            redis_cache=redis_cache, # NAYA
            token_manager=token_manager, # NAYA
            queue_wrapper=queue_wrapper # FIX: Explicitly pass queue_wrapper for handlers
        )
    except Exception as e:
...
async def bot_webhook(update: dict, background_tasks: BackgroundTasks, request: Request):
    ...
    try:
        ...
        # NAYA: Update process karne ke liye hamesha current primary bot ka istemal karein
        # Aur TokenManager bhi pass karein
        current_bot = token_manager.get_current_bot() 
        # FIX: queue_wrapper ko feed_update mein pass kiya gaya hai.
        background_tasks.add_task(_process_update_safe, telegram_update, current_bot, token_manager)
        return {"ok": True}
    except Exception as e:
...
async def health_check():
    ...
    elif not redis_ok and redis_cache.enabled:
         status_msg = "degraded_redis_cache"
    
    queue_status = queue_wrapper.get_queue_status()
    ...
    return {
        ...
        "redis_cache_connected": redis_ok, # NAYA
        "multi_token_count": len(token_manager.all_tokens), # NAYA
        "search_logic": "Hybrid (Smart Sequence > Fuzzy)",
        "fuzzy_cache_size": len(fuzzy_movie_cache),
        "queue_active_tasks": queue_status['current_active_tasks'], # NAYA
        "queue_max_concurrent": queue_status['max_concurrent'], # NAYA
        "uptime": get_uptime(),
...
async def ensure_capacity_or_inform(
    ...
    # NAYA: token_manager.get_current_bot() ka istemal karein
    bot_instance = token_manager.get_current_bot()
    ...
    if active >= CURRENT_CONC_LIMIT:
        ...
        if target_chat_id:
            # NAYA: safe_tg_call ka istemal karein
            await safe_tg_call(
                bot_instance.send_message(target_chat_id, overflow_message(active), reply_markup=get_full_limit_keyboard()),
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        mongo_1_ready, mongo_2_ready, neon_ready, redis_ready = await asyncio.gather(
            safe_db_call(db_primary.is_ready(), default=False),
            safe_db_call(db_fallback.is_ready(), default=False),
            safe_db_call(db_neon.is_ready(), default=False),
            redis_cache.is_ready() 
        ...
        token_status = f"{len(token_manager.all_tokens)} active tokens."
        ...
        admin_message = (
            ...
            f"  - {status_icon(redis_ready)} Cache 4: Redis Cache (Search Results)\n\n" 
        ...
        # NAYA: safe_tg_call aur current bot ka istemal karein
        await safe_tg_call(bot_instance.send_message(message.chat.id, admin_message), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    if is_member:
        ...
        await safe_tg_call(bot_instance.send_message(message.chat.id, welcome_text, reply_markup=None), semaphore=TELEGRAM_COPY_SEMAPHORE)
    else:
        welcome_text = (
            f"🎬 Namaste <b>{user.first_name}</b>!\n"
        ...
        if join_markup:
            await safe_tg_call(bot_instance.send_message(message.chat.id, welcome_text, reply_markup=join_markup), semaphore=TELEGRAM_COPY_SEMAPHORE)
        else:
            logger.error("User ne start kiya par koi JOIN_CHANNEL/GROUP set nahi hai.")
            await safe_tg_call(bot_instance.send_message(message.chat.id, "Configuration Error: Admin ne join channels set nahi kiye hain."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def help_command(message: types.Message, db_primary: Database):
    ...
    help_text = (
        ...
        "Yeh bot free server par hai. Agar 15 min use na ho, toh server 'so' (sleep) jaata hai. Dobara /start karne par उसे 'jagne' (wake up) mein 10-15 second lag sakte hain. Ek baar jaagne ke baad, search hamesha fast rahegi."
    )
    await safe_tg_call(bot_instance.send_message(message.chat.id, help_text), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def check_join_callback(callback: types.CallbackQuery, db_primary: Database):
    ...
    if is_member:
        ...
        try:
            ...
            await safe_tg_call(bot_instance.send_message(user.id, success_text, reply_markup=None), semaphore=TELEGRAM_COPY_SEMAPHORE)
    else:
...
@priority_task_wrapper("MEDIUM") 
@handler_timeout(20)
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCache):
    user = message.from_user
    if not user: return
    user_id = user.id
    ...
    if len(original_query) < 2:
        await safe_tg_call(bot_instance.send_message(message.chat.id, "🤔 Query bahut chhoti hai. Kam se kam 2 characters likhein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
        
    clean_query = clean_text_for_search(original_query) # "katra"
    if not clean_query:
        await safe_tg_call(bot_instance.send_message(message.chat.id, "🤔 Query mein search karne laayak kuch nahi mila."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
        
    if not fuzzy_movie_cache:
        logger.error(f"FATAL: User {user_id} ne search kiya, lekin fuzzy cache khaali hai!")
        if user.id == ADMIN_USER_ID:
            await safe_tg_call(bot_instance.send_message(message.chat.id, "⚠️ ADMIN WARNING: Fuzzy cache khaali hai. /reload_fuzzy_cache chalayein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        else:
            await safe_tg_call(bot_instance.send_message(message.chat.id, "⚠️ Bot abhi initialize ho raha hai, kripya 30 second baad try karein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return

    searching_msg = await safe_tg_call(bot_instance.send_message(message.chat.id, f"🔎 <b>{original_query}</b> search ho raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not searching_msg: return

    # --- NAYA: REDIS CACHE CHECK ---
    cached_results = await redis_cache.get_search_results(original_query)
    if cached_results:
        logger.info(f"User {user_id} searching for '{clean_query}' - CACHE HIT")
        final_results = cached_results
    else:
        # --- NAYA HYBRID SEARCH (Smart Sequence > Fuzzy) ---
        logger.info(f"User {user_id} searching for '{clean_query}' - CACHE MISS, running search...")
        ...
        # --- NAYA: REDIS CACHE SET ---
        await redis_cache.set_search_results(original_query, final_results)
...
async def get_movie_callback(callback: types.CallbackQuery, db_primary: Database, db_fallback: Database):
    ...
    if not movie:
        ...
        if user.id == ADMIN_USER_ID:
            await safe_tg_call(bot_instance.send_message(user.id, f"ADMIN NOTE: Movie <code>{imdb_id}</code> search mein hai par DBs mein nahi. Please run sync commands."))
        return
    ...
    try:
        ...
        if not success:
            ...
            if not movie.get("file_id"):
                ...
                # NAYA: safe_tg_call ka istemal karein
                send_result = await safe_tg_call(bot_instance.send_document(
                    chat_id=user.id,
    ...
    if success:
        ...
        try:
            ...
            await safe_tg_call(bot_instance.send_message(user.id, error_text), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def migration_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if not message.forward_from_chat or message.forward_from_chat.id != LIBRARY_CHANNEL_ID:
        if LIBRARY_CHANNEL_ID == 0: await safe_tg_call(bot_instance.send_message(message.chat.id, "❌ Migration Error: `LIBRARY_CHANNEL_ID` set nahi hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        else: await safe_tg_call(bot_instance.send_message(message.chat.id, f"Migration ke liye, files ko seedha apne `LIBRARY_CHANNEL` (ID: `{LIBRARY_CHANNEL_ID}`) se forward karein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    if not info or not info.get("title"):
        ...
        await safe_tg_call(bot_instance.send_message(message.chat.id, f"❌ Migration Skipped: MessageID `{message.forward_from_message_id}` ka caption parse nahi kar paya."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
...
async def auto_index_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # NAYA: Task ko queue mein nahi dalte (Channel Post hai), seedha run karte hain
...
async def stats_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    # NAYA: token_manager.get_current_bot() ka istemal karein
    bot_instance = token_manager.get_current_bot()
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "📊 System Telemetry fetch ki ja rahi hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    ...
    stats_msg = (
        ...
        f"  - 🔑 Tokens: {len(token_manager.all_tokens)} active\n\n" 
        f"<b>Node Connectivity</b>\n"
        f"  - {status_icon(mongo_1_ready)} Node 1: Mongo-Primary (Exact Search)\n"
        f"  - {status_icon(mongo_2_ready)} Node 2: Mongo-Fallback (Data Replica)\n"
        f"  - {status_icon(neon_ready)} Node 3: Neon-Backup (File Index)\n"
        f"  - {status_icon(redis_ready)} Cache 4: Redis Cache\n\n" 
        f"<b>Queue Status</b>\n" 
        f"  - 🔄 Active/Waiting Tasks: {queue_status['current_queue_size']}\n"
        f"  - ⚙️ Max Concurrency: {queue_status['max_concurrent']}\n"
        f"  - ℹ️ Active Tasks: {queue_status['current_active_tasks']}\n\n"
        f"<b>Search Logic</b>\n"
        f"  - 🧠 Fuzzy Cache: {len(fuzzy_movie_cache):,} titles loaded\n"
        f"  - <b>{search_status}</b>"
    )
...
async def search_switch_command(message: types.Message):
    # NAYA: token_manager.get_current_bot() ka istemal karein
    bot_instance = token_manager.get_current_bot()
    await safe_tg_call(bot_instance.send_message(message.chat.id, "ℹ️ Search switch ki ab zaroorat nahi hai.\nBot ab hamesha <b>Hybrid</b> ka istemal karta hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def broadcast_command(message: types.Message, db_primary: Database):
    ...
    if not message.reply_to_message:
        await safe_tg_call(bot_instance.send_message(message.chat.id, "❌ Broadcast karne ke liye kisi message ko reply karein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    users = await safe_db_call(db_primary.get_all_users(), timeout=60, default=[])
    if not users:
        await safe_tg_call(bot_instance.send_message(message.chat.id, "❌ Database (Mongo 1) mein koi active users nahi mile."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
        
    total = len(users); msg = await safe_tg_call(bot_instance.send_message(message.chat.id, f"📤 Broadcast shuru... {total:,} users ko target kiya gaya."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def cleanup_users_command(message: types.Message, db_primary: Database):
    # NAYA: token_manager.get_current_bot() ka istemal karein
    bot_instance = token_manager.get_current_bot()
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "🧹 (Mongo 1) 30 din se purane inactive users ko clean kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def get_user_command(message: types.Message, db_primary: Database):
    ...
    if len(args) < 2 or not args[1].isdigit():
        await safe_tg_call(bot_instance.send_message(message.chat.id, "❌ Istemal: /get_user `USER_ID`"), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    user_id_to_find = int(args[1])
    user_data = await safe_db_call(db_primary.get_user_info(user_id_to_find))
    if not user_data:
        await safe_tg_call(bot_instance.send_message(message.chat.id, f"❌ User <code>{user_id_to_find}</code> database (Mongo 1) mein nahi mila."), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    ...
    await safe_tg_call(bot_instance.send_message(message.chat.id, user_text), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if not message.reply_to_message or not message.reply_to_message.document:
        await safe_tg_call(bot_instance.send_message(message.chat.id, "❌ Kripya .json file ko reply karke command dein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    doc = message.reply_to_message.document
    if not doc.file_name or (not doc.file_name.lower().endswith(".json") and doc.mime_type != "application/json"):
        await safe_tg_call(bot_instance.send_message(message.chat.id, "❌ Yeh .json file nahi hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, f"⏳ `{doc.file_name}` download ho raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    try:
        # NAYA: bot_instance ka istemal karein
        ...
        fio = io.BytesIO(); await bot_instance.download_file(file.file_path, fio); fio.seek(0)
        loop = asyncio.get_running_loop()
    ...
    for i, item in enumerate(mlist):
        try:
            ...
            neon_tasks.append(db_neon.add_movie(message_id, channel_id, fid_str, file_unique_id, imdb, title)) # message.message_id ki jagah file ka message_id
    ...
    await load_fuzzy_cache(db_primary)
    await safe_tg_call(bot_instance.send_message(message.chat.id, "✅ Fuzzy cache import ke baad reload ho गया है।"))
...
async def remove_dead_movie_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    args = message.text.split(maxsplit=1)
    if len(args) < 2: await safe_tg_call(bot_instance.send_message(message.chat.id, "❌ Istemal: /remove_dead_movie `IMDB_ID`"), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    imdb_id = args[1].strip()
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, f"⏳ <code>{imdb_id}</code> ko teeno DBs se hataya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def cleanup_mongo_1_command(message: types.Message, db_primary: Database):
    ...
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "⏳ (Mongo 1) mein `imdb_id` duplicates dhoondhe ja rahe hain (Batch: 100)..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def cleanup_mongo_2_command(message: types.Message, db_fallback: Database):
    ...
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "⏳ (Mongo 2) mein `imdb_id` duplicates dhoondhe ja rahe hain (Batch: 100)..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def remove_library_duplicates_command(message: types.Message, db_neon: NeonDB):
    ...
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "⏳ NeonDB se `file_unique_id` duplicates dhoondhe ja rahe hain... (Batch: 100)"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    ...
    async def delete_message(msg_id: int, chat_id: int):
        ...
        # NAYA: safe_tg_call ka istemal karein
        res = await safe_tg_call(bot_instance.delete_message(chat_id=chat_id, message_id=msg_id), semaphore=TELEGRAM_DELETE_SEMAPHORE)
...
async def backup_channel_command(message: types.Message, db_neon: NeonDB):
    ...
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        await safe_tg_call(bot_instance.send_message(message.chat.id, "❌ Istemal: /backup_channel `BACKUP_CHANNEL_ID_OR_USERNAME`"), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    try:
        ...
        await safe_tg_call(bot_instance.send_message(message.chat.id, f"❌ Error: {e}"), semaphore=TELEGRAM_COPY_SEMAPHORE); return

    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "⏳ NeonDB se unique files ki list fetch ki ja rahi hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    ...
    async def copy_file(msg_id: int, chat_id: int):
        ...
        res = await safe_tg_call(
            bot_instance.copy_message(chat_id=target_channel, from_chat_id=chat_id, message_id=msg_id),
...
async def sync_mongo_1_to_neon_command(message: types.Message, db_primary: Database, db_neon: NeonDB):
    ...
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "⏳ (Mongo 1) se sabhi movies fetch ki ja rahi hain..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def rebuild_clean_titles_m1_command(message: types.Message, db_primary: Database):
    ...
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "⏳ (Mongo 1) mein `clean_title` field ko rebuild kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    ...
    await load_fuzzy_cache(db_primary)
    await safe_tg_call(bot_instance.send_message(message.chat.id, "✅ Fuzzy cache bhi reload ho गया है।"))
...
async def rebuild_clean_titles_m2_command(message: types.Message, db_fallback: Database):
    ...
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "⏳ (Mongo 2) mein `clean_title` field ko rebuild kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def set_limit_command(message: types.Message):
    ...
    if len(args)<2 or not args[1].isdigit(): await safe_tg_call(bot_instance.send_message(message.chat.id, f"Istemal: /set_limit N (Abhi: {CURRENT_CONC_LIMIT})"), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    try:
        val = int(args[1]); assert 5 <= val <= 5000 
        CURRENT_CONC_LIMIT = val; await safe_tg_call(bot_instance.send_message(message.chat.id, f"✅ Concurrency limit ab {CURRENT_CONC_LIMIT} par set hai."), semaphore=TELEGRAM_COPY_SEMAPHORE); logger.info(f"Concurrency limit admin ne {CURRENT_CONC_LIMIT} kar diya hai.")
    except (ValueError, AssertionError): await safe_tg_call(bot_instance.send_message(message.chat.id, "❌ Limit 5 aur 5000 ke beech ek number hona chahiye."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def rebuild_neon_vectors_command(message: types.Message, db_neon: NeonDB):
    ...
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "⏳ (NeonDB Backup) mein `clean_title` aur FTS Data ko rebuild kiya ja raha hai... (Jo NULL hain)"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def reload_fuzzy_cache_command(message: types.Message, db_primary: Database):
    ...
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "🧠 In-Memory Fuzzy Cache ko Mongo se reload kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    await load_fuzzy_cache(db_primary)
...
async def check_db_command(message: types.Message, db_primary: Database, db_neon: NeonDB):
    ...
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, "🕵️‍♂️ Diagnostics run ki ja rahi hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def errors_handler(update: types.Update, exception: Exception):
    ...
    if target_chat_id:
        try: 
            # NAYA: token_manager.get_current_bot() ka istemal karein
            await token_manager.get_current_bot().send_message(target_chat_id, error_message) 
...
async def main_polling():
    logger.warning("Local polling is disabled in production webhook mode.")
    # Agar aapko phir bhi local mein chalana hai, toh WEBHOOK_URL ko empty rakhein.
...
if __name__ == "__main__":
    ...
    if not WEBHOOK_URL:
        ...
        # FastAPI Uvicorn mode ke liye koi blocking code yahan nahi hona chahiye.
        logger.error("WEBHOOK_URL set hai. Local polling nahi chalega. Running FastAPI app via Gunicorn/Uvicorn.")
