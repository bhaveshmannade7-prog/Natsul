...
try:
    ...
    ALTERNATE_BOT_TOKENS = [t.strip() for t in ALTERNATE_BOT_TOKENS_RAW.split(',') if t.strip()] if ALTERNATE_BOTS_RAW else []
    ...
    QUEUE_MAX_CONCURRENT = int(os.getenv("CONCURRENT_LIMIT", "200")) # Use CONCURRENT_LIMIT key 
...
try:
    ...
    # Fix: Use the correct concurrency limit setting
...
async def shutdown_procedure():
    ...
    try:
        ...
        if db_fallback and db_fallback.client:
            ...
            logger.info("MongoDB (Fallback) client connection close ho गया।")
        if db_neon:
            await db_neon.close()
