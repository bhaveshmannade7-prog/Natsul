...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Smart Hybrid Search:
    1. Limit = 3000 (Badi range taaki 'Kantara' miss na ho).
    2. Sequence Logic: Agar spelling order sahi hai, toh Massive Boost (+40).
    """
    ...
    try:
        ...
        # --- FIX: LIMIT INCREASED TO 3000 ---
        # Pehle 100 tha, isliye junk files (Kaali/Katha) asli movie ko dhakel rahi thi.
        # 3000 karne se 'Kantara' list mein aa jayegi.
        ...
        for (matched_title, base_score, _) in candidates:
            ...
            # --- LOGIC: SEQUENCE BONUS ---
            # Katra -> Kantara (Match! +40 Points) -> Score 130+ (TOP #1)
            # Katra -> Katha (Fail! No Bonus) -> Score 72 (Low)
            if is_subsequence(query, matched_title):
                final_score += 40
            
            # Startswith Bonus (e.g. 'Avtar' -> 'Avatar')
            if matched_title.startswith(query):
                final_score += 10
            
            # Exact Match Bonus
            if matched_title == query:
                final_score += 20
        ...
        # Re-sort karke top results nikalna
