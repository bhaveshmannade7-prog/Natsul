...
# ============ CONFIGURATION ============
try:
    # --- Main Token (Webhook/Primary Bot) ---
    BOT_TOKEN_MAIN = os.environ["BOT_TOKEN"]
    
    # --- Worker Tokens (Comma-separated, Optional) ---
    BOT_TOKENS_WORKERS_RAW = os.getenv("BOT_TOKENS", "")
    BOT_TOKENS_WORKERS = [t.strip() for t in BOT_TOKENS_WORKERS_RAW.split(',') if t.strip()]

    # --- NAYA LOGIC: Combine Main Token and Worker Tokens into one pool ---
    # Main token pool mein Main Token ko hamesha pehle rakhein
    BOT_TOKENS_POOL = [BOT_TOKEN_MAIN] 
    BOT_TOKENS_POOL.extend(BOT_TOKENS_WORKERS)
    
    # Yeh token webhook URL banane ke liye use hoga (Main Token)
    BOT_TOKEN_WEBHOOK = BOT_TOKEN_MAIN 
...
def build_webhook_url() -> str:
    ...
    if base:
        base = base.rstrip("/")
        # BOT_TOKEN_WEBHOOK ka istemal karein (Main Token)
...
try:
    ...
    token_manager = TokenManager(BOT_TOKENS_POOL) # Naya Manager, combined pool use karein
    ...
    # Hum Main token ka bot instance use karenge sirf Dispatcher initialization ke liye
    initial_bot_instance = token_manager.token_pool[BOT_TOKEN_MAIN]['bot'] 
    ...
    # --- NAYA: Debug Middleware Register karein ---
...
async def wrapped_safe_tg_call(
    ...
    for attempt in range(token_manager.pool_size):
        ...
        try:
            ...
            # --- NAYA PATCH: Admin ko Flood Wait Alert bhejein ---
            if ADMIN_USER_ID != 0:
                 admin_alert_text = (
                     f"🚨 **URGENT: Flood Wait Detected!**\n"
                     f"Token ending `{str(current_bot.id)[-4:]}` has been rate-limited.\n"
                     f"Switching to next available token. Cooldown: {e.retry_after:.1f}s."
                 )
                 try:
                     # Alert bhejne ke liye Main bot instance use karein (jo active ho)
                     await token_manager.token_pool[BOT_TOKEN_MAIN]['bot'].send_message(
                         ADMIN_USER_ID, admin_alert_text, parse_mode=ParseMode.MARKDOWN
                     )
                 except Exception as alert_e:
                     logger.error(f"Admin ko flood alert bhejne mein error: {alert_e}")
            # --- END NAYA PATCH ---
...
async def cached_python_fuzzy_search_wrapper(
    ...
    # NOTE: Fix for Redis compatibility - query empty nahi hona chahiye
    ...
    # NAYA PATCH: Agar cached_results None/empty list/dict aata hai, toh use skip karke full search karein
    if cached_results is not None and isinstance(cached_results, list) and len(cached_results) > 0:
    ...
    fuzzy_hits_task = loop.run_in_executor(executor, python_fuzzy_search, clean_query, 50) # Thoda zyada limit
...
# ============ FILTERS & HELPER FUNCTIONS (Logic unchanged) ============
...
async def check_user_membership(user_id: int, token_manager: TokenManager) -> bool:
    # --- NAYA PATCH: Admin ID ko membership check se chhoot (exempt) milega ---
...
async def lifespan(app: FastAPI):
    ...
    # --- NAYA PATCH: Webhook Flood Wait/Race Condition Fix ---
    await asyncio.sleep(2.0) # 2-second delay to synchronize workers

    if WEBHOOK_URL:
        try:
            # TokenManager ka istemal karein, Main Token ka bot object is already available in the pool
            bot_instance_for_webhook = token_manager.token_pool[BOT_TOKEN_WEBHOOK]['bot']
            current_webhook = await bot_instance_for_webhook.get_webhook_info()
            
            # Check if the previous attempt failed due to flood wait recently (within 60s)
            if current_webhook.last_error_date is not None and (time.time() - current_webhook.last_error_date) < 60 and "flood" in current_webhook.last_error_message.lower():
                 logger.warning("Webhook skipping set_webhook: Pichhla attempt 60s ke andar flood-wait mein fail hua tha.")
                 
            elif not current_webhook or current_webhook.url != WEBHOOK_URL:
...
@app.post(f"/bot/{BOT_TOKEN_WEBHOOK}") # Ab Main token webhook path mein
...
async def ensure_capacity_or_inform(
    ...
    if user.id == ADMIN_USER_ID: 
        logger.info(f"Admin user {user.id} ke liye capacity check skip kiya.") 
        return True
...
async def start_command(
    ...
    if user_id == ADMIN_USER_ID:
        ...
        await wrapped_safe_tg_call(lambda b: b.send_message(message.chat.id, admin_message), semaphore=TELEGRAM_COPY_SEMAPHORE, token_manager=token_manager)
        return
...
async def help_command(message: types.Message, db_primary: Database, token_manager: TokenManager):
    ...
    help_text = (
        ...
        "Yeh bot free server par hai. Agar 15 min use na ho, toh server 'so' (sleep) jaata hai. Dobara /start karne par use 'jagne' (wake up) mein 10-15 second lag sakte hain. Ek baar jaagne ke baad, search hamesha fast raheगी।"
    )
...
async def check_join_callback(callback: types.CallbackQuery, db_primary: Database, token_manager: TokenManager):
    ...
    # ensure_capacity_or_inform mein Admin check shamil hai
...
async def search_movie_handler(
    ...
    # --- MEMBERSHIP CHECK (Non-Admin users ke liye search block) ---
    if user.id != ADMIN_USER_ID and not await check_user_membership(user.id, token_manager):
        logger.warning(f"User {user.id} ne search kiya, lekin membership fail hui. Search block kiya.")
        join_markup = get_join_keyboard()
        if join_markup:
            await wrapped_safe_tg_call(
                lambda b: b.send_message(
                    message.chat.id, 
                    "❌ Search karne se pehle, kripya zaroori Channel aur Group join karein.", 
                    reply_markup=join_markup
                ), 
                semaphore=TELEGRAM_COPY_SEMAPHORE, token_manager=token_manager)
        else:
            await wrapped_safe_tg_call(lambda b: b.send_message(message.chat.id, "❌ Aapko membership check pass karna hoga."), semaphore=TELEGRAM_COPY_SEMAPHORE, token_manager=token_manager)
        return
...
async def get_movie_callback(callback: types.CallbackQuery, db_primary: Database, db_fallback: Database, token_manager: TokenManager):
    ...
    # --- MEMBERSHIP CHECK (Non-Admin users ke liye search block) ---
    if user.id != ADMIN_USER_ID and not await check_user_membership(user.id, token_manager):
        logger.warning(f"User {user.id} ne callback kiya, lekin membership fail hui. File access block kiya.")
        join_markup = get_join_keyboard()
        await wrapped_safe_tg_call(
            lambda b: b.send_message(
                callback.message.chat.id, 
                "❌ File access karne se pehle, kripya zaroori Channel aur Group join karein.", 
                reply_markup=join_markup
            ), 
            semaphore=TELEGRAM_COPY_SEMAPHORE, token_manager=token_manager)
        return
...
async def broadcast_command(message: types.Message, db_primary: Database, token_manager: TokenManager):
    ...
    async def send_to_user(user_id: int):
        ...
        res = await wrapped_safe_tg_call(
            lambda b: message.reply_to_message.copy_to(user_id, bot=b), 
            timeout=10, 
...
async def get_user_command(message: types.Message, db_primary: Database, token_manager: TokenManager):
    ...
    if len(args) < 2 or not args[1].isdigit():
        await wrapped_safe_tg_call(lambda b: b.send_message(message.chat.id, "❌ Istemal: /get_user `USER_ID`"), semaphore=TELEGRAM_COPY_SEMAPHORE, token_manager=token_manager); return
    user_id_to_find = int(args[1])
    user_data = await safe_db_call(db_primary.get_user_info(user_id_to_find))
    if not user_data:
        await wrapped_safe_tg_call(lambda b: b.send_message(message.chat.id, f"❌ User <code>{user_id_to_find}</code> database (Mongo 1) mein nahi mila."), semaphore=TELEGRAM_COPY_SEMAPHORE, token_manager=token_manager); return
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB, token_manager: TokenManager):
    if not message.reply_to_message or not message.reply_to_message.document:
        await wrapped_safe_tg_call(lambda b: b.send_message(message.chat.id, "❌ Kripya .json file ko reply karke command dein."), semaphore=TELEGRAM_COPY_SEMAPHORE, token_manager=token_manager); return
    doc = message.reply_to_message.document
    if not doc.file_name or (not doc.file_name.lower().endswith(".json") and doc.mime_type != "application/json"):
        await wrapped_safe_tg_call(lambda b: b.send_message(message.chat.id, "❌ Yeh .json file nahi hai."), semaphore=TELEGRAM_COPY_SEMAPHORE, token_manager=token_manager); return
...
async def backup_channel_command(message: types.Message, db_neon: NeonDB, token_manager: TokenManager):
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        await wrapped_safe_tg_call(lambda b: b.send_message(message.chat.id, "❌ Istemal: /backup_channel `BACKUP_CHANNEL_ID_OR_USERNAME`"), semaphore=TELEGRAM_COPY_SEMAPHORE, token_manager=token_manager); return
...
async def force_rebuild_m1_command(message: types.Message, db_primary: Database, token_manager: TokenManager):
    ...
    await load_fuzzy_cache(db_primary)
    await wrapped_safe_tg_call(lambda b: b.send_message(message.chat.id, "✅ Fuzzy cache bhi reload ho गया है।"), token_manager=token_manager)
...
async def rebuild_neon_vectors_command(message: types.Message, db_neon: NeonDB, token_manager: TokenManager):
    ...
    if updated_count >= 0:
        ...
        await wrapped_safe_tg_call(
            lambda b: b.edit_message_text(chat_id=msg.chat.id, message_id=msg.message_id, text="❌ (NeonDB Backup) FTS vectors ko rebuild karne mein error आया।"),
            token_manager=token_manager
        )
...
async def check_db_command(message: types.Message, db_primary: Database, db_neon: NeonDB, token_manager: TokenManager):
    ...
    reply_text = (
        ...
        f"ℹ️ Agar 'Cleaned' field <b>'--- KHAALI HAI ---'</b> dikha raha hai, iska matlab aapko zaroori <b>/rebuild...</b> command dobara chalana होगा।"
    )
    await wrapped_safe_tg_call(
