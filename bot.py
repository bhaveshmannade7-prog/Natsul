...
async def ban_user_command(message: types.Message, db_primary: Database):
    ...
    reason = message.text.split(maxsplit=2)[-1] if len(message.text.split()) > 2 else "Admin action."

    banned = await safe_db_call(db_primary.ban_user(target_id, reason))
    
    if banned:
        await safe_tg_call(message.answer(f"✅ User <code>{target_id}</code> ko safaltapoorvak ban kar diya gaya hai।"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    else:
...
async def backup_channel_command(message: types.Message, db_neon: NeonDB):
    ...
    msg = await safe_tg_call(message.answer("⏳ NeonDB se unique files ki list fetch ki ja रही hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    doc = message.reply_to_message.document
    if not doc.file_name or (not doc.file_name.lower().endswith(".json") and doc.mime_type != "application/json"):
        await safe_tg_call(message.answer("❌ Yeh .json file nahi hai।"), semaphore=TELEGRAM_COPY_SEMAP
