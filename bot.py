...
async def cached_python_fuzzy_search_wrapper(
    ...
    # NAYA PATCH: JSON dumps mein None values ko hata diya gaya hai, lekin yahan store karte waqt None ko filter karna safe hai.
    # Python dicts mein None values ko store karne se pehle sanitize karna zaroori nahi hai, lekin JSON library handles it.
    if final_results:
...
async def ensure_capacity_or_inform(
    ...
    if user.id == ADMIN_USER_ID: 
        logger.info(f"Admin user {user.id} ke liye capacity check skip kiya.") # NAYA PATCH: Diagnostic log
        return True
