...
# --- NEW IMPORTS ---
# F.I.X: Yahan DB_SEMAPHORE import ho raha hai, lekin yeh ab safe_db_call ke andar hi use hoga
...
async def shutdown_procedure():
    ...
    try:
        if db_primary and db_primary.client:
            # db_primary.client.close() is synchronous, no need for safe_db_call here in shutdown
            db_primary.client.close()
            logger.info("MongoDB (Primary) client connection close ho gaya.")
        if db_fallback and db_fallback.client:
            # db_fallback.client.close() is synchronous
...
class BannedFilter(BaseFilter):
    async def __call__(self, message: types.Message, db_primary: Database) -> bool:
        ...
        # safe_db_call will run db_primary.is_user_banned synchronously in executor
...
async def load_fuzzy_cache(db: Database):
    ...
    async with FUZZY_CACHE_LOCK:
        ...
        try:
            # db.get_all_movies_for_fuzzy_cache is a synchronous call, now run in executor via safe_db_call
...
async def lifespan(app: FastAPI):
    ...
    # F.I.X: Executor ko default set karna, jiska upyog safe_db_call karega
    ...
    try:
        # init_db is a synchronous call, now run in executor via safe_db_call
    ...
    try:
        # NeonDB init mein ab concurrency lock hai 
        await db_neon.init_db() # Assuming db_neon uses async driver (e.g., asyncpg)
    ...
    # FIX for AttributeError: check_if_lock_exists is a synchronous call, now run in executor via safe_db_call
    ...
    if not is_webhook_already_set:
        ...
        # Lock acquire karne ki koshish (timeout 300 seconds). Synchronous call.
        ...
        if lock_acquired:
            try:
                ...
                # Lock release karna bahut zaroori hai! Synchronous call.
...
async def health_check():
    ...
    # All check methods are synchronous, now run in executor via safe_db_call
...
async def ensure_capacity_or_inform(
    ...
    # db_primary.add_user is a synchronous call, now run in executor via safe_db_call
    await safe_db_call(db_primary.add_user(user.id, user.username, user.first_name, user.last_name))
    
    if user.id == ADMIN_USER_ID: 
        return True
        
    # get_concurrent_user_count is a synchronous call, now run in executor via safe_db_call
...
async def check_join_callback(callback: types.CallbackQuery, bot: Bot, db_primary: Database, redis_cache: RedisCacheLayer):
    ...
    # is_user_banned is a synchronous call, now run in executor via safe_db_call
    is_banned = await safe_db_call(db_primary.is_user_banned(user.id), default=False)
    if is_banned:
...
async def search_movie_handler(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    loop = asyncio.get_running_loop()
    # python_fuzzy_search ab original_query leta hai (fetching up to 45 results for pagination logic)
...
async def refresh_search_callback(callback: types.CallbackQuery, bot: Bot, redis_cache: RedisCacheLayer):
    ...
    if redis_cache.is_ready():
        # redis_cache.incr is async
        await redis_cache.incr(limit_key)
        # redis_cache.expire is async
...
async def get_movie_callback(callback: types.CallbackQuery, bot: Bot, db_primary: Database, db_fallback: Database, redis_cache: RedisCacheLayer):
    ...
    # get_movie_by_imdb is a synchronous call, now run in executor via safe_db_call
...
async def migration_handler(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    if not message.forward_from_chat or message.forward_from_chat.id != LIBRARY_CHANNEL_ID:
        ...
        else: await safe_tg_call(message.answer(f"❌ **Invalid Source**: Forward from Library Channel (ID: `{LIBRARY_CHANNEL_ID}`) only."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    if not info or not info.get("title"):
        ...
        await safe_tg_call(message.answer(f"❌ **Parse Error**: Caption missing/invalid for MsgID `{message.forward_from_message_id}`."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    # All add_movie calls are synchronous, now run in executor via safe_db_call
    ...
    neon_task = safe_db_call(db_neon.add_movie(message_id, channel_id, file_id, file_unique_id, imdb_id, title)) # Assuming db_neon.add_movie is async
...
async def auto_index_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    # db.add_movie is a synchronous call, now run in executor via safe_db_call
    ...
    neon_task = db_neon.add_movie(message.message_id, message.chat.id, file_id, file_unique_id, imdb_id, title) # Assuming db_neon.add_movie is async
    
    async def run_tasks():
        res = await db1_task # Already wrapped in safe_db_call (a future/coroutine)
        await db2_task
        await safe_db_call(neon_task) # Wrap async neon task in safe_db_call
...
async def stats_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    # All sync calls are now wrapped in safe_db_call
    ...
    search_status = f"⚡ Hybrid (Smart V5)"
...
async def show_admin_panel(message: types.Message, is_edit: bool = False):
    panel_text = (
        ...
        "⚙️ **CONFIG**\n"
        "• <code>/set_limit N</code> - Set Concurrency Limit"
    )
...
async def broadcast_command(message: types.Message, db_primary: Database):
    ...
    # get_all_users is a synchronous call, now run in executor via safe_db_call
    ...
    async def send_to_user(user_id: int):
        ...
        if res: success_count += 1
            ...
            failed_count += 1
            # deactivate_user is a synchronous call, now run in executor via safe_db_call
...
async def cleanup_users_command(message: types.Message, db_primary: Database):
    ...
    if not msg: return
    # cleanup_inactive_users is a synchronous call, now run in executor via safe_db_call
    removed = await safe_db_call(db_primary.cleanup_inactive_users(days=30), timeout=90, default=0)
    # get_user_count is a synchronous call, now run in executor via safe_db_call
...
async def get_user_command(message: types.Message, db_primary: Database):
    ...
    # get_user_info is a synchronous call, now run in executor via safe_db_call
...
async def export_users_command(message: types.Message, db_primary: Database):
    ...
    # get_all_user_details is a synchronous call, now run in executor via safe_db_call
...
async def ban_user_command(message: types.Message, db_primary: Database):
    ...
    # ban_user is a synchronous call, now run in executor via safe_db_call
...
async def unban_user_command(message: types.Message, db_primary: Database):
    ...
    # unban_user is a synchronous call, now run in executor via safe_db_call
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    for i, item in enumerate(mlist):
        ...
        try:
            ...
            # db_neon.add_movie is async, but wrapping it in safe_db_call is safe and handles possible blocking in a separate executor (if needed)
...
async def remove_dead_movie_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # All remove_movie_by_imdb calls are synchronous, now run in executor via safe_db_call
    ...
    neon_task = safe_db_call(db_neon.remove_movie_by_imdb(imdb_id)) # Assuming db_neon.remove_movie_by_imdb is async/sync
...
async def cleanup_mongo_1_command(message: types.Message, db_primary: Database):
    ...
    if not msg: return
    # cleanup_mongo_duplicates is a synchronous call, now run in executor via safe_db_call
...
async def backup_channel_command(message: types.Message, db_neon: NeonDB):
    ...
    # get_unique_movies_for_backup is async/sync, now run in executor via safe_db_call
...
async def sync_mongo_1_to_neon_command(message: types.Message, db_primary: Database, db_neon: NeonDB):
    ...
    # get_all_movies_for_neon_sync is a synchronous call, now run in executor via safe_db_call
    ...
    # sync_from_mongo is async/sync, now run in executor via safe_db_call
...
async def sync_mongo_1_to_2_command(message: types.Message, db_primary: Database, db_fallback: Database):
    ...
    for movie in mongo_movies_full:
        ...
        # F.I.X: task ko safe_db_call से बनाएं (ये blocking I/O को executor में भेजेगा)
...
async def rebuild_clean_titles_m1_command(message: types.Message, db_primary: Database):
    ...
    if not msg: return
    # rebuild_clean_titles is a synchronous call, now run in executor via safe_db_call
    updated, total = await safe_db_call(db_primary.rebuild_clean_titles(clean_text_for_search), timeout=240, default=(0,0))
    # create_mongo_text_index is a synchronous call, now run in executor via safe_db_call
...
async def rebuild_neon_vectors_command(message: types.Message, db_neon: NeonDB):
    ...
    if not msg: return
    # rebuild_fts_vectors is async/sync, now run in executor via safe_db_call
...
async def check_db_command(message: types.Message, db_primary: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    # check_mongo_clean_title is a synchronous call, now run in executor via safe_db_call
    mongo_check_task = safe_db_call(db_primary.check_mongo_clean_title, default={"title": "Error", "clean_title": "Mongo check failed"})
    # check_neon_clean_title is async/sync, now run in executor via safe_db_call
