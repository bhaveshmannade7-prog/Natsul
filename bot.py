...
# --- NAYE IMPORTS ---
from token_manager import TokenManager # Feature 1
from redis_cache import SearchCache   # Feature 2
...
logging.getLogger("redis_cache").setLevel(logging.INFO)
logging.getLogger("token_manager").setLevel(logging.INFO)


# ============ CONFIGURATION ============
try:
    # --- NAYA: BOT_TOKEN ko BOT_TOKENS mein badle ---
    BOT_TOKENS_RAW = os.environ["BOT_TOKENS"] # Primary token aur alternates, comma separated
    ...
    # --- ALTERNATE_BOTS ko remove kiya gaya, ab BOT_TOKENS_RAW mein sab hai ---
    # Par hume existing ALTERNATE_BOTS logic ko hataana nahi hai, sirf re-use karna hai.
    # Existing ALTERNATE_BOTS ko ek list mein rakhe
...
# ============ WEBHOOK URL ============
def build_webhook_url(main_token: str) -> str: # NAYA: main_token argument
    base = RENDER_EXTERNAL_URL or PUBLIC_URL
    if base:
        base = base.rstrip("/")
        # NAYA: Webhook path hamesha MAIN token ke naam par hoga
        webhook_path = f"/bot/{main_token}" 
...
try:
    # NAYA: BOT_TOKEN_MANAGER initialize karein (Feature 1)
    all_tokens = [t.strip() for t in BOT_TOKENS_RAW.split(',') if t.strip()]
    MAIN_BOT_TOKEN = all_tokens[0]
    FALLBACK_TOKENS = all_tokens[1:]
    
    token_manager = TokenManager(MAIN_BOT_TOKEN, FALLBACK_TOKENS)
    # FIX: get_main_bot ab synchronous hai, so no await here
    bot = token_manager.get_main_bot() # Main bot instance for Dispatcher, Webhook Setup
    ...
    # --- NAYA: Redis Search Cache Object ---
    ...
    dp = Dispatcher(
        ...
        db_neon=db_neon,
        search_cache=search_cache, # NAYA: Search Cache inject karein
        token_manager=token_manager # NAYA: Token Manager inject karein
    )
    
    logger.info("Bot, Dispatcher, 3 Database objects (M+M+N), TokenManager, aur SearchCache initialize ho gaye.")
except Exception as e:
    logger.critical(f"Bot/Dispatcher initialize nahi ho paya: {e}", exc_info=True)
    raise SystemExit("Bot initialization fail.")
    
# NAYA: Webhook URL ko ab MAIN_BOT_TOKEN se banaayein
WEBHOOK_URL = build_webhook_url(MAIN_BOT_TOKEN)
...
async def shutdown_procedure():
    ...
    if WEBHOOK_URL:
        try:
            # FIX: get_main_bot is now sync, so no await here
            await token_manager.get_main_bot().delete_webhook(drop_pending_updates=True) 
    ...
    # NAYA: Sabhi bot sessions close karein
...
def handler_timeout(timeout: int = HANDLER_TIMEOUT):
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            # Message/Callback se user_id nikaalna
            user_id = None
            if args:
                if isinstance(args[0], types.Message):
                    target_chat_id = args[0].chat.id
                    user_id = args[0].from_user.id if args[0].from_user else None
            ...
            # NAYA: Bot instance ko TokenManager se fetch karein (User-specific load balancing)
            bot_instance = await token_manager.get_bot(user_id=user_id) 

            try:
                # NAYA: Wrapped function mein bot instance pass karein
                await asyncio.wait_for(func(bot=bot_instance, *args, **kwargs), timeout=timeout) 
            except asyncio.TimeoutError:
                logger.error(f"Handler {func.__name__} {timeout}s ke baad time out ho gaya.")
                if target_chat_id:
                    try: 
                        await bot_instance.send_message(target_chat_id, "⚠️ Server request time out ho gayi. Kripya dobara koshish karein.")
        ...
        # NAYA: Return wrapper function jo 'bot' keyword argument leta hai.
        # Original handler functions mein 'bot' argument add karna hoga.
        return wrapper
    return decorator
...
# NAYA: safe_tg_call ko modify karein Flood Wait aur TokenManager ke liye (Feature 1)
async def safe_tg_call(coro, timeout=TG_OP_TIMEOUT, semaphore: asyncio.Semaphore | None = None):
    ...
    # Coroutine ke bot instance ko nikalna
    if not hasattr(coro, '__self__') or not isinstance(coro.__self__, Bot):
        logger.warning("safe_tg_call: Coro mein Bot instance nahi mila. Default Bot istemal hoga.")
        current_bot = token_manager.get_main_bot()
    else:
        current_bot = coro.__self__
        
    current_token = current_bot.token
    ...
    try:
    ...
    except TelegramRetryAfter as e: # NAYA: Flood limit hit
        logger.warning(f"TG: FLOOD WAIT (429) on token ...{current_token[-4:]}. Retry after {e.retry_after}s.")
        # TokenManager ko inform karein ki yeh token flooded hai.
        await token_manager.mark_token_flooded(current_token, e.retry_after)
        
        # Ab, agar possible ho, toh call ko kisi dusre bot par retry karein.
        # Yeh implementation complex ho jayegi (blocking I/O) aur NON-BREAKABLE RULE #5 ko tod sakti hai,
        # isliye hum sirf user ko inform karenge aur call fail kar denge, 
        # Taki next request naye token par jaye.
        return None # Call failed, will not retry.
        ...
        if "bot was blocked" in error_msg or "user is deactivated" in error_msg:
            ...
            # Flood wait jo TelegramRetryAfter se nahi aaya. Safest hai isko bhi mark karna.
            logger.warning(f"TG: FLOOD WAIT (Too Many Requests) on token ...{current_token[-4:]}. {e}")
            await token_manager.mark_token_flooded(current_token, 60) # Default 60s mark karein
            return None
        else:
...
# NAYA: check_user_membership ko 'bot' instance zaroor chahiye
...
def get_full_limit_keyboard() -> InlineKeyboardMarkup | None:
    # NAYA: Alternate bots ab sirf join_channel_username ko hi istemal karte hain agar set hai.
    # Existing ALTERNATE_BOTS logic ko hataana nahi hai.
    if not ALTERNATE_BOTS: return None
    # ALTERNATE_BOTS list mein usernames (@ ke bina) honge
...
# ============ NAYA FUZZY CACHE FUNCTIONS (No change) ============
...
#
# ==================================================
# +++++ NEW: SMART INTENT ENGINE V5 (No change) ++++++++++++++++
# ==================================================
#
...
async def lifespan(app: FastAPI):
    ...
    # NAYA: Webhook main bot par set hoga
    # FIX: token_manager.get_main_bot() is now synchronous
    main_bot_instance = token_manager.get_main_bot()

    if WEBHOOK_URL:
        try:
            # FIX: main_bot_instance is now a Bot object, so get_webhook_info is callable
            current_webhook = await main_bot_instance.get_webhook_info() 
...
# NAYA: _process_update_safe mein 'bot_instance' argument remove karein
async def _process_update_safe(update_obj: Update): 
    try:
        # User ID se bot instance lein (agar message/callback hai)
        user_id = update_obj.message.from_user.id if update_obj.message and update_obj.message.from_user else \
                  update_obj.callback_query.from_user.id if update_obj.callback_query and update_obj.callback_query.from_user else None
                  
        bot_instance = await token_manager.get_bot(user_id=user_id) # NAYA: TokenManager se bot lein
        
        # 3 DBs, Cache aur Manager ko pass karein
        await dp.feed_update(
            ...
            db_fallback=db_fallback, 
            db_neon=db_neon,
            search_cache=search_cache, # NAYA
            token_manager=token_manager # NAYA
        )
    except Exception as e:
        logger.exception(f"Update process karte waqt error {update_obj.update_id}: {e}")

# NAYA: Webhook route ab MAIN_BOT_TOKEN par sunega (security)
@app.post(f"/bot/{MAIN_BOT_TOKEN}") 
async def bot_webhook(update: dict, background_tasks: BackgroundTasks, request: Request):
    ...
    try:
        ...
        background_tasks.add_task(_process_update_safe, telegram_update) # NAYA: bot argument remove kiya
        return {"ok": True}
    except Exception as e:
...
async def health_check():
    ...
    return {
        ...
        "redis_cache_status": "enabled" if search_cache.is_ready else "disabled", # NAYA
        "uptime": get_uptime(),
...
# NAYA: ensure_capacity_or_inform ko 'bot' instance chahiye
...
@handler_timeout(15)
# NAYA: 'bot' argument add kiya
async def start_command(bot: Bot, message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        # NAYA: TokenManager info
        tokens_info = ", ".join([f"...{t[-4:]} ({'Flooded' if t not in token_manager.available_tokens else 'Active'})" for t in token_manager.bots.keys()])
        ...
        admin_message = (
            ...
            f"<b>⚙️ Backend Load & Caching</b>\n" # NAYA
            f"  - 🔑 Tokens ({len(token_manager.bots)}): {tokens_info}\n"
            f"  - 🧠 Redis Cache: {'🟢 Active' if search_cache.is_ready else '🟡 Disabled/Error'}\n"
            f"  - <b>/check_token_status</b> (Check token flooding)\n\n"
    ...
    # --- NAYA: Regular User ---
    if not await ensure_capacity_or_inform(bot, message, db_primary): # NAYA: bot pass kiya
        return
        
    is_member = await check_user_membership(bot, user.id) # NAYA: bot pass kiya
...
@handler_timeout(10)
# NAYA: 'bot' argument add kiya
async def help_command(bot: Bot, message: types.Message, db_primary: Database):
    user = message.from_user
    if not user: return
    await safe_db_call(db_primary.add_user(user.id, user.username, user.first_name, user.last_name))
...
@handler_timeout(20)
# NAYA: 'bot' argument add kiya
async def check_join_callback(bot: Bot, callback: types.CallbackQuery, db_primary: Database):
    ...
    if not await ensure_capacity_or_inform(bot, callback, db_primary): # NAYA: bot pass kiya
        return
...
@handler_timeout(20)
# NAYA: 'bot' argument aur 'search_cache' argument add kiya
async def search_movie_handler(bot: Bot, message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB, search_cache: SearchCache):
    user = message.from_user
    if not user: return
    user_id = user.id
    ...
    # NAYA: Redis Cache Check (Feature 2)
    cache_key = f"search:{clean_query}"
    fuzzy_hits_raw = await search_cache.get_search_results(cache_key)

    if fuzzy_hits_raw is None: # Cache Miss
        ...
        logger.info(f"User {user_id} searching for '{clean_query}' (Cache MISS)")
        ...
        # NAYA: Cache Store (Non-blocking)
        if fuzzy_hits_raw:
            asyncio.create_task(search_cache.set_search_results(cache_key, fuzzy_hits_raw, ttl_seconds=3600))
    else:
        logger.info(f"User {user_id} searching for '{clean_query}' (Cache HIT)")
        
        # Edit 'searching' message only if one was sent (i.e. if it wasn't a cache hit)
        # But we still need an initial message if we want to reply to it.
        pass
        
    unique_movies = {}
    for movie in fuzzy_hits_raw:
        # NOTE: Original logic had a bug (or feature) that only unique imdb_id were collected
    ...
    if not unique_movies:
        # NAYA: Check if searching_msg exists before editing
        if 'searching_msg' in locals() and searching_msg:
             await safe_tg_call(searching_msg.edit_text(f"🥲 Maaf kijiye, '<b>{original_query}</b>' ke liye kuch nahi mila."))
        else:
             await safe_tg_call(message.answer(f"🥲 Maaf kijiye, '<b>{original_query}</b>' ke liye kuch nahi mila."))
        return
    ...
    # NAYA: Reply to original message if search_msg wasn't sent (cache hit)
    if 'searching_msg' in locals() and searching_msg:
         await safe_tg_call(searching_msg.edit_text(
            f"⚡️ <b>{original_query}</b> - {result_count_text} results mile:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
         ))
    else: # Cache Hit ka case
         await safe_tg_call(message.answer(
            f"⚡️ <b>{original_query}</b> - {result_count_text} results mile (Cache Hit):",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
         ))
...
@handler_timeout(20)
# NAYA: 'bot' argument add kiya
async def get_movie_callback(bot: Bot, callback: types.CallbackQuery, db_primary: Database, db_fallback: Database):
    user = callback.from_user
    if not user: 
...
@handler_timeout(20)
# NAYA: 'bot' argument add kiya
async def migration_handler(bot: Bot, message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
...
@handler_timeout(20)
# NAYA: 'bot' argument add kiya
async def auto_index_handler(bot: Bot, message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
...
@handler_timeout(15)
# NAYA: 'bot' argument add kiya
async def stats_command(bot: Bot, message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
...
@handler_timeout(10)
# NAYA: 'bot' argument add kiya
async def search_switch_command(bot: Bot, message: types.Message):
...
# NAYA: 'bot' argument add kiya
async def broadcast_command(bot: Bot, message: types.Message, db_primary: Database):
    ...
    # NAYA: Broadcast ke liye main bot ka istemal karein
    main_bot_instance = token_manager.get_main_bot()
    
    async def send_to_user(user_id: int):
        nonlocal success_count, failed_count
        # NAYA: main_bot_instance ka istemal karein
        res = await safe_tg_call(message.reply_to_message.copy_to(user_id, bot=main_bot_instance), timeout=10, semaphore=TELEGRAM_BROADCAST_SEMAPHORE)
...
@handler_timeout(120)
# NAYA: 'bot' argument add kiya
async def cleanup_users_command(bot: Bot, message: types.Message, db_primary: Database):
...
@handler_timeout(10)
# NAYA: 'bot' argument add kiya
async def get_user_command(bot: Bot, message: types.Message, db_primary: Database):
...
# NAYA: 'bot' argument add kiya
async def import_json_command(bot: Bot, message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if not msg: return
    try:
        # NAYA: download_file ko bot instance se call karein
...
@handler_timeout(15)
# NAYA: 'bot' argument add kiya
async def remove_dead_movie_command(bot: Bot, message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    args = message.text.split(maxsplit=1)
...
@handler_timeout(300)
# NAYA: 'bot' argument add kiya
async def cleanup_mongo_1_command(bot: Bot, message: types.Message, db_primary: Database):
...
@handler_timeout(300)
# NAYA: 'bot' argument add kiya
async def cleanup_mongo_2_command(bot: Bot, message: types.Message, db_fallback: Database):
...
@handler_timeout(3600)
# NAYA: 'bot' argument add kiya
async def remove_library_duplicates_command(bot: Bot, message: types.Message, db_neon: NeonDB):
...
# NAYA: 'bot' argument add kiya
async def backup_channel_command(bot: Bot, message: types.Message, db_neon: NeonDB):
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
...
@handler_timeout(1800)
# NAYA: 'bot' argument add kiya
async def sync_mongo_1_to_neon_command(bot: Bot, message: types.Message, db_primary: Database, db_neon: NeonDB):
...
@handler_timeout(1800)
# NAYA: 'bot' argument add kiya
async def sync_mongo_1_to_2_command(bot: Bot, message: types.Message, db_primary: Database, db_fallback: Database):
    msg = await safe_tg_call(message.answer("⏳ (Mongo 1) se sabhi movies fetch ki ja rahi hain..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
@handler_timeout(300)
# NAYA: 'bot' argument add kiya
async def rebuild_clean_titles_m1_command(bot: Bot, message: types.Message, db_primary: Database):
...
@handler_timeout(900) 
# NAYA: 'bot' argument add kiya
async def force_rebuild_m1_command(bot: Bot, message: types.Message, db_primary: Database):
...
@handler_timeout(300)
# NAYA: 'bot' argument add kiya
async def rebuild_clean_titles_m2_command(bot: Bot, message: types.Message, db_fallback: Database):
...
@handler_timeout(10)
# NAYA: 'bot' argument add kiya
async def set_limit_command(bot: Bot, message: types.Message):
    ...
    except (ValueError, AssertionError): await safe_tg_call(message.answer("❌ Limit 5 aur 5000 ke beech એક number hona chahiye."), semaphore=TELEGRAM_COPY_SEMAPHORE)


@dp.message(Command("rebuild_neon_vectors"), AdminFilter())
@handler_timeout(600)
# NAYA: 'bot' argument add kiya
async def rebuild_neon_vectors_command(bot: Bot, message: types.Message, db_neon: NeonDB):
...
@handler_timeout(300)
# NAYA: 'bot' argument add kiya
async def reload_fuzzy_cache_command(bot: Bot, message: types.Message, db_primary: Database):
    msg = await safe_tg_call(message.answer("🧠 In-Memory Fuzzy Cache ko Mongo se reload kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    await load_fuzzy_cache(db_primary)
...
@handler_timeout(15)
# NAYA: 'bot' argument add kiya
async def check_db_command(bot: Bot, message: types.Message, db_primary: Database, db_neon: NeonDB):
    ...
    # NAYA: Token Status Command
@dp.message(Command("check_token_status"), AdminFilter())
@handler_timeout(10)
# NAYA: 'bot' argument add kiya
async def check_token_status_command(bot: Bot, message: types.Message):
    # NAYA: TokenManager se status lein
    tokens_status = []
    now = datetime.now(timezone.utc)
    
    async with token_manager.bot_lock:
        for token, instance in token_manager.bots.items():
            token_end = token[-4:]
            status = "🟢 Active"
            recovery_time = token_manager.flooded_tokens.get(token)
            
            if recovery_time:
                remaining_time = int((recovery_time - now).total_seconds())
                if remaining_time > 0:
                    status = f"🔴 Flooded (T-{remaining_time}s)"
                else:
                    # Recovery ke baad bhi list mein reh sakta hai agar cleanup na chala ho
                    status = "🟡 Recovered (Cleanup pending)" 
            
            tokens_status.append(f"  - Token ...{token_end}: {status}")

    # --- FIX: String joining ko f-string expression se bahar le jaaya gaya ---
    # Python f-string expression part cannot include a backslash.
    status_joined = '\n'.join(tokens_status)
    
    reply_text = (
        f"""<b>🔑 Bot Token Health Check</b>

{status_joined}

ℹ️ Flooded token se user ko automatic dusre token par redirect kiya jaata hai."""
    )
    # -------------------------
    await safe_tg_call(message.answer(reply_text))
...
async def errors_handler(update: types.Update, exception: Exception):
    ...
    # NAYA: Bot instance ko TokenManager se nikaalna
    ...
    bot_instance = await token_manager.get_bot(user_id=user_id) # NAYA

    error_message = "❗️ Ek anjaani error aa gayi hai. Team ko soochit kar diya gaya hai. Kripya thodi der baad try karein."
    if target_chat_id:
        try: await bot_instance.send_message(target_chat_id, error_message) # NAYA: bot_instance
...
async def main_polling():
    ...
    try:
        ...
        await db_fallback.init_db()
        await db_neon.init_db()
        await search_cache.init_cache() # NAYA
    ...
    main_bot_instance = token_manager.get_main_bot() # NAYA
    await main_bot_instance.delete_webhook(drop_pending_updates=True) # NAYA
    global monitor_task
    monitor_task = asyncio.create_task(monitor_event_loop())
    setup_signal_handlers()
    ...
    try:
        await dp.start_polling(
            main_bot_instance, # NAYA: Main bot instance
            allowed_updates=dp.resolve_used_update_types(),
            db_primary=db_primary,
            db_fallback=db_fallback,
            db_neon=db_neon,
