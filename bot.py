    ...
    async def __call__(self, message: types.Message, db_primary: Database) -> bool:
        ...
        if is_banned:
            ...
            try:
                ...
                ban_text = "🚫 **ACCESS DENIED**\n━━━━━━━━━━━━━━━━\nYour account has been restricted by the administration. Contact support if you believe this is an error."
                await safe_tg_call(
...
def get_join_keyboard() -> InlineKeyboardMarkup | None:
    ...
    # Button Labels Redesigned for Premium Look
    ...
    if JOIN_CHANNEL_USERNAME or USER_GROUP_USERNAME: 
        # UI Enhancement: Primary action button with glowing emoji
        buttons.append([InlineKeyboardButton(text="⚡ Verify Access", callback_data="check_join")])
...
def overflow_message(active_users: int) -> str:
    return (
        f"⚠️ **HIGH TRAFFIC WARNING**\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Server is currently handling maximum capacity ({active_users}/{CURRENT_CONC_LIMIT}).\n\n"
        f"**Status:** 🔴 Busy\n"
        f"**Action:** Please wait 30 seconds before trying again.\n\n"
        f"⚡ _Premium servers are auto-scaling..._"
    )
...
async def ensure_capacity_or_inform(
    ...
    if active >= CURRENT_CONC_LIMIT:
        ...
        if isinstance(message_or_callback, types.CallbackQuery):
            # UI Enhancement: Friendly callback answer
...
async def start_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    if user_id == ADMIN_USER_ID:
        admin_panel_text = (
            ...
            f"• `/check_db` - Integrity Scan\n"
            f"• `/health` - Server Pulse\n\n"
            f"👥 **USER MANAGEMENT**\n"
            f"• `/get_user [ID]` - View Profile\n"
            f"• `/ban [ID]` - Suspend Access\n"
            f"• `/unban [ID]` - Restore Access\n"
            f"• `/broadcast` - Global Announcement\n\n"
            f"⚙️ **SYSTEM MAINTENANCE**\n"
            f"• `/reload_fuzzy_cache` - Refresh Index\n"
            f"• `/cleanup_users` - Purge Inactive\n"
            f"• `/rebuild_clean_titles_m1` - Repair DB\n"
        )
        await safe_tg_call(message.answer(admin_panel_text), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    if is_member:
        ...
        welcome_text = (
            f"🎬 **PREMIUM CINEMA HUB**\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"Welcome, <b>{user.first_name}</b>. Your personal library is ready.\n\n"
            f"🔍 **HOW TO SEARCH**\n"
            f"Simply type the <b>Movie</b> or <b>Series</b> name.\n"
            f"• Example: <code>Inception</code>\n"
            f"• Example: <code>Breaking Bad</code>\n\n"
            f"⚡ **FEATURES**\n"
            f"• Instant Delivery\n"
            f"• Smart Typo Correction\n"
            f"• 4K & HD Quality Available\n"
        )
        
        # UI Enhancement: App-like main menu buttons
        main_menu = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="💡 Search Tips", callback_data="help_cmd"),
            ],
            [
                ...
                InlineKeyboardButton(text="🆘 Support", callback_data="help_cmd"),
            ]
        ])
        ...
        welcome_text = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"To access the premium library, please verify your membership below.\n\n"
            f"1️⃣ Join the **Required Channels**.\n"
            f"2️⃣ Click **Verify Access** to unlock."
        )
        if join_markup:
...
async def help_command(message: types.Message, bot: Bot, db_primary: Database, redis_cache: RedisCacheLayer):
    ...
    help_text = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "🔍 **Basic Search**\n"
        "Just send the title. No commands needed.\n"
        "• Input: <code>Jawan</code>\n\n"
        "✨ **Smart Correction**\n"
        "Made a typo? We'll fix it.\n"
        "• Input: <code>Avngers</code> → Result: **Avengers**\n\n"
        "📅 **Precision Search**\n"
        "Add the year to filter results.\n"
        "• Input: <code>Pathaan 2023</code>\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "⚡ _Powered by High-Speed Cloud Indexing_"
    )
    
    # UI Enhancement: Add a return button for continuity
    back_button = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ Return Home", callback_data="start_cmd")]
    ])
...
async def check_join_callback(callback: types.CallbackQuery, bot: Bot, db_primary: Database, redis_cache: RedisCacheLayer):
    ...
    is_banned = await safe_db_call(db_primary.is_user_banned(user.id), default=False)
    if is_banned:
        await safe_tg_call(callback.answer("❌ Restricted Access.", show_alert=True))
        return
    ...
    if is_member:
        ...
        success_text = (
            f"✅ **ACCESS GRANTED**\n"
            f"━━━━━━━━━━━━━━━━\n"
            f"Verification successful. Welcome to the elite club, <b>{user.first_name}</b>.\n\n"
            f"👉 **Start Searching:** Type a movie name now."
        )
        ...
        await safe_tg_call(callback.answer("⚠️ Verification Failed: Please join the required channels first.", show_alert=True))
        join_markup = get_join_keyboard()
...
async def no_url_join_callback(callback: types.CallbackQuery):
    ...
    await safe_tg_call(callback.answer("ℹ️ Private Invite: Link will be provided by Admin. Once joined, click Verify.", show_alert=True))
...
async def search_movie_handler(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    if not clean_query:
        await safe_tg_call(message.answer("⚠️ **Invalid Query**: Please use alphanumeric characters."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
        
    if not fuzzy_movie_cache:
        logger.error(f"FATAL: User {user_id} ne search kiya, lekin fuzzy cache khaali hai!")
        if user.id == ADMIN_USER_ID:
            await safe_tg_call(message.answer("⚠️ ADMIN WARNING: Fuzzy cache is empty. Run /reload_fuzzy_cache."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        else:
            # UI Enhancement: Soft initialization error
            await safe_tg_call(message.answer("🔄 **System Initializing**: Please wait 30 seconds and try again."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return

    # UI Enhancement: Polished searching message
    searching_msg = await safe_tg_call(message.answer(f"🔎 **Searching Database...**\nQuery: <i>{original_query}</i>"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    ...
    if not unique_movies:
        ...
        await safe_tg_call(searching_msg.edit_text(f"❌ **No Results Found**\n━━━━━━━━━━━━━━━━\nWe couldn't find '<b>{original_query}</b>'.\n\n• Check spelling\n• Try a shorter keyword"))
        return
    ...
    await safe_tg_call(searching_msg.edit_text(
        f"🎬 **RESULTS FOUND**\n━━━━━━━━━━━━━━━━━━━━━━━━\nBest matches for '<b>{original_query}</b>':\n👇 _Tap to Download_",
...
async def get_movie_callback(callback: types.CallbackQuery, bot: Bot, db_primary: Database, db_fallback: Database, redis_cache: RedisCacheLayer):
    ...
    if not movie:
        ...
        await safe_tg_call(callback.message.edit_text("❌ **File Unavailable**: This title has been removed from the database."))
        if user.id == ADMIN_USER_ID:
            await safe_tg_call(callback.message.answer(f"ADMIN NOTE: Movie <code>{imdb_id}</code> missing in DB. Sync required."))
        return
    ...
    if success:
        # UI Enhancement: Success message
        success_text = f"✅ **DELIVERY COMPLETE**\n━━━━━━━━━━━━━━━━\nYour file for '<b>{movie['title']}</b>' has been sent. Enjoy! 🍿"
        try:
        ...
        error_text = (
            f"❌ **DELIVERY FAILED**\n"
            f"━━━━━━━━━━━━━━━━\n"
            f"We could not deliver '<b>{movie['title']}</b>'.\n"
...
async def migration_handler(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    if not message.forward_from_chat or message.forward_from_chat.id != LIBRARY_CHANNEL_ID:
        if LIBRARY_CHANNEL_ID == 0: await safe_tg_call(message.answer("❌ **Configuration Error**: `LIBRARY_CHANNEL_ID` not set."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        else: await safe_tg_call(message.answer(f"❌ **Invalid Source**: Files must be forwarded from Library Channel (ID: `{LIBRARY_CHANNEL_ID}`)."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
    ...
    if not info or not info.get("title"):
        ...
        await safe_tg_call(message.answer(f"❌ **Parse Error**: MessageID `{message.forward_from_message_id}` caption invalid."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
...
async def stats_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    msg = await safe_tg_call(message.answer("⚙️ **Gathering Intelligence...**"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    ...
    admin_message = (
        ...
        f"  ┌ **Primary Node (M1):** {node_status_icon(mongo_1_ready)}\n"
        f"  ├ **Fallback Node (M2):** {node_status_icon(mongo_2_ready)}\n"
        f"  ├ **Index Node (Neon):** {node_status_icon(neon_ready)}\n"
        f"  └ **Cache Layer:** {cache_status_icon(redis_ready)}\n\n"
        
        f"**LIVE METRICS**\n"
        f"  • **Active Load:** `{concurrent_users:,}` / `{CURRENT_CONC_LIMIT}`\n"
        f"  • **Total Users:** `{count_str(user_count)}`\n"
        f"  • **Uptime:** `{get_uptime()}`\n\n"
        
        f"**DATABASE VOLUME**\n"
        f"  • Primary Docs: `{count_str(mongo_1_count)}`\n"
        f"  • Index Vectors: `{count_str(neon_count)}`\n"
        f"  • Cache Entries: `{len(fuzzy_movie_cache):,}`\n\n"
...
async def search_switch_command(message: types.Message):
    ...
    dep_text = "ℹ️ **Notice**: Search switching is deprecated. The system now uses **Hybrid Smart-Sequence** automatically."
...
async def broadcast_command(message: types.Message, db_primary: Database):
    if not message.reply_to_message:
        await safe_tg_call(message.answer("⚠️ **Action Required**: Please reply to the message you want to broadcast."), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    users = await safe_db_call(db_primary.get_all_users(), timeout=60, default=[])
    if not users:
        await safe_tg_call(message.answer("⚠️ **Error**: No users found in database."), semaphore=TELEGRAM_COPY_SEMAPHORE); return
        
    total = len(users); msg = await safe_tg_call(message.answer(f"📡 **Initiating Broadcast**\nTarget: **{total:,}** users..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def cleanup_users_command(message: types.Message, db_primary: Database):
    msg = await safe_tg_call(message.answer("🧹 **Purging Inactive Users**..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    ...
    txt = f"✅ **Cleanup Finished**\n\n**Removed:** {removed:,} inactive accounts.\n**Remaining:** {new_count:,} active users."
...
async def get_user_command(message: types.Message, db_primary: Database):
    ...
    if len(args) < 2 or not args[1].isdigit():
        await safe_tg_call(message.answer("⚠️ **Usage**: `/get_user ID`"), semaphore=TELEGRAM_COPY_SEMAPHORE); return
...
async def export_users_command(message: types.Message, db_primary: Database):
    msg = await safe_tg_call(message.answer("⏳ **Exporting Data**..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    
    user_data_list = await safe_db_call(db_primary.get_all_user_details(), timeout=50, default=[])
    
    if not user_data_list:
        await safe_tg_call(msg.edit_text("❌ No data to export.")); return
    ...
    try:
        ...
        await safe_tg_call(
            message.answer_document(
                ...
                caption=f"✅ **Export Ready**: {len(user_data_list):,} records."
            ),
            semaphore=TELEGRAM_COPY_SEMAPHORE
        )
...
async def ban_user_command(message: types.Message, db_primary: Database):
    ...
    if target_id is None:
        await safe_tg_call(message.answer("⚠️ Usage: `/ban ID`"), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    
    if target_id == ADMIN_USER_ID:
        await safe_tg_call(message.answer("❌ Error: Cannot ban Admin."), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    ...
    if banned:
        await safe_tg_call(message.answer(f"🚫 **User Banned**\nID: <code>{target_id}</code>\nReason: {reason}"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    else:
        await safe_tg_call(message.answer(f"❌ Failed to ban user <code>{target_id}</code>."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def unban_user_command(message: types.Message, db_primary: Database):
    target_id = await _get_target_user_id(message)
    if target_id is None:
        await safe_tg_call(message.answer("⚠️ Usage: `/unban ID`"), semaphore=TELEGRAM_COPY_SEMAPHORE); return

    unbanned = await safe_db_call(db_primary.unban_user(target_id))
    
    if unbanned:
        await safe_tg_call(message.answer(f"✅ **User Unbanned**\nAccess restored for <code>{target_id}</code>."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    else:
        await safe_tg_call(message.answer(f"❌ User <code>{target_id}</code> not found in ban list."), semaphore=TELEGRAM_COPY_SEMAPHORE)
# --- END NAYA FEATURE 2 ---
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    if not message.reply_to_message or not message.reply_to_message.document:
        await safe_tg_call(message.answer("⚠️ **Error**: Reply to a JSON file."), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    doc = message.reply_to_message.document
    if not doc.file_name or (not doc.file_name.lower().endswith(".json") and doc.mime_type != "application/json"):
        await safe_tg_call(message.answer("⚠️ **Error**: Only `.json` allowed."), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    ...
    await safe_tg_call(msg.edit_text(f"⏳ **Importing** {total:,} items..."))
    ...
    for i, item in enumerate(mlist):
        ...
        if processed_count % 100 == 0 or (now - start_import_time).total_seconds() > 10 or processed_count == total:
            ...
            try: 
                ...
                await safe_tg_call(msg.edit_text(f"⏳ **Import Status**\nDone: {processed_count:,}/{total:,}\nSkipped: {s:,} | Failed: {f:,}"))
            except TelegramBadRequest: pass
            last_update_time = now
...
async def remove_dead_movie_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    msg = await safe_tg_call(message.answer(f"⏳ Deleting <code>{imdb_id}</code>..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    ...
    await safe_tg_call(msg.edit_text(f"**Deletion Report**\nID: <code>{imdb_id}</code>\n{db1_stat} | {db2_stat} | {neon_stat}"))
...
async def cleanup_mongo_1_command(message: types.Message, db_primary: Database):
    msg = await safe_tg_call(message.answer("⏳ **Scanning M1 for Duplicates**..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    deleted_count, duplicates_found = await safe_db_call(db_primary.cleanup_mongo_duplicates(batch_limit=100), default=(0,0))
    if deleted_count > 0:
        await safe_tg_call(msg.edit_text(f"✅ **Cleanup Batch Done**\nDeleted: {deleted_count}\nRemaining: {max(0, duplicates_found - deleted_count)}"))
        await load_fuzzy_cache(db_primary)
    else:
        await safe_tg_call(msg.edit_text("✅ **M1 is Clean**"))
...
async def cleanup_mongo_2_command(message: types.Message, db_fallback: Database):
    msg = await safe_tg_call(message.answer("⏳ **Scanning M2 for Duplicates**..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    deleted_count, duplicates_found = await safe_db_call(db_fallback.cleanup_mongo_duplicates(batch_limit=100), default=(0,0))
    if deleted_count > 0:
        await safe_tg_call(msg.edit_text(f"✅ **Cleanup Batch Done**\nDeleted: {deleted_count}\nRemaining: {max(0, duplicates_found - deleted_count)}"))
    else:
        await safe_tg_call(msg.edit_text("✅ **M2 is Clean**"))
...
async def remove_library_duplicates_command(message: types.Message, db_neon: NeonDB):
    msg = await safe_tg_call(message.answer("⏳ **Scanning Library Index**..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    
    messages_to_delete, total_duplicates = await safe_db_call(db_neon.find_and_delete_duplicates(batch_limit=100), default=([], 0))
    
    if not messages_to_delete:
        await safe_tg_call(msg.edit_text("✅ **Index Clean**: No duplicates found."))
        return
        
    await safe_tg_call(msg.edit_text(f"⚠️ Found {total_duplicates} duplicates. Deleting {len(messages_to_delete)}..."))
    ...
    await safe_tg_call(msg.edit_text(
        f"✅ **Batch Delete Complete**\n"
        f"🗑️ Deleted: {deleted_count}\n"
        f"❌ Failed: {failed_count}\n"
        f"ℹ️ Pending: {max(0, total_duplicates - deleted_count)}"
    ))
...
async def backup_channel_command(message: types.Message, db_neon: NeonDB):
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        await safe_tg_call(message.answer("⚠️ Usage: `/backup_channel TARGET`"), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    ...
    msg = await safe_tg_call(message.answer("⏳ **Preparing Backup**..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    
    unique_files = await safe_db_call(db_neon.get_unique_movies_for_backup(), default=[])
    if not unique_files:
        await safe_tg_call(msg.edit_text("❌ No unique files found.")); return
        
    total_files = len(unique_files)
    await safe_tg_call(msg.edit_text(f"🚀 **Backing up {total_files:,} files** to `{target_channel}`..."))
...
async def sync_mongo_1_to_neon_command(message: types.Message, db_primary: Database, db_neon: NeonDB):
    ...
    if not mongo_movies:
        await safe_tg_call(msg.edit_text("❌ No data in M1.")); return
    
    await safe_tg_call(msg.edit_text(f"🔄 Processing {len(mongo_movies):,} records..."))
...
async def sync_mongo_1_to_2_command(message: types.Message, db_primary: Database, db_fallback: Database):
    ...
    await safe_tg_call(msg.edit_text(f"⏳ Fetching data..."))
    mongo_movies_full = await safe_db_call(db_primary.get_all_movies_for_neon_sync(), timeout=300)
    if not mongo_movies_full:
        await safe_tg_call(msg.edit_text("❌ No data in M1.")); return
        
    await safe_tg_call(msg.edit_text(f"🔄 Syncing {len(mongo_movies_full):,} records..."))
...
async def set_limit_command(message: types.Message):
    ...
    try:
        ...
        await safe_tg_call(message.answer(f"✅ Limit Updated: **{CURRENT_CONC_LIMIT}**"), semaphore=TELEGRAM_COPY_SEMAPHORE)
        logger.info(f"Concurrency limit admin ne {CURRENT_CONC_LIMIT} kar diya hai।")
    except (ValueError, AssertionError): 
        await safe_tg_call(message.answer("❌ Error: Invalid limit."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def rebuild_neon_vectors_command(message: types.Message, db_neon: NeonDB):
    ...
    if updated_count >= 0:
        await safe_tg_call(msg.edit_text(f"✅ **Vectors Updated**\nCount: {updated_count:,}"))
    else:
        await safe_tg_call(msg.edit_text("❌ Rebuild Failed"))
...
async def check_db_command(message: types.Message, db_primary: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    fuzzy_cache_check = {"title": "N/A", "clean_title": "--- EMPTY ---"}
    ...
    reply_text = (
        ...
        f"━━━━━━━━━━━━━━━━\n"
        f"**DB1: Mongo Primary**\n"
        f"Sample: <code>{mongo_res.get('clean_title')}</code>\n\n"
        f"**DB3: Neon Index**\n"
        f"Sample: <code>{neon_res.get('clean_title')}</code>\n\n"
        f"**Cache Layer ({redis_status})**\n"
        f"Size: {len(fuzzy_movie_cache):,}\n"
        f"Sample: <code>{fuzzy_cache_check.get('clean_title')}</code>"
    )
...
async def errors_handler(update: types.Update, exception: Exception):
    ...
    error_message = "⚠️ **System Error**: Something went wrong. The engineering team has been notified."
    if target_chat_id:
        try: 
    ...
    if callback_query:
        try: 
            await callback_query.answer("⚠️ System Error", show_alert=True)
