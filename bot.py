...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Hybrid V2 (RapidFuzz Broad Net -> Smart Sequence Re-Rank)")
...
def get_advanced_sequence_score(query: str, target: str) -> int:
    ...
    """
    # Base logic: Find characters of Query INSIDE Target, in order.
    
    last_target_idx = -1
    matches = 0
    score = 0
    
    # Loop through query characters
    for idx, char in enumerate(query):
        # Find this char in target, STARTING after the last found position
        ...
        if found_at != -1:
            ...
            # 2. START BONUS (Query starts with same letter as Target)
            if idx == 0 and found_at == 0:
                score += 50 # Huge bonus for first letter match
                
            # 3. CONSECUTIVE BONUS
            # If this match is immediately after the previous match in target
            # Ex: 'Ka' matches 'Ka'ntara. 'a' is at 1, 'K' was at 0. Diff is 1.
            if found_at == last_target_idx + 1:
                score += 5
            else:
                # PENALTY FOR GAP IN TARGET
                # Kantara (Target) vs Katra (Query).
                # K(0)->a(1)->t(3). Gap between a and t is 1 (n). Small penalty.
                gap = found_at - last_target_idx - 1
                score -= min(gap, 5) # Cap penalty so long titles don't lose too much
            
            last_target_idx = found_at
            
        else:
            # Char not found in sequence. 
            # 'Kajgtara': 'j' not found. We ignore it and continue (Simulating "Skip Junk")
            pass

    # 4. COVERAGE BONUS
    # How much of the Query did we actually find?
    # Katra (5 chars) -> Kantara (Matches 5). Coverage 100%.
    # Katra (5 chars) -> Kesari (Matches K, a, r = 3). Coverage 60%.
    if len(query) > 0:
        coverage_pct = matches / len(query)
        score += int(coverage_pct * 100) # Max +100 points
        
        # If coverage is too low, kill the score
        if coverage_pct < 0.5: # Less than 50% query matched
            score = 0
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Optimized Hybrid Search V2:
    1. RapidFuzz (Broad Net - Top 200) -> Low CPU Usage.
    2. Smart Sequence (Re-Rank) -> High Accuracy (Fixes 'Katra'->'Kantara').
    """
    if not fuzzy_movie_cache:
        return []

    try:
        # 1. Inputs Clean karein
        q_tight = clean_text_for_search(query) # "katra" (no spaces)
        q_fuzzy = clean_text_for_fuzzy(query)  # "katra" (spaces allowed)
        
        if not q_tight: return []

        # 2. STEP 1: RAPIDFUZZ C++ EXTRACT (The Broad Net)
        # Increase limit to 200 to catch items that are "far" in standard fuzzy logic
        # Decrease cutoff to 30 to catch heavy typos
        all_titles = list(fuzzy_movie_cache.keys())
        
        pre_filtered = process.extract(
            q_fuzzy, 
            all_titles, 
            limit=200, # Increased from 50 to 200 (Safety Net)
            scorer=fuzz.WRatio, 
            score_cutoff=30 # Low cutoff to allow "Aveger" to pass through
        )
        
        candidates = []
        
        # 3. STEP 2: RE-RANKING (Apply Smart Logic on Top 200)
        # Processing 200 items is instant (0.001s), so no CPU issue.
        for clean_title_key, fuzz_score, _ in pre_filtered:
            data = fuzzy_movie_cache.get(clean_title_key)
            if not data: continue
            
            # Prepare for smart scoring
            t_tight = clean_title_key 
            
            # --- A. SMART SEQUENCE SCORE ---
            ...
            # If Sequence match is strong, BOOST it massively
            if seq_score > 60:
                final_score = 300 + seq_score # Priority Boost (Overrides Fuzzy Score)
                match_type = "sequence"
                
                # Length Penalty (Kantara vs Kantara Chap 1)
                len_diff = abs(len(t_tight) - len(q_tight))
                final_score -= (len_diff * 2) 
            else:
