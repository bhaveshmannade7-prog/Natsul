...
# --- FIX: SAFE CLEANING LOGIC (Rollback to original, safer version) ---
def clean_text_for_search(text: str) -> str:
    """Cleans text for search indexing (Rolled back to safer version)."""
    if not text: return ""
    text = text.lower()
    # Separators ko space mein badle
    text = re.sub(r"[._\-]+", " ", text)
    # Sirf a-z aur 0-9 rakhein
    text = re.sub(r"[^a-z0-9\s]+", "", text)
    # Season info remove karein
    text = re.sub(r"\b(s|season)\s*\d{1,2}\b", "", text)
    # Year info ko mat hatayein (taki title ki uniqueness bani rahe)
...
def python_fuzzy_search(query: str, limit: int = 12) -> List[Dict]:
    ...
    try:
        ...
        results = process.extract(
            query,
            choices, 
            scorer=fuzz.WRatio, # FIX: WRatio (Best for typos + partial matches)
            limit=limit,
            score_cutoff=50 # <-- FINAL FIX: Cutoff 50 किया गया for maximum sensitivity
        )
