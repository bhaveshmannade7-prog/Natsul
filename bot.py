...
try:
    ...
    ALTERNATE_BOT_TOKENS_RAW = os.getenv("ALTERNATE_BOT_TOKENS", "") # BADLA: Name change
...
# TELEGRAM semaphores ab sirf TG calls ko throttle karne ke liye hain. 
# Flood control ab TokenManager se hoga.
...
@priority_task_wrapper("LOW") # NAYA: Priority Queue
@handler_timeout(15)
...
@priority_task_wrapper("MEDIUM") # NAYA: Priority Queue
@handler_timeout(20)
...
@priority_task_wrapper("HIGH") # NAYA: Priority Queue
@handler_timeout(20)
...
async def get_user_command(message: types.Message, db_primary: Database):
    ...
    if not user_data:
        await safe_tg_call(bot_instance.send_message(message.chat.id, f"❌ User <code>{user_id_to_find}</code> database (Mongo 1) mein nahi mila."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
...
@priority_task_wrapper("LOW") # NAYA: Priority Queue (Long running)
...
async def backup_channel_command(message: types.Message, db_neon: NeonDB):
    ...
    msg = await safe_tg_call(bot_instance.send_message(message.chat.id, f"⏳ NeonDB se unique files ki list fetch ki ja rahi hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
...
async def errors_handler(update: types.Update, exception: Exception):
    ...
    if target_chat_id:
        try: 
            # NAYA: token_manager.get_current_bot() ka istemal karein
            # FIX: msg = await hata diya gaya, direct call kiya gaya
