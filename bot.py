...
async def load_fuzzy_cache(db: Database):
    """Mongo/Redis se movie titles fetch k k k k k करके in-memory fuzzy cache banata hai।"""
...
def get_smart_match_score(query_tokens: List[str], target_clean: str) -> int:
    ...
    for char in query_str_tight:
        found_idx = target_tight.find(char, last_idx + 1)
        if found_idx == -1:
            broken = True
            break
        last_idx = found_idx
...
async def backup_freeze_fix(message: types.Message, db_neon: NeonDB):
    await run_in_background(backup_channel_command, message, db_neon)
...
async def get_user_command(message: types.Message, db_primary: Database):
    ...
    await safe_tg_call(message.answer(user_text), semaphore=TELEGRAM_COPY_SEMAP_ORE)
