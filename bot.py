...
from functools import wraps, lru_cache # --- [UPDATED] lru_cache import kiya ---
...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Optimized Hybrid (LRU Cache + Smart Sequence + Lazy Fuzzy)")
...
def clean_text_for_search(text: str) -> str:
    ...
    Removes spaces to match 'Kantara' vs 'Katra' (skipping spaces logic).
    """
    if not text: return ""
    text = text.lower()
    text = re.sub(r"\b(s|season)\s*\d{1,2}\b", "", text)
    text = re.sub(r"[^a-z0-9]+", "", text) # Remove all symbols AND spaces
    return text
...
def parse_filename(filename: str) -> Dict[str, str | None]:
    """Filename se tags hatakar saaf Title aur Year nikalta hai."""
...
#
# ==================================================
# +++++ [UPDATED] OPTIMIZED SEARCH LOGIC +++++
# ==================================================
#

# --- 1. OPTIMIZED CACHE LOADING (with Pre-Processing) ---
async def load_fuzzy_cache(db: Database):
    """
    Pre-calculate everything so search doesn't have to do work.
    Structure: 
    {
        'clean_title_key': {
            'data': movie_dict,
            't_tight': 'kantara',  # Pre-cleaned for sequence
            't_len': 7             # Pre-calculated length
        }
    }
    """
    global fuzzy_movie_cache
    async with FUZZY_CACHE_LOCK:
        logger.info("In-Memory Fuzzy Cache load ho raha hai (Optimized)...")
        try:
                ...
                for movie_dict in movies_list:
                    ...
                    if orig_clean:
                         if orig_clean not in temp_cache:
                             # YAHAN HUM PEHLE HI SAB PREPARE KAR LENGE
                             temp_cache[orig_clean] = {
                                 'data': movie_dict,
                                 't_tight': orig_clean.replace(" ", ""), # Sequence ke liye taiyar
                                 't_len': len(orig_clean.replace(" ", "")) # Length pehle se saved
                             }
                fuzzy_movie_cache = temp_cache
                
                # Cache clear karein taki naya data aaye
                cached_search_logic.cache_clear()
                
                logger.info(f"✅ Optimized Cache Loaded: {len(fuzzy_movie_cache):,} titles.")
            else:
...
# --- 2. SEQUENCE LOGIC (UNCHANGED) ---
def get_advanced_sequence_score(query: str, target: str) -> int:
    """
    Calculates a score based on sequential character matching.
    Allows skipping 'junk' in query (Kajgtara -> Kantara matches K, a, t, a, r, a).
    Penalizes gaps and mismatches.
    """
    last_target_idx = -1
    matches = 0
    score = 0
    for idx, char in enumerate(query):
        found_at = target.find(char, last_target_idx + 1)
        if found_at != -1:
            matches += 1
            score += 10 
            if idx == 0 and found_at == 0: score += 50 
            if found_at == last_target_idx + 1: score += 5
            else:
                gap = found_at - last_target_idx - 1
                score -= min(gap, 5) 
            last_target_idx = found_at
        else: pass

    if len(query) > 0:
        coverage_pct = matches / len(query)
        score += int(coverage_pct * 100) 
        if coverage_pct < 0.5: score = 0
    return score

# --- 3. OPTIMIZED SEARCH FUNCTION (LRU CACHE + LAZY FUZZY) ---
@lru_cache(maxsize=2000) # <-- MAGIC: Pichi 2000 searches ka result yaad rakhega
def cached_search_logic(query: str):
    """
    Ye function result ko cache karega. 
    Agar same search dobara aayi to ye run nahi hoga, seedha purana result dega.
    """
    if not fuzzy_movie_cache: return []
    ...
    # Optimization: Sirf wahi movies check karo jo length me aas paas ho
    # Range: Query Length - 2 se lekar Query Length * 3 tak
    min_len = max(1, q_len - 2)
    max_len = q_len * 3 

    for title_key, item in fuzzy_movie_cache.items():
        t_tight = item['t_tight']
        t_len = item['t_len']

        # --- STEP 1: LENGTH FILTER (CPU SAVER) ---
        if t_len < min_len or t_len > max_len:
            continue # Skip this movie immediately

        # --- STEP 2: SEQUENCE MATCH ---
        seq_score = get_advanced_sequence_score(q_tight, t_tight)
        
        final_score = 0
        
        if seq_score > 50:
            # Agar Sequence match mil gaya, to Fuzzy mat chalao (CPU Bachao)
            final_score = 200 + seq_score
            final_score -= abs(t_len - q_len) # Length penalty
            
            candidates.append({
                'imdb_id': item['data']['imdb_id'],
                'title': item['data']['title'],
                'year': item['data'].get('year'),
                'score': final_score,
                'match_type': 'sequence'
            })
            continue # Loop ke agle item pe jao, niche wala code skip

        # --- STEP 3: FUZZY FALLBACK (Lazy) ---
        # Only if sequence failed
        fuz_score = fuzz.WRatio(query, title_key) 
        if fuz_score > 65:
            candidates.append({
                'imdb_id': item['data']['imdb_id'],
                'title': item['data']['title'],
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Wrapper function jo cached logic ko call karega
    """
    return cached_search_logic(query)
...
async def health_check():
    ...
    return {
        ...
        "search_logic": "Optimized Hybrid (LRU+Sequence+Fuzzy)",
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        search_status = f"⚡️ Optimized Hybrid (LRU+Sequence)"
        ...
        admin_message = (
            ...
            f"<b>⚙️ Search Configuration (OPTIMIZED)</b>\n"
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- NAYA HYBRID SEARCH CALL (Cached) ---
    
    logger.info(f"User {user_id} searching for '{clean_query}'")
    
    loop = asyncio.get_running_loop()
    # Ab cached wrapper call hoga
...
# =======================================================
# +++++ BOT HANDLERS: ADMIN MIGRATION (UPDATED FOR CACHE STRUCTURE) +++++
# =======================================================
...
async def migration_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- [UPDATED] Cache Logic for New Structure ---
    if db1_res is True:
        async with FUZZY_CACHE_LOCK:
            if clean_title_val not in fuzzy_movie_cache:
                fuzzy_movie_cache[clean_title_val] = {
                    ...
                    "t_tight": clean_title_val.replace(" ", ""),
                    "t_len": len(clean_title_val.replace(" ", ""))
                }
                cached_search_logic.cache_clear() # Cache invalidate karo
        ...
        if res is True: 
            async with FUZZY_CACHE_LOCK:
                if clean_title_val not in fuzzy_movie_cache:
                    ...
                    cached_search_logic.cache_clear()
            logger.info(f"{log_prefix} Fuzzy cache (Optimized) mein add ho gayi.")
...
async def search_switch_command(message: types.Message):
    await safe_tg_call(message.answer("ℹ️ Search switch ki ab zaroorat nahi hai.\nBot ab hamesha <b>Optimized Hybrid</b> ka istemal karta hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def check_db_command(message: types.Message, db_primary: Database, db_neon: NeonDB):
    ...
    # --- [UPDATED] Cache Check for New Structure ---
    ...
    if fuzzy_movie_cache:
        try:
            ...
            sample = fuzzy_movie_cache[first_key]['data'] # New structure access
    ...
    reply_text = (
        ...
        f"<b>Cache: Python (Optimized Fuzzy)</b>\n"
