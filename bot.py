...
async def check_user_membership(user_id: int, current_bot: Bot) -> bool:
    ...
    def normalize_chat_id(identifier):
        # Agar identifier numeric hai ya negative numeric string hai
        if identifier and (identifier.startswith('-') and identifier[1:].isdigit() or identifier.isdigit()):
            # Telegram API calls ke liye numeric ID ko seedha int mein pass kar sakte hain
            return int(identifier)
        # Agar username hai, toh @ sign lagakar string return karein
...
def get_join_keyboard() -> InlineKeyboardMarkup | None:
    ...
    if JOIN_CHANNEL_USERNAME:
        if is_numeric_id(JOIN_CHANNEL_USERNAME):
             # Numeric ID hai (private channel/group), URL set nahi hoga. User ko khud join karna hoga.
             # FIX: Is baar 'Username not found' error is button ko click karne se nahi aana chahiye.
             buttons.append([InlineKeyboardButton(text="📢 Channel Join Karein (Private)", callback_data="no_url_join")])
        else:
             # Username hai, URL set hoga
             buttons.append([InlineKeyboardButton(text="📢 Channel Join Karein", url=f"https.t.me/{JOIN_CHANNEL_USERNAME.lstrip('@')}")])

    if USER_GROUP_USERNAME:
        if is_numeric_id(USER_GROUP_USERNAME):
             ...
             # Username hai, URL set hoga
             buttons.append([InlineKeyboardButton(text="👥 Group Join Karein", url=f"https.t.me/{USER_GROUP_USERNAME.lstrip('@')}")])
...
async def broadcast_command(message: types.Message, db_primary: Database):
    ...
    for i, user_id in enumerate(users):
        ...
        if (i + 1) % 100 == 0 or (now - last_update_time).total_seconds() > 15 or processed_count == total:
            # Rule: DO NOT add ANY blocking I/O or long waits.
