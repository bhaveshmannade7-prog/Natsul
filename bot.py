...
# --- NAYA SEARCH LOGIC ---
# Ab M+M+N mein se sirf M1 (Exact) aur N (Fuzzy) search ke liye istemal honge
# M2 (Fallback) sirf file retrieval ke liye istemal hoga
logger.info("Search Logic: Hybrid Parallel (Mongo1-Exact + Neon-Fuzzy)")
...
# --- YEH FUNCTION AB DONO MONGO AUR NEON KE LIYE CENTRAL HAI ---
...
async def health_check():
    ...
    return {
        ...
        "search_logic": "Hybrid Parallel (Mongo1-Exact + Neon-Fuzzy)",
        "uptime": get_uptime(),
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        search_status = "⚡️ Hybrid (Mongo1-Exact + Neon-Fuzzy)"
        if not mongo_1_ready: search_status += " (⚠️ Mongo1 Down)"
        if not neon_ready: search_status += " (⚠️ Neon Down)"
        ...
        admin_message = (
            ...
            f"  - {status_icon(mongo_1_ready)} Node 1: Mongo-Primary (Users + Exact Search)\n"
            f"  - {status_icon(mongo_2_ready)} Node 2: Mongo-Fallback (Data Replica)\n"
            f"  - {status_icon(neon_ready)} Node 3: Neon-Fuzzy (Typo Search)\n\n"
            f"<b>⚙️ Search Configuration</b>\n"
            f"  - <b>Default Engine: {search_status}</b>\n"
            f"  - /search_switch (Ab zaroorat nahi)\n\n"
            ...
            f"  - <b>/rebuild_neon_vectors</b> ⚠️ (Fix Neon FTS/Clean data)\n"
            ...
            f"<b>⭐️ Mongo Index:</b> Dono Mongo DBs par 'clean_title' par text index banaya jaayega."
        )
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- NAYA HYBRID PARALLEL SEARCH ---
    # Hum M1 (Exact) aur Neon (Fuzzy) ko ek saath (parallel) search karenge
    
    logger.info(f"User {user_id} searching for '{clean_query}'")
    
    # 1. Dono search tasks ko parallel mein start karein
    exact_hits_task = safe_db_call(db_primary.mongo_primary_search(clean_query, limit=10), default=[])
    fuzzy_hits_task = safe_db_call(db_neon.neondb_search(clean_query, limit=10), default=[])
    
    # 2. Dono ke results ka intezaar karein
    exact_hits, fuzzy_hits = await asyncio.gather(exact_hits_task, fuzzy_hits_task)
    
    # Failover check (agar list ke bajaye None aaya)
    if exact_hits is None: exact_hits = []
    if fuzzy_hits is None: fuzzy_hits = []

    # 3. Results ko merge karein (de-duplicate)
    unique_movies = {}
    engines_used = []
    
    # Step 3a: Pehle Exact hits (M1) daalein, kyonki yeh zyaada relevant hain
    if exact_hits:
        engines_used.append("Exact")
        for movie in exact_hits:
            if movie.get('imdb_id') and movie['imdb_id'] not in unique_movies:
                unique_movies[movie['imdb_id']] = movie
                
    # Step 3b: Ab Fuzzy hits (Neon) daalein (jo pehle se nahi hain)
    if fuzzy_hits:
        engines_used.append("Fuzzy")
        for movie in fuzzy_hits:
            if movie.get('imdb_id') and movie['imdb_id'] not in unique_movies:
                unique_movies[movie['imdb_id']] = movie
    ...
    # unique_movies dictionary se values nikal kar list banayein
    ...
    search_source_text = " + ".join(engines_used)
    if not search_source_text: search_source_text = "N/A"
    
    await safe_tg_call(searching_msg.edit_text(
        f"⚡️ <b>{original_query}</b> - {result_count_text} results mile:\n(Source: {search_source_text})",
...
async def get_movie_callback(callback: types.CallbackQuery, db_primary: Database, db_fallback: Database):
    ...
    # --- NAYA SMOOTH CAPTION LOGIC (Step 1) ---
    # User ko 'loading' popup dikhayein, message edit na karein
    ...
    # DB1 (Primary) se movie nikalne ki koshish karein
    movie = await safe_db_call(db_primary.get_movie_by_imdb(imdb_id), timeout=DB_OP_TIMEOUT)

    # Agar DB1 fail hota hai ya movie nahi milti, tab DB2 (Fallback) check karein
    if not movie:
    ...
    if not movie:
        # --- NAYA SMOOTH CAPTION LOGIC (Step 2 - Fail) ---
        # Ab button waale message ko edit karke error batayein
    ...
    # --- YEH LINE HATA DI GAYI HAI (Aapka Fix) ---
    # await safe_tg_call(callback.message.edit_text(f"✅ <b>{movie['title']}</b> bheji ja rahi hai..."))
    # Isse ab file ke upar extra caption nahi aayega.
        ...
        if is_valid_for_copy:
            copy_result = await safe_tg_call(
                bot.copy_message(
                    ...
                    caption=None # Original caption (file ke saath) barkaraar rakhein
                ), 
    ...
    # --- NAYA SMOOTH CAPTION LOGIC (Step 3 - Final) ---
    # Ab button waale message ko final status ke saath edit karein
    if success:
        try:
            await safe_tg_call(callback.message.edit_text(f"✅ <b>{movie['title']}</b> bhej di gayi hai!"))
        except Exception:
            pass # Agar user ne message delete kar diya ho toh fail ho sakta hai
    else:
        ...
        try:
            ...
            # Agar message edit nahi kar paaye, toh naya message bhej do
...
async def migration_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- FIX: Neon ko ab 'clean_title' ki zaroorat nahi, woh khud banayega ---
...
async def stats_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    stats_msg = (
        ...
        f"  - {status_icon(mongo_2_ready)} Node 2: Mongo-Fallback (Replica)\n"
        f"  - {status_icon(neon_ready)} Node 3: Neon-Fuzzy (Typo Search)\n\n"
        f"<b>Search Logic: {search_status}</b>"
    )
...
async def search_switch_command(message: types.Message):
    await safe_tg_call(message.answer("ℹ️ Search switch ki ab zaroorat nahi hai.\nBot ab hamesha <b>Hybrid Parallel Search</b> (Mongo1-Exact + Neon-Fuzzy) ka istemal karta hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    for i, item in enumerate(mlist):
        try:
            ...
            # --- FIX: Neon ko ab 'clean_title' ki zaroorat nahi ---
...
async def sync_mongo_1_to_2_command(message: types.Message, db_primary: Database, db_fallback: Database):
    ...
    # Hum 'get_all_movies_for_neon_sync' ka istemal karenge (lekin 'clean_title' ke saath)
    # Iske liye humein 'get_all_movies_for_sync' (jo 'clean_title' laata hai) ka istemal karna chahiye
    
    mongo_movies = await safe_db_call(db_primary.get_all_movies_for_sync(), timeout=300)
    ...
    for movie in mongo_movies:
        ...
        # Data 'get_all_movies_for_sync' se aa raha hai, jismein file_id, msg_id nahi hai
        # Hamein M1 se M2 sync ke liye doosra function istemal karna hoga
        # Hum 'add_movie' ka istemal karenge, lekin data 'get_all_movies_for_neon_sync' se lenge
        # Taaki file_id, msg_id, channel_id sab aaye
        
        # --- Logic ko Re-Write karein ---
        pass
    ...
    for movie in mongo_movies_full:
        ...
        tasks.append(safe_db_call(db_fallback.add_movie(
            ...
            year=None, # Yeh field 'get_all_movies_for_neon_sync' se nahi aati
...
async def rebuild_clean_titles_m1_command(message: types.Message, db_primary: Database):
    ...
    await safe_db_call(db_primary.create_mongo_text_index()) # Index ko dobara banayein
...
async def rebuild_clean_titles_m2_command(message: types.Message, db_fallback: Database):
    ...
    await safe_db_call(db_fallback.create_mongo_text_index()) # Index ko dobara banayein
...
async def rebuild_neon_vectors_command(message: types.Message, db_neon: NeonDB):
    msg = await safe_tg_call(message.answer("⏳ (NeonDB) mein `clean_title` aur FTS Data ko rebuild kiya ja raha hai... (Jo NULL hain)"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
