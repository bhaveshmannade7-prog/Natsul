...
# --- FIX: SAFE CLEANING LOGIC (Synchronized) ---
def clean_text_for_search(text: str) -> str:
    """Cleans text for search indexing (Synchronized with database.py)।"""
    if not text: return ""
    text = text.lower()
...
#
# ==================================================
# +++++ FUZZY SEARCH (Scorer = PartialRatio - Final Fix) +++++
# ==================================================
#
def python_fuzzy_search(query: str, limit: int = 12) -> List[Dict]:
    """
    `rapidfuzz` ka istemal karke in-memory cache mein search karta hai।
    FIX: Switched to PartialRatio with lowest cutoff (50) for maximum typo tolerance in short words.
    """
    ...
    try:
        ...
        results = process.extract(
            query,
            choices, 
            scorer=fuzz.partial_ratio, # FIX: PartialRatio (छोटे शब्दों के लिए बेहतर)
            limit=limit,
            score_cutoff=50 # FIX: Cutoff 50 किया गया for maximum sensitivity
        )
        ...
        # FIX: Results ko score ke hisaab se sort karein
    ...
    if WEBHOOK_URL:
        try:
            ...
            if not current_webhook or current_webhook.url != WEBHOOK_URL:
                 ...
                 logger.info("Webhook पहले से सही set है।")
        except Exception as e:
            logger.error(f"Webhook setup mein error: {e}", exc_info=True)
    else:
        logger.warning("WEBHOOK_URL set नहीं है।")

    setup_signal_handlers()
    logger.info("Application startup पूरा हुआ। Bot तैयार है।")
    yield
    logger.info("Application shutdown sequence शुरू हो रहा है...")
    await shutdown_procedure()
    logger.info("Application shutdown पूरा हुआ।")
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        admin_message = (
            ...
            f"  - <b>/rebuild_mongo_index</b> ⚠️ (Exact Search Index FIX) <-- NAYA COMMAND\n"
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    exact_hits_task = safe_db_call(db_primary.mongo_primary_search(clean_query, limit=12), default=[])
    
    loop = asyncio.get_running_loop()
    fuzzy_hits_task = loop.run_in_executor(executor, python_fuzzy_search, clean_query, 12)
    ...
    for movie in final_results[:max_buttons]:
        ...
        score_str = f" ({int(movie.get('score', 0))}%)" if 'score' in movie else ""
...
# --- FINAL FIX: MONGODB INDEX REBUILD COMMAND ---
@dp.message(Command("rebuild_mongo_index"), AdminFilter())
@handler_timeout(300)
async def rebuild_mongo_index_command(message: types.Message, db_primary: Database):
    msg = await safe_tg_call(message.answer("⚠️ MongoDB Text Index ko DROP aur REBUILD kiya ja raha hai (Exact Search fix ke liye)..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    
    success = await safe_db_call(db_primary.force_rebuild_text_index(), default=False)
    
    if success:
        await safe_tg_call(msg.edit_text("✅ MongoDB Text Index safaltapoorvak DROP aur REBUILD ho gaya hai। Ab Exact Search theek se kaam karna chahiye।"))
    else:
        await safe_tg_call(msg.edit_text("❌ MongoDB Text Index rebuild fail ho gaya। Log check karein।"))
