...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Smart Sequence Scanner (Katra -> Kantara)")
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Hybrid Search: Smart Sequence > Fuzzy
    """
    ...
    try:
        ...
        q_fuzzy = clean_text_for_fuzzy(query)  # "katra" (with spaces if any)
        
        if not q_tight: return []

        candidates = []
        
        for title_key, data in fuzzy_movie_cache.items():
            t_tight = title_key.replace(" ", "") # "kantara"
            t_fuzzy = title_key # "kantara"
            
            # --- A. SMART SEQUENCE SCORE ---
            # This handles "Katra" -> "Kantara" (High Score)
            # This handles "Kajgtara" -> "Kantara" (High Score, skips j,g)
            # This handles "Katra" -> "Kesari" (Low Score, missing letters)
            seq_score = get_advanced_sequence_score(q_tight, t_tight)
            
            final_score = 0
            match_type = "fuzzy"
            
            if seq_score > 60: # Threshold for a decent sequence match
                final_score = 200 + seq_score # Boost above fuzzy
                match_type = "sequence"
                
                # Penalty for huge length difference (Kantara vs Kantara Part 2...)
                len_diff = abs(len(t_tight) - len(q_tight))
                final_score -= len_diff
                
            else:
                # --- B. FUZZY FALLBACK ---
                # Only if sequence failed significantly
                fuz_score = fuzz.WRatio(q_fuzzy, t_fuzzy)
                if fuz_score > 60:
                    final_score = fuz_score
                else:
