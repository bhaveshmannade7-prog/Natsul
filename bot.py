...
# +++++ NAYE HELPER FUNCTIONS (Purane code se) +++++
...
def extract_movie_info(caption: str | None) -> Dict[str, str] | None:
    """Purane code ka behtar caption parser."""
...
def parse_filename(filename: str) -> Dict[str, str | None]:
    """
    Yeh purane code ka behtar file parser hai.
    """
...
# =======================================================
# +++++ BOT HANDLERS: SEARCH & SEND (KOI CHANGE NAHI) +++++
# =======================================================

@dp.message(F.text & ~F.text.startswith("/") & (F.chat.type == "private"))
@handler_timeout(20)
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    # --- YEH HANDLER SAHI THA. ISE BADALNE KI ZAROORAT NAHI. ---
    # Yeh M+M+N architecture ke liye failover a-b-c-b-a-c se behtar karta hai.
...
# =======================================================
# +++++ BOT HANDLERS: ADMIN MIGRATION (FIXED) +++++
# =======================================================
...
async def migration_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # 'clean_title_val' ko DB calls mein pass karein (jahaan zaroori hai)
...
async def auto_index_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # 'clean_title_val' ko DB calls mein pass karein
    ...
    db2_task = db_fallback.add_movie(imdb_id, title, year, file_id, message_id, message.chat.id, clean_title_val, file_unique_id)
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    for i, item in enumerate(mlist):
        ...
        if (i + 1) % 100 == 0 or (now - start_import_time).total_seconds() > 10 or (i+1) == total:
            # Tasks ko parallel execute karein
            ...
            db1_tasks, db2_tasks, neon_tasks = [], [], [] # Lists ko clear karein
