...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Smart Anchor V3 (1000 Candidates + Strict Subsequence)")
...
#
# ==================================================
# +++++ NEW: SMART ANCHORED SEQUENCE SCANNER +++++++
# ==================================================
#
def get_anchored_sequence_score(query: str, target: str) -> int:
    """
    Strict Subsequence Check with Anchors.
    1. First letter MUST match (Bonus).
    2. Last letter MUST match (Bonus).
    3. ALL query characters must appear in target in order.
    """
    if not query or not target: return 0
    
    # 1. Start Anchor
    if query[0] != target[0]:
        return 0 # Start doesn't match? Trash it.
        
    score = 100 # Base score since start matches
    
    # 2. End Anchor Bonus
    if query[-1] == target[-1]:
        score += 50
        
    # 3. Strict Sequence Walk
    last_idx = -1
    gaps = 0
    matches = 0
    ...
    for char in query:
        ...
        if found_idx == -1:
            # Critical failure: Character not found in order
            # Example: 'Karma' searching for 't' (from Katra) -> Fail
            return 0
        ...
        # a(1) -> t(3): Gap 1 (n)
        if last_idx != -1:
            gap = found_idx - last_idx - 1
            gaps += gap
            
        matches += 1
        last_idx = found_idx

    # If we reached here, ALL chars exist in sequence.
    # Penalize for gaps (distance), reward for compactness
    score += (matches * 10)
    score -= (gaps * 2) # Small penalty for skipping letters
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Smart V3 Search:
    1. Fetch TOP 1000 candidates (Broad net).
    2. Apply Anchored Sequence Logic (Accuracy).
    """
    ...
    try:
        ...
        # 2. STEP 1: RAPIDFUZZ BROAD FETCH (Limit 1000)
        # Using limit=1000 ensures 'Kantara' is in the list even if scored low initially
        all_titles = list(fuzzy_movie_cache.keys())
        
        pre_filtered = process.extract(
            q_fuzzy, 
            all_titles, 
            limit=1000,  # Huge limit to ensure target is present
            scorer=fuzz.WRatio, 
            score_cutoff=30
        )
        ...
        # 3. STEP 2: SMART ANCHOR RE-RANKING
        # Iterating 1000 items in Python is fast enough (approx 5-10ms)
        for clean_title_key, fuzz_score, _ in pre_filtered:
            ...
            # "Katra" vs "Kantara" -> Returns High Score (~180)
            # "Katra" vs "Karma" -> Returns 0 (Missing 't')
            seq_score = get_anchored_sequence_score(q_tight, t_tight)
            ...
            if seq_score > 0:
                ...
                final_score = 500 + seq_score # Massive Boost
                match_type = "smart_seq"
                ...
                # Subsequence broken (e.g. Karma), use fuzzy score
