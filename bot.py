...
from aiogram.exceptions import TelegramAPIError, TelegramBadRequest, TelegramRetryAfter # NAYA: TelegramRetryAfter
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if not current_webhook or current_webhook.url != WEBHOOK_URL:
                     logger.info(f"Webhook set kiya ja raha hai: {WEBHOOK_URL} (Attempt {attempt+1}/{max_attempts})...")
                ...
                break # Agar safal ho gaya toh loop se bahar
            except TelegramRetryAfter as e:
                wait_time = int(e.retry_after or retry_delay) + 1
                logger.warning(f"Webhook Set Flood Control hit hua. {wait_time} seconds tak wait kar rahe hain...")
                if attempt + 1 == max_attempts:
                     logger.error("FATAL: Webhook set karne ki max koshish (5) poori ho gayi।")
                     raise RuntimeError("Webhook setup failed due to Telegram Flood Control.") from e
                await asyncio.sleep(wait_time)
            except Exception as e:
                logger.error(f"Webhook setup mein error: {e}", exc_info=True)
                if attempt + 1 == max_attempts:
                    raise RuntimeError("Webhook setup failed due to unexpected error.") from e
                await asyncio.sleep(3) # General error ke liye 3 second wait karein
        else:
            logger.warning("WEBHOOK_URL set nahi ho saka ya Telegram Flood Control ki wajah se fail hua।")
    else:
...
async def bot_webhook(update: dict, background_tasks: BackgroundTasks, request: Request):
    ...
    try:
        ...
        if telegram_update.message:
            ...
            elif telegram_update.message.forward_from_chat:
                 priority = PriorityQueueSystem.PRIORITY_CRITICAL # Migration ko bhi high priority
            # User message ya callback hai toh SEARCH/COPY priority de sakte hain
            elif telegram_update.message.text and not telegram_update.message.text.startswith('/'):
                 priority = PriorityQueueSystem.PRIORITY_SEARCH
        elif telegram_update.callback_query and telegram_update.callback_query.data.startswith('get_'):
             priority = PriorityQueueSystem.PRIORITY_COPY
