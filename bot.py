...
try:
    ...
    # --- FIX 1: Redis URL ke aadhar par Storage set karein ---
...
# --- FIX 2: Webhook Setup Lock Key ---
WEBHOOK_SETUP_LOCK = "webhook:setup:lock"
...
async def shutdown_procedure():
    ...
    # Webhook delete karne se pehle lock release karein
    try:
        if redis_cache.enabled:
            await redis_cache.release_lock(NEON_INIT_LOCK_KEY)
            await redis_cache.release_lock(WEBHOOK_SETUP_LOCK) 
            logger.info("Released Redis locks.")
    except Exception as e:
...
# Fuzzy cache load background task
...
async def lifespan(app: FastAPI):
    ...
    # --- FIX 3: NeonDB Init ko Redis Lock se synchronize karein ---
    lock_acquired = False
    if redis_cache.enabled:
        ...
        if not redis_cache.enabled or lock_acquired: 
            ...
            logger.info("Database 3 (NeonDB/Postgres Backup) initialization safal.")
             ...
             if not await redis_cache.wait_for_lock_release(NEON_INIT_LOCK_KEY, timeout=60):
                 logger.warning("NeonDB init lock release nahi hua, proceeding (Potential DB not ready).")
             else:
                 logger.info("NeonDB init lock released, proceeding.")
        
        # --- FIX: DB init ke baad thoda wait karein (Postgres Concurrent Update fix) ---
        if lock_acquired:
             await asyncio.sleep(1) # Doosre workers ko thoda time dein
             logger.info("NeonDB init ke baad 1s ka delay diya gaya।")
    ...
    # --- FIX 4: Fuzzy Cache Load background task mein shuru karein ---
    global monitor_task
    ...
    # --- FIX 5: Webhook Setup ko Lock se synchronize karein ---
    if WEBHOOK_URL:
        webhook_lock_acquired = False
        if redis_cache.enabled:
            # Webhook setup lock acquire karein
            webhook_lock_acquired = await redis_cache.acquire_lock(WEBHOOK_SETUP_LOCK, timeout=60)
            if not webhook_lock_acquired:
                logger.warning("⚠️ Webhook setup lock nahi mila, Webhook setup skip. Doosra worker handle kar raha hoga.")

        if not redis_cache.enabled or webhook_lock_acquired:
            
            # --- FIX 6: Flood control handle karein ---
            retry_count = 0
            while retry_count < 3:
                try:
                    ...
                    logger.warning(f"Telegram Flood Limit (SetWebhook). {retry_after}s ke liye wait kar raha hoon.")
            ...
            if redis_cache.enabled and webhook_lock_acquired:
                await redis_cache.release_lock(WEBHOOK_SETUP_LOCK)
