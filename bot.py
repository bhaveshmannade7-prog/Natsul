...
def parse_filename(filename: str) -> Dict[str, str]:
    ...
    year = None
    # 1. Look for (YYYY)
    match_paren = re.search(r"\(((19[89]\d|20[0-3]\d))\)", filename) # 1980-2039 in parens
    if match_paren:
        year = match_paren.group(1)
    else:
        # 2. Look for bare YYYY
        matches_bare = re.findall(r"\b((19[89]\d|20[0-3]\d))\b", filename) # 1980-2039 bare
        if matches_bare:
            year = matches_bare[-1][0] # Get last bare year
            
    title = os.path.splitext(filename)[0].strip()
    
    # 3. Clean title (remove year, resolution, tags, etc.)
    if year:
        # Remove year from end
        title = re.sub(rf"(\s*\(?{year}\)?\s*)$", "", title, flags=re.IGNORECASE).strip()
        
    # Remove common tags and resolution
    title = re.sub(r"\[.*?\]", "", title, flags=re.IGNORECASE) # Remove [anything]
    title = re.sub(r"\(.*?\)", "", title, flags=re.IGNORECASE) # Remove (anything) - this is broad, but parse_filename(year) ran first
    ...
    # If title became empty, fall back to original (minus tags)
    if not title:
        title = os.path.splitext(filename)[0].strip()
...
async def start_command(message: types.Message):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        # FIX: Admin ko batayein ki Algolia ka data count kya hai
        if algolia_ready:
             # Algolia mein data count check karne ke liye koi direct API nahi hai, 
             # lekin hum Algolia se ek dummy search request karke dekh sakte hain ya /sync_algolia chalaane ko keh sakte hain.
             # Yahan hum MongoDB count dikhate hain aur Algolia sync ke liye hint dete hain.
             algolia_hint = " (Chalaane ke liye /sync_algolia zaroor karein)" if movie_count_raw > 0 and not algolia_ready else ""
        else:
             algolia_hint = " (Algolia Disconnected)"
        ...
        admin_message = (
            ...
            f"👥 Users: {user_count:,}\n"
            f"🎬 Movies (DB): {movie_count_str}\n"
            f"⚡️ Algolia: <b>{algolia_status}</b>{algolia_hint}\n"
            f"⏰ Uptime: {get_uptime()}\n\n"
            f"<b>Commands</b>\n"
            f"/stats | /health\n"
            f"/broadcast (Reply)\n"
...
async def search_movie_handler(message: types.Message):
    ...
    # CRITICAL DIAGNOSTIC: Query ko log karein
    logger.info(f"Searching Algolia for query: '{original_query}' (User: {user_id})")

    search_results = await algolia_search(original_query, limit=20) # Use the wrapper

    # 6. Handle Results
    if not search_results:
        # FIX: Agar results nahi mile, toh user ko data sync ke liye hint dein (agar admin hai)
        db_count = await safe_db_call(db.get_movie_count(), default=0)
        
        fail_message = f"🥲 Maaf kijiye, '<b>{original_query}</b>' ke liye kuch nahi mila.\n(Spelling mistakes check kar li gayi hain)."
        
        if user_id == ADMIN_USER_ID and db_count > 0:
            fail_message += f"\n\n🚨 <b>ADMIN NOTE:</b> DB mein {db_count:,} movies hain, lekin Algolia search mein nahi mili. Kripya <b>/sync_algolia</b> chalaakar index ko update karein."
        elif db_count == 0:
            fail_message += "\n\nℹ️ <b>NOTE:</b> Database mein koi movie nahi hai."
            
        await safe_tg_call(searching_msg.edit_text(fail_message))
        return
    ...
    for movie in search_results[:max_buttons]:
        ...
        # Agar year hai, toh use title ke saath add karein
        if movie.get('year'):
             display_title = f"{display_title} ({movie['year']})"
...
async def import_json_command(message: types.Message):
    ...
    for i, item in enumerate(mlist):
        try:
            ...
            # This fixes the null 'year' field from the JSON
