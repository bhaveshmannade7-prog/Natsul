...
class BannedFilter(BaseFilter):
    async def __call__(self, message: types.Message, db_primary: Database) -> bool:
        ...
        # is_user_banned is an async method in database.py, but uses synchronous motor/pymongo
        # We must call it and await the coroutine it returns
...
async def lifespan(app: FastAPI):
    ...
    # F.I.X: init_db coroutines/futures ko store karne ke liye list
    db_init_tasks = [] 
    
    # F.I.X: init_db calls are ASYNC methods using MOTOR, BUT they rely on synchronous blocking
    # logic for reconnection/setup inside, so they must be treated as synchronous I/O
    ...
    try:
        ...
        if db1_success:
             ...
             logger.critical("FATAL: Database 1 (MongoDB Primary) initialize nahi ho paya (safe_db_call failed).")
    ...
    try:
        ...
        if db_fallback_success:
             ...
             logger.warning("Database 2 (MongoDB Fallback) initialize nahi ho paya. Bot degraded mode mein chalega.")
    except Exception as e:
    ...
    # FIX for check_if_lock_exists: It is an async method in database.py
...
async def health_check():
    ...
    # All check methods are async methods in database.py
