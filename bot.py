...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Hybrid (Mongo1-Exact + Python-Smart-Hybrid-Fuzzy)")
...
# --- FIX: IMPROVED & CONSISTENT CLEANING LOGIC ---
def clean_text_for_search(text: str) -> str:
    """Cleans text for search indexing (Strict & Consistent)."""
    if not text: return ""
    text = text.lower()
    # Season info remove karein pehle
    text = re.sub(r"\b(s|season)\s*\d{1,2}\b", "", text)
    # Special characters ko space bana dein taaki words merged na hon
...
#
# ==================================================
# +++++ FINAL PRO FUZZY SEARCH (Hybrid Logic) +++++
# ==================================================
#
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    HYBRID SEARCH LOGIC (Best for Typo + Partial Match):
    1. Step 1: Use 'token_set_ratio' to find TOP 50 candidates (Broad Search).
       - Yeh "Avengers Endgame" dhoond lega jab user "Avengers" likhega.
    2. Step 2: Use 'ratio' (Levenshtein) to Re-Rank them.
       - Yeh "Katra" ko "Kantara" se match karega, par "Toilet..." se ignore karega.
    3. Step 3: Weightage (60% Ratio + 40% Token) for final score.
    """
    ...
    try:
        ...
        # Step 1: Broad Extraction (Kachra bhi aayega, par kaam ki cheez miss nahi hogi)
        # Cutoff 50 rakha hai taaki typos miss na ho
        ...
        for (matched_title, token_score, _) in candidates:
            movie_dict = fuzzy_movie_cache[matched_title]
            
            # Step 2: Strict Spelling Check (The Magic Filter)
            # 'fuzz.ratio' pure spelling check karta hai.
            # Aveger vs Avengers = 92 (High)
            # Aveger vs Kesari Veer = 30 (Low)
            spelling_score = fuzz.ratio(query, matched_title)
            
            # Step 3: Weighted Final Score
            # Hum Spelling ko 60% importance denge, aur Token (Words) ko 40%
            # Isse Katra -> Kantara (High Ratio) select hoga, aur Toilet (Low Ratio) reject.
            final_score = (spelling_score * 0.6) + (token_score * 0.4)
            
            # BONUS: Agar movie ka naam query se start hota hai
            if matched_title.startswith(query):
                final_score += 10
            
            # BONUS: Agar query 4 letter se badi hai aur spelling score bohot high hai
            if len(query) > 3 and spelling_score > 80:
                final_score += 10

            # Filtering: Agar final score acceptable hai (60+)
            if final_score > 60:
        ...
        # Step 4: Sort by Final Weighted Score
...
async def health_check():
    ...
    return {
        ...
        "search_logic": "Hybrid (Mongo1-Exact + Python-Smart-Hybrid-Fuzzy)",
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        search_status = f"⚡️ Hybrid (Mongo-Exact + Python-Smart-Hybrid-Fuzzy)"
        ...
        admin_message = (
            ...
            f"<b>⚙️ Search Configuration (SMART)</b>\n"
            ...
            f"  - <b>/force_rebuild_m1</b> ⚠️ (ZABARDASTI M1 data fix karein - USE THIS IF SEARCH IS BAD)\n" # <-- NAYA COMMAND
...
async def search_switch_command(message: types.Message):
    await safe_tg_call(message.answer("ℹ️ Search switch ki ab zaroorat nahi hai.\nBot ab hamesha <b>Hybrid (Mongo-Exact + Python-Smart-Hybrid-Fuzzy)</b> ka istemal karta hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
