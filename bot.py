...
try:
    ...
    logging.warning("Redis library not found. Install via: pip install redis")
    redis = None
...
try:
    ...
    # --- NEW: Multi-Token & Redis Config ---
    WORKER_BOT_TOKENS = os.getenv("WORKER_BOT_TOKENS", "").split(",")
    REDIS_URL = os.getenv("REDIS_URL", "")
    REDIS_TTL = int(os.getenv("REDIS_TTL", "86400")) # 24 Hours default
    
    # --- NEW: Priority Queue Config ---
    SEM_LIMIT_HIGH = int(os.getenv("SEM_LIMIT_HIGH", "200")) # File Send
    SEM_LIMIT_MED = int(os.getenv("SEM_LIMIT_MED", "150"))   # Search
    SEM_LIMIT_LOW = int(os.getenv("SEM_LIMIT_LOW", "50"))    # Menu/Help
...
# --- NEW: Priority Semaphores ---
PRIORITY_HIGH = asyncio.Semaphore(SEM_LIMIT_HIGH)  # File Sending
PRIORITY_MED = asyncio.Semaphore(SEM_LIMIT_MED)    # Searching
PRIORITY_LOW = asyncio.Semaphore(SEM_LIMIT_LOW)    # General
...
class RedisCache:
    ...
    async def connect(self):
        if not self.url or not redis:
            logger.info("Redis URL missing or lib not installed. Running in DB-only mode.")
            return
        try:
    ...
    async def set(self, key: str, value: str, ttl: int = 86400):
        ...
        try: await self.client.set(key, value, ex=ttl)
        except Exception as e: logger.error(f"Redis SET error: {e}")
...
# ============ NEW: MULTI-BOT POOL MANAGER ============
class BotPool:
    def __init__(self, main_token, worker_tokens):
        self.main_bot = Bot(token=main_token, default=DefaultBotProperties(parse_mode=ParseMode.HTML))
        self.worker_bots = []
        for t in worker_tokens:
            if t.strip() and t.strip() != main_token:
                self.worker_bots.append(Bot(token=t.strip(), default=DefaultBotProperties(parse_mode=ParseMode.HTML)))
        
        # Combine all for internal rotation, but separate logic ensures main bot is priority
        self.all_bots = [self.main_bot] + self.worker_bots
        self.iterator = cycle(self.all_bots)
        logger.info(f"🤖 Bot Pool Initialized: 1 Main + {len(self.worker_bots)} Workers.")

    def get_next_bot(self) -> Bot:
        """Rotates through bots for high-load operations."""
        if not self.worker_bots:
            return self.main_bot
        return next(self.iterator)

    async def close_all(self):
        if self.main_bot.session: await self.main_bot.session.close()
        for b in self.worker_bots:
            if b.session: await b.session.close()

bot_pool = BotPool(BOT_TOKEN, WORKER_BOT_TOKENS)
# Compatibility alias for existing code
bot = bot_pool.main_bot 
...
async def shutdown_procedure():
    ...
    try:
        ...
        logger.info("All Bot sessions close ho gaye.")
...
async def health_check():
    ...
    return {
        ...
        "redis_connected": bool(redis_cache.client),
        "worker_bots": len(bot_pool.worker_bots),
        "uptime": get_uptime(),
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    async with PRIORITY_LOW: # PRIORITY: Low
        ...
        if user_id == ADMIN_USER_ID:
            ...
            admin_message = (
                ...
                f"  - 🧠 Redis Cache: {'Active' if redis_cache.client else 'Inactive'}\n"
                f"  - 🤖 Worker Bots: {len(bot_pool.worker_bots)} active\n"
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    async with PRIORITY_MED: # PRIORITY: Medium
        user = message.from_user
        if not user: return
        user_id = user.id
        ...
        cached_json = await redis_cache.get(f"search:{clean_query}")
        unique_movies = {}
        if cached_json:
            try:
                unique_movies = json.loads(cached_json)
                logger.info(f"Redis HIT for '{clean_query}'")
            except Exception as e:
                logger.error(f"Redis JSON load error: {e}")
        ...
        if not unique_movies:
            ...
            if unique_movies:
                try:
                    await redis_cache.set(f"search:{clean_query}", json.dumps(unique_movies), ttl=REDIS_TTL)
                except Exception as e:
            ...
            # --- End Hybrid Search ---
             ...
             searching_msg = await safe_tg_call(message.answer(f"🔎 <b>{original_query}</b> (Cached) found..."), semaphore=TELEGRAM_COPY_SEMAPHORE)

        if not unique_movies:
            if searching_msg:
                await safe_tg_call(searching_msg.edit_text(f"🥲 Maaf kijiye, '<b>{original_query}</b>' ke liye kuch nahi mila."))
            else:
                 await safe_tg_call(message.answer(f"🥲 Maaf kijiye, '<b>{original_query}</b>' ke liye kuch nahi mila."), semaphore=TELEGRAM_COPY_SEMAPHORE)
            return
...
async def get_movie_callback(callback: types.CallbackQuery, db_primary: Database, db_fallback: Database):
    async with PRIORITY_HIGH: # PRIORITY: High (File Send)
        user = callback.from_user
        if not user: 
        ...
        # --- NEW: Token Rotation for Sending ---
        # NOTE: For Private Chats, only the Main Bot can usually reply.
        # However, for robustness, we try rotating. If user hasn't started worker bot, it fails.
        # Fallback logic handles this.
        sender_bot = bot_pool.get_next_bot()
        ...
        try:
            ...
            if is_valid_for_copy:
                try:
                    ...
                    # Retry with Main Bot if Worker failed due to "User hasn't started bot"
                    copy_result = await safe_tg_call(
                        bot.copy_message(
                            chat_id=user.id,
                            from_chat_id=int(movie["channel_id"]),
            ...
            if not success:
                ...
                if not movie.get("file_id"):
                    ...
                    try:
                        send_result = await safe_tg_call(sender_bot.send_document(
                            chat_id=user.id,
...
async def stats_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    stats_msg = (
        ...
        f"  - 🔄 Workers: {len(bot_pool.worker_bots)}\n\n"
...
async def broadcast_command(message: types.Message, db_primary: Database):
    ...
    async def send_to_user(user_id: int):
        ...
        # --- NEW: Use Worker Bot for Broadcast to avoid Main Bot Flood ---
        broadcaster = bot_pool.get_next_bot()
        try:
            res = await safe_tg_call(
                broadcaster.copy_message(chat_id=user_id, from_chat_id=message.chat.id, message_id=message.reply_to_message.message_id),
                timeout=10, semaphore=TELEGRAM_BROADCAST_SEMAPHORE
            )
        except TelegramForbiddenError:
             # Retry with main bot if failed
             res = await safe_tg_call(
