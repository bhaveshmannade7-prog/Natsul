...
# FIX: DB Concurrency ko 15 se 50 kiya gaya taki stalling kam ho
...
def handler_timeout(timeout: int = HANDLER_TIMEOUT):
    def decorator(func: Callable):
        @wraps(func)
        @priority_wrapper(getattr(func, '__handler_priority__', 'LOW'))
...
# ============ SMART INTENT ENGINE V5 (No Change) ============
def get_smart_match_score(query: str, target: str) -> int:
    ...
    if target.startswith("the "):
        ...
        if not target_core: target_core = target 
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    ...
    try:
        q_tight = clean_text_for_search(query) 
...
async def lifespan(app: FastAPI):
    ...
    try:
        # NOTE: db_neon.init_db() ab core_services.py mein NEON_INIT_LOCK se protected hai
        ...
        # Fix: RuntimeError exception ko init_db mein hi handle kiya gaya hai agar lock fail ho.
        # Agar yahaan pahunchta hai to yeh ek unrecoverable error hai.
...
# ============ WEBHOOK / HEALTHCHECK ROUTES (No Change) ============
...
# =======================================================
# +++++ BOT HANDLERS: USER COMMANDS (No Change) +++++
# =======================================================
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB, db_cache_wrapper: DatabaseCacheWrapper):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        # FIX: Active Tokens ko list mein daala gaya
        active_tokens = [b.token[:5] + '...' for b in bot_manager.bots if bot_manager.bot_status.get(b.token, 0.0) <= time.time()]
        flood_tokens = [b.token[:5] + '...' for b in bot_manager.bots if bot_manager.bot_status.get(b.token, 0.0) > time.time()]
        ...
        admin_message = (
            ...
            f"<b>🔑 Bot Tokens (Active/Flood)</b>\n" # NAYA SECTION
            f"  - ✅ Active: {len(active_tokens)}/{len(bot_manager.bots)} ({', '.join(active_tokens)})\n"
            f"  - ❌ Flood Wait: {len(flood_tokens)} ({', '.join(flood_tokens)})\n\n"
    ...
    # ... (Rest of start_command remains unchanged)
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    clean_query = clean_text_for_search(original_query) 
    if not clean_query:
...
# =======================================================
# +++++ BOT HANDLERS: ADMIN MIGRATION (No Change) +++++
# =======================================================
...
# =======================================================
# +++++ BOT HANDLERS: ADMIN COMMANDS (No Change) +++++
# =======================================================
...
async def force_rebuild_m1_command(message: types.Message, db_primary: Database):
    ...
    await safe_db_call(db_primary.force_rebuild_text_index()) 
...
# =======================================================
# +++++ LOCAL POLLING (Testing ke liye) (No Change) +++++
# =======================================================
