...
class BotManager:
    ...
    def __init__(self, main_token: str, alternate_tokens: List[str]):
        ...
        # Note: We rely on the 'bot' variable being initialized below
        self.main_bot = None 
        ...
        # Initialize bots later in try block to capture exceptions
    ...
    def get_bot_by_token(self, token: str) -> Bot:
        ...
        # Load balancing ke liye yeh hash/token based selection theek hai
...
async def lifespan(app: FastAPI):
    ...
    # --- NEW: Webhook set up for ALL BOTS (Multi-Token Load Balancing) ---
    for bot_instance in bot_manager.get_all_bots():
...
# NOTE: _process_update_safe is now handled by the PriorityQueueWorker
...
async def bot_webhook(token: str, update: dict, background_tasks: BackgroundTasks, request: Request):
    ...
    try:
        ...
        # Rule: DO NOT add ANY blocking I/O or long waits. The submit call is non-blocking.
...
async def start_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        user_count, mongo_1_count, mongo_2_count, neon_count, concurrent_users, mongo_1_ready, mongo_2_ready, neon_ready, redis_ready = await asyncio.gather(
            user_count_task, mongo_1_count_task, mongo_2_count_task, neon_count_task, concurrent_users_task, mongo_1_ready_task, mongo_2_ready_task, neon_ready_task, redis_ok_task
        )
