...
logging.getLogger("asyncpg").setLevel(logging.WARNING) # Add asyncpg
...
if not BOT_TOKEN:
    ...
    raise SystemExit(1)
if not DATABASE_URL:
    logger.critical("Missing DATABASE_URL environment variable! Exiting.")
    raise SystemExit(1)
if ADMIN_USER_ID == 0:
...
async def shutdown_procedure(loop):
    ...
    # 5. Dispose Database Engine Pool (important for clean exit)
    try:
        if db and db.engine:
...
def handle_shutdown_signal(signum, frame):
    ...
    try:
        ...
        # Use ensure_future or create_task depending on Python version / preference
        asyncio.ensure_future(shutdown_procedure(loop), loop=loop)
        # Optionally, force stop loop after a delay if shutdown hangs
        # loop.call_later(10, loop.stop) # Force stop after 10 seconds
    except RuntimeError:
...
def get_uptime() -> str:
    delta = datetime.utcnow() - start_time; total_seconds = int(delta.total_seconds())
...
async def check_user_membership(user_id: int) -> bool:
    ...
    try:
        ...
        # Log specific reasons for failure if needed
        # if not is_in_channel: logger.debug(f"User {user_id} not in channel {JOIN_CHANNEL_USERNAME} (Status: {getattr(channel_member, 'status', channel_member)})")
        # if not is_in_group: logger.debug(f"User {user_id} not in group {USER_GROUP_USERNAME} (Status: {getattr(group_member, 'status', group_member)})")

        return is_in_channel and is_in_group

    except Exception as e:
         # Log only unexpected errors, not common API errors like "user not found" which indicate non-membership
...
def extract_movie_info(caption: str):
    ...
    # Look for Season pattern more robustly
    if len(lines) > 1 and re.search(r"^\s*[Ss](eason)?\s*\d{1,2}\b", lines[1]): title += " " + lines[1].strip()
    if title: info["title"] = title
    imdb = re.search(r"(tt\d{7,})", caption); year = re.findall(r"\b(19[89]\d|20[0-2]\d)\b", caption) # Regex for years 1980-2029
    if imdb: info["imdb_id"] = imdb.group(1)
    if year: info["year"] = year[-1] # Get last found year
    return info if "title" in info else None

def parse_filename(filename: str) -> Dict[str, str]:
    year = None; match_paren = re.search(r"\(((19[89]\d|20[0-2]\d))\)", filename) # 1980-2029 in parens
    if match_paren: year = match_paren.group(1)
    else: matches_bare = re.findall(r"\b((19[89]\d|20[0-2]\d))\b", filename); year = matches_bare[-1][0] if matches_bare else None # Last bare year 1980-2029
    title = os.path.splitext(filename)[0].strip()
    # Clean year string from title if found
    if year: title = re.sub(rf"(\s*\(?{year}\)?\s*)$", "", title).strip() # Remove year from end
    # Basic cleaning (replace dots/underscores with spaces)
...
async def lifespan(app: FastAPI):
    ...
    try:
        await db.init_db() # This now includes a connection test
    ...
    try:
        algolia_ok = await initialize_algolia() # Now called correctly from here
        if algolia_ok: logger.info("Algolia initialization successful via lifespan.")
...
async def health_check():
    ...
    db_ok = isinstance(db_ok_raw, int) and db_ok_raw >= -1 # Allow 0 or more movies, or -1 for error state
    ...
    # Could add more checks here (e.g., check Telegram API connection)
    ...
    }#, status_code=status_code # FastAPI returns status based on return, can use Response object for explicit code
...
async def check_join_callback(callback: types.CallbackQuery):
    ...
    # Ensure capacity before potentially long membership check
    ...
    if is_member:
        ...
        # Inform user clearly, don't show alert if join keyboard is present
        await safe_tg_call(callback.answer("❌ Aapne abhi tak Channel/Group join nahi kiya hai. Join karke dobara try karein.", show_alert=False))
        # Maybe resend the join message if the original was deleted? (Optional)
        # join_markup = get_join_keyboard()
        # if join_markup:
        #    await safe_tg_call(callback.message.answer("Kripya join karein:", reply_markup=join_markup))
...
async def get_movie_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Check channel_id validity more strictly
...
async def cleanup_users_command(message: types.Message):
    ...
    if msg: await safe_tg_call(msg.edit_text(txt)); else: await safe_tg_call(message.answer(txt))
...
async def add_movie_command(message: types.Message):
    ...
    db_res = await safe_db_call(db.add_movie(imdb_id, title, year, file_id, msg_id, chan_id))
...
async def import_json_command(message: types.Message):
    ...
    if not msg: return
    try:
        fio = io.BytesIO(); file = await bot.get_file(doc.file_id); loop = asyncio.get_running_loop();
        # Download in executor to avoid blocking
        await loop.run_in_executor(executor, lambda: bot.download_file(file.file_path, fio, timeout=180)) # Longer timeout for download
        fio.seek(0);
        mlist = json.loads(fio.read().decode('utf-8')); assert isinstance(mlist, list)
    ...
    total = len(mlist); a, u, s, fdb = 0, 0, 0, 0; ag_batch = []; st = datetime.utcnow()
    await safe_tg_call(msg.edit_text(f"⏳ Processing {total:,} items (DB)..."))
    for i, item in enumerate(mlist):
        try:
            fid = item.get("file_id"); fname = item.get("filename") or item.get("title"); # Use filename or title
            if not fid or not fname: s += 1; continue # Skip if no file_id or name
            # Generate IMDB ID if missing, using file_id hash
            imdb = item.get("imdb_id") or f"json_{hashlib.md5(fid.encode()).hexdigest()}"
            info = parse_filename(fname); title = info["title"] or "Untitled"; year = info["year"]
            # Call DB add/update
            db_res = await safe_db_call(db.add_movie(imdb, title, year, fid, AUTO_MESSAGE_ID_PLACEHOLDER, 0)) # Placeholder msg/channel
...
async def rebuild_index_command(message: types.Message):
    msg = await safe_tg_call(message.answer("🔧 Rebuilding `clean_title` index in PostgreSQL DB... (DB Only)"))
    if not msg: return
...
async def export_csv_command(message: types.Message):
    ...
    try:
        if kind == "users":
            ...
            # Ensure proper CSV quoting for names containing commas or quotes
            csv_data = header + "\n".join([f"{r['user_id']},\"{r['username'] or ''}\",\"{r['first_name'].replace('\"','\"\"') if r['first_name'] else ''}\",\"{r['last_name'].replace('\"','\"\"') if r['last_name'] else ''}\",{r['joined_date']},{r['last_active']},{r['is_active']}" for r in rows])
...
async def auto_index_handler(message: types.Message):
    ...
    db_res = await safe_db_call(db.add_movie(imdb_id, title, year, file_id, message.message_id, message.chat.id))
...
if __name__ == "__main__":
    ...
    # If running locally without uvicorn/FastAPI, ensure DB & Algolia are initialized.
    # Running with `python bot.py` might require different initialization logic
    # if you remove FastAPI entirely.
