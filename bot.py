...
async def stats_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    stats_msg = (
        ...
        f"<b>Queue Status</b>\n" # NAYA
        f"  - 🔄 Active Tasks: {queue_status['current_active_tasks']}\n"
        f"  - ⏳ Queue Size: {queue_status['current_queue_size']}\n"
...
async def rebuild_clean_titles_m1_command(message: types.Message, db_primary: Database):
    ...
    await load_fuzzy_cache(db_primary)
    await safe_tg_call(bot_instance.send_message(message.chat.id, "✅ Fuzzy cache bhi reload ho gaya hai."))
