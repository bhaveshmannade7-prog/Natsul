...
async def start_command(bot: Bot, message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    is_member = await check_user_membership(bot, user_id) # NAYA: bot pass kiya
...
async def set_limit_command(bot: Bot, message: types.Message):
    ...
    except (ValueError, AssertionError): await safe_tg_call(message.answer("❌ Limit 5 aur 5000 ke beech ek number hona chahiye."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def check_token_status_command(bot: Bot, message: types.Message):
    ...
    reply_text = (
        ...
        f"{'\n'.join(tokens_status)}\n\n"
        f"ℹ️ Flooded token se user ko automatic dusre token par redirect kiya jaata hai."
    )
