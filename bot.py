...
try:
    ...
    logging.critical("--- rapidfuzz library nahi मिली! ---")
        ...
        if webhook_url_for_token:
            async def set_webhook_safely(bot_instance: Bot, url: str):
                ...
                async with WEBHOOK_SEMAPHORE: # Ek baar mein sirf ek webhook call
                    # Flood limit se bachne ke liye thoda extra buffer
                    await asyncio.sleep(0.5) 
                    try:
                        current_webhook = await safe_tg_call(bot_instance.get_webhook_info())
                        
                        # Agar current_webhook None ya False hai, toh get_webhook_info fail hua
                        ...
                        if not is_webhook_set:
                             ...
                             # safe_tg_call ke andar hi retry logic hai
                        ...
                        # Error handling agar safe_tg_call ke bahar koi exception aaya
