...
async def shutdown_procedure():
    ...
    # NEW: Stop Managers
    # --- FIX: Ensure lock is released during shutdown ---
    try:
...
async def lifespan(app: FastAPI):
    ...
    # --- FIX: Use Redis Lock for NeonDB initialization to prevent concurrent 'tuple concurrently updated' error ---
    if redis_cache.enabled:
        ...
        if lock_acquired:
            ...
            logger.warning(f"⚠️ Redis Lock '{NEON_INIT_LOCK_KEY}' nahi mila, 120s wait kiya.")
            # If lock is not acquired within timeout, another worker might be initializing.
            # We will proceed, but expect potential error if the other worker is still running init_db.
    
    try:
        # This call should now be safer due to the lock
