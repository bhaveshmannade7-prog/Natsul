...
async def shutdown_procedure():
    ...
    try:
        if redis_cache.enabled:
            ...
            await redis_cache.release_lock(WEBHOOK_SETUP_LOCK) # FIX: Webhook lock bhi release karein
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    if not message.reply_to_message or not message.reply_to_message.document:
        await safe_tg_call(message.answer("❌ Kripya .json file ko reply k करके command dein."), semaphore=TELEGRAM_COPY_SEMAPHORE)
        return
