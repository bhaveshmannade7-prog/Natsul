...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    ...
    try:
        ...
        # Clean query for exact anchoring (spaces, dots, underscores replaced by single space)
        ...
        # 2. Match with removed prefix 'the '
        if q_anchor.startswith('the '):
             q_no_the = q_anchor[4:]
             if q_no_the in fuzzy_movie_cache:
                 data = fuzzy_movie_cache[q_no_the]
                 if data['imdb_id'] not in seen_imdb:
                      candidates.append({
                         'imdb_id': data['imdb_id'],
                         'title': data['title'],
        ...
        for clean_title_key, fuzz_score, _ in pre_filtered:
            ...
            # Note: Intent score ke liye ab hum q_anchor (space-separated) use kar rahe hain
