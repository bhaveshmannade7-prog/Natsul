...
# --- MASTER FIX: Updated clean_text_for_search ---
def clean_text_for_search(text: str) -> str:
    """Cleans text for search indexing, supporting Unicode (Hindi)."""
    if not text: return ""
    
    # 1. Lowercase
    text = text.lower()
    
    # 2. Remove bracketed/parenthesized content (e.g., (2018), [Hindi])
    # Yeh search ko clean rakhega
    text = re.sub(r"\[.*?\]", " ", text, flags=re.UNICODE)
    text = re.sub(r"\(.*?\)", " ", text, flags=re.UNICODE)
    
    # 3. Keep only Unicode letters, numbers, and spaces.
    # Yeh emoji, punctuation (.,-!), aur underscores (_) ko hata dega.
    # \w Unicode letters+numbers ko match karta hai (Hindi sahit)
    # \s whitespace ko match karta hai
    text = re.sub(r"[^\w\s]", " ", text, flags=re.UNICODE)
    
    # 4. Remove "season" and "s01" type patterns
    text = re.sub(r"\b(s|season)\s*\d{1,2}\b", "", text, flags=re.IGNORECASE | re.UNICODE)
    
    # 5. Normalize whitespace (ek se zyada space ko ek space banao)
...
def extract_movie_info(caption: str):
    ...
    # Common channel/username tags ko title se hatayein
    title_line = re.sub(r"(@\w+)", "", title_line).strip()
    
    # Agar title line mein (Year) hai, toh use alag karein
    year_match = re.search(r"(\((\d{4})\))", title_line) # (YYYY)
    year = None
    if year_match:
        year = year_match.group(2)
        # Title mein se (YYYY) ko hatayein
        title_line = title_line.replace(year_match.group(1), "").strip()
...
def parse_filename(filename: str) -> Dict[str, str]:
    ...
    year = None
    title = filename # Original filename se start karein
    
    # 1. Look for (YYYY)
    match_paren = re.search(r"\(((19[89]\d|20[0-3]\d))\)", title) # 1980-2039 in parens
    if match_paren:
        year = match_paren.group(1)
        # Title se (YYYY) hatayein
        title = title.replace(match_paren.group(0), " ").strip()
    else:
        # 2. Look for bare YYYY
        matches_bare = re.findall(r"\b((19[89]\d|20[0-3]\d))\b", title) # 1980-2039 bare
        if matches_bare:
            year = matches_bare[-1][0] # Get last bare year
            # Title se year hatayein (saavdhaani se)
            title = re.sub(rf"\b({year})\b", " ", title).strip()
            
    # File extension hatayein (agar hai toh)
    ...
    title = re.sub(r"\(.*?\)", " ", title, flags=re.IGNORECASE) # Remove (anything left)
    
    # Common keywords hatayein
    title = re.sub(r"\b(web-rip|org|hindi|dd 5.1|english|480p|720p|1080p|web-dl|hdrip|bluray|dual audio|hevc)\b", " ", title, flags=re.IGNORECASE)
    ...
    # If title became empty, fall back
    if not title:
        title = os.path.splitext(filename)[0].strip()
        # Bas extension aur common dots hatayein
        title = re.sub(r'[._]', ' ', title).strip()
        title = re.sub(r"\s+", " ", title).strip()
...
async def add_movie_command(message: types.Message):
    ...
    # --- FIX: Use the *new* clean_text_for_search ---
...
async def import_json_command(message: types.Message):
    ...
    for i, item in enumerate(mlist):
        try:
            ...
            # --- FIX: Use the *new* parse_filename ---
...
async def sync_algolia_command(message: types.Message):
    ...
    try:
        ...
        # --- FIX: Yeh ab database.py ke *sahi* clean_text function ko use karega (agar zaroorat padi) ---
...
async def rebuild_index_command(message: types.Message):
    ...
    if not msg: return
    # --- FIX: Yeh *bot.py* ke naye, sahi function ko pass karega ---
...
async def auto_index_handler(message: types.Message):
    ...
    # --- FIX: Use the *new* extract_movie_info ---
