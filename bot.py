...
def clean_text_for_search(text: str) -> str:
    ...
    text = re.sub(r"\b(s|season)\s*\d{1,2}\b", "", text) # \b word boundary
...
async def import_json_command(message: types.Message):
    ...
    if not msg: return
    try:
        fio = io.BytesIO(); file = await bot.get_file(doc.file_id)
        
        # === YEH HAI AAPKA JSON DOWNLOAD FIX ===
        # Pehle: await loop.run_in_executor(executor, lambda: bot.download_file(file.file_path, fio, timeout=180)) (GALAT)
        # Ab:
        await bot.download_file(file.file_path, destination=fio, timeout=180)
        # === FIX END ===
