...
# ==================================================
#
def python_fuzzy_search(clean_query: str, original_query: str, limit: int = 10) -> List[Dict]:
    """
    ...
    FIX: 'force_ascii' error fix kiya gaya, aur clean_query ka istemal kiya gaya.
    """
    ...
    try:
        # Behtar performance ke liye keys (clean_titles) ko pehle ek list mein daalein
        choices = list(fuzzy_movie_cache.keys())
        
        # NAYA FIX: Cleaned query ko seedha choices (clean_titles) ke against run karein
        # 'processor' aur 'force_ascii' hataya gaya, kyonki woh error de rahe the.
        results = process.extract(
            clean_query, # Cleaned query ka istemal karein
            choices, 
            scorer=fuzz.WRatio, 
            limit=limit,
            score_cutoff=75 # Cutoff ko 75 par rakha gaya hai
        )
        
        final_list = []
        for (matched_clean_title, score, index) in results:
            # Matched clean title se poori movie dict haasil karein
        ...
        # NAYA FIX (Accuracy Guarantee): Agar exact clean_title match hota hai, 
        # toh use force-add karein aur 100% score dein.
        if clean_query in fuzzy_movie_cache:
            is_already_in_list = any(r['title'] == fuzzy_movie_cache[clean_query]['title'] for r in final_list)
            
            if not is_already_in_list:
                 movie_dict = fuzzy_movie_cache[clean_query]
                 final_list.insert(0, {
                    'imdb_id': movie_dict['imdb_id'],
                    'title': movie_dict['title'],
                    'year': movie_dict.get('year'),
                    'score': 100.0 # Force 100% score
                 })
                 final_list = final_list[:limit] 

        # High-score results ko pehle dikhayein
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    loop = asyncio.get_running_loop()
    # NAYA FIX: python_fuzzy_search ko ab clean_query aur original_query dono denge
    fuzzy_hits_task = loop.run_in_executor(executor, python_fuzzy_search, clean_query, original_query, 10) 
    ...
    if fuzzy_hits:
        ...
        # NAYA FIX: Fuzzy hits को score के हिसाब से sort करके daalein
        fuzzy_hits.sort(key=lambda x: x.get('score', 0), reverse=True)
    ...
    # NAYA FIX: Final results को भी score के हिसाब से sort karein 
