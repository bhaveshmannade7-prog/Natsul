...
# --- NEW IMPORTS ---
# FIXED: name 'TELEGRAM_DELETE_SEMAP_RE' replaced with 'TELEGRAM_DELETE_SEMAPHORE'
...
async def check_join_callback(callback: types.CallbackQuery, bot: Bot, db_primary: Database, redis_cache: RedisCacheLayer):
    ...
    if is_member:
        ...
        try:
            ...
            await safe_tg_call(bot.send_message(user.id, success_text, reply_markup=main_menu), semaphore=TELEGRAM_BROADCAST_SEMAPHORE)
    else:
...
async def get_movie_callback(callback: types.CallbackQuery, bot: Bot, db_primary: Database, db_fallback: Database, redis_cache: RedisCacheLayer):
    ...
    if success and sent_msg_id:
        ...
        # FIXED: Removed typo semaphore name and called from isolated task
...
async def show_admin_panel(message: types.Message, is_edit: bool = False):
    panel_text = (
        ...
        \"• <code>/setshort ON/OFF [LINK]</code> - Monetization Config\\n\"
        \"━━━━━━━━━━━━━━━━━━━━\"
    )
...
# ==========================================
# PROBLEM FIX: FREEZE-PROOF SYNC COMMANDS
# ==========================================
...
async def sync_m12_freeze_fix(message: types.Message, db_primary: Database, db_fallback: Database):
    # Launch logic from original sync_mongo_1_to_2_command in background
