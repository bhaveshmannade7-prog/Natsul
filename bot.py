...
# clean_text_for_search() ko hata diya gaya hai kyonki Atlas Search iski zaroorat nahi.
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        admin_message = (
            ...
            f"<b>⭐️ Atlas Index:</b> Dono Mongo DBs par 'default' Atlas Search Index banayein."
        )
...
# =======================================================
# +++++ BOT HANDLERS: SEARCH & SEND (Atlas Search ke liye taiyar) +++++
# =======================================================

@dp.message(F.text & ~F.text.startswith("/") & (F.chat.type == "private"))
@handler_timeout(20)
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    # Yeh handler M+M+N architecture aur Atlas Search ke liye bilkul sahi hai.
    # Isse badalne ki zaroorat nahi.
...
# =======================================================
# +++++ BOT HANDLERS: ADMIN MIGRATION (FIXED for Atlas) +++++
# =======================================================
...
async def migration_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    info = extract_movie_info(message.caption or "") # <-- Updated helper
    ...
    # clean_title_val ki ab zaroorat nahi hai
    
    # DB calls se clean_title_val ko hata diya gaya hai (7 arguments)
    db1_task = safe_db_call(db_primary.add_movie(imdb_id, title, year, file_id, message_id, channel_id, file_unique_id))
    db2_task = safe_db_call(db_fallback.add_movie(imdb_id, title, year, file_id, message_id, channel_id, file_unique_id))
...
async def auto_index_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # DB calls se clean_title_val ko hata diya gaya hai (7 arguments)
    db1_task = db_primary.add_movie(imdb_id, title, year, file_id, message.message_id, message.chat.id, file_unique_id)
    db2_task = db_fallback.add_movie(imdb_id, title, year, file_id, message.message_id, message.chat.id, file_unique_id)
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    for i, item in enumerate(mlist):
        try:
            ...
            info = parse_filename(fname); # <-- Updated helper
            ...
            # DB calls se clean_title_val ko hata diya gaya hai (7 arguments)
            db1_tasks.append(db_primary.add_movie(imdb, title, year, fid_str, message_id, channel_id, file_unique_id))
            db2_tasks.append(db_fallback.add_movie(imdb, title, year, fid_str, message_id, channel_id, file_unique_id))
...
# --- REBUILD INDEX COMMANDS HATA DIYE GAYE HAIN ---
# Atlas Search ke liye inki zaroorat nahi hai.
