...
# --- Database Imports ---
# clean_text_for_search ki ab zaroorat nahi
...
# --- NAYA: Search Switch Logic ---
SEARCH_MODES = ["mongo_1", "neon", "mongo_2"]
# --- FIX: DEFAULT 1 (NEON) PAR SET KIYA GAYA HAI ---
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        admin_message = (
            ...
            f"/search_switch (Toggle Default)\n\n"
            f"<b>🛠️ Commands</b>\n"
    ...
    if is_member:
        welcome_text = (
            ...
            f"Example: <code>Kantara 2022</code> ya <code>Katra</code>\n\n"
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- NAYA Search Engine Waterfall (Preferred + Failover) ---
    ...
    # Try Neon (Fuzzy) - Yeh typos (Katra) ko pakdega
    ...
    # Try Mongo 1 (Fuzzy Autocomplete)
    ...
    # --- De-duplication ---
    # Results ko unique karein (agar teeno DBs se same result aaye)
    unique_movies = {}
...
async def search_switch_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    await safe_tg_call(message.answer(f"✅ Default search mode ab <b>{CURRENT_SEARCH_MODE.capitalize()}</b> par set hai.\nStatus: {status}"), semaphore=TELEGRAM_COPY_SEMAPHORE)
