...
# --- Search Switch Logic ---
SEARCH_MODES = ["mongo_1", "neon", "mongo_2"]
CURRENT_SEARCH_MODE_INDEX = 1 # Default is Neon (Fuzzy Search)
...
def extract_movie_info(caption: str | None) -> Dict[str, str] | None:
    """Caption se Title, Year, aur IMDB ID nikalta hai."""
...
def parse_filename(filename: str) -> Dict[str, str | None]:
    """
    Filename se tags (jaise 1080p, Hindi, etc.) hatakar saaf Title aur Year nikalta hai.
    """
...
async def health_check():
    ...
    return {
        ...
        "default_search_mode": CURRENT_SEARCH_MODE,
        "uptime": get_uptime(),
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        if (CURRENT_SEARCH_MODE == 'mongo_1' and not mongo_1_ready) or \
           (CURRENT_SEARCH_MODE == 'neon' and not neon_ready) or \
           (CURRENT_SEARCH_MODE == 'mongo_2' and not mongo_2_ready):
        ...
        admin_message = (
            ...
            f"  - 🗄️ Fuzzy Index (Neon): {count_str(neon_count)} vectors\n"
            f"  - ⏱️ System Uptime: {get_uptime()}\n\n"
            f"<b>📡 Node Connectivity (Latency Check)</b>\n"
            f"  - {status_icon(mongo_1_ready)} Node 1: Mongo-Primary (Users + Data)\n"
            f"  - {status_icon(mongo_2_ready)} Node 2: Mongo-Fallback (Data Replica)\n"
            f"  - {status_icon(neon_ready)} Node 3: Neon-Fuzzy (Search Index)\n\n"
            f"<b>⚙️ Search Configuration</b>\n"
            f"  - <b>Default Engine: {search_status}</b>\n"
            f"  - /search_switch (Cycle default search engine)\n\n"
            ...
            f"  - <b>/cleanup_mongo_1</b> (De-duplicate M1)\n"
            f"  - <b>/cleanup_mongo_2</b> (De-duplicate M2)\n"
            f"  - <b>/rebuild_neon_vectors</b> ⚠️ (Fix Neon FTS data)\n"
            f"  - /cleanup_users (Prune inactive users)\n"
            f"  - /rebuild_clean_titles_m1 (Fix M1 search)\n"
            f"  - /rebuild_clean_titles_m2 (Fix M2 search)\n\n"
            f"<b>🗃️ Library Management</b>\n"
            f"  - <b>/remove_library_duplicates</b> ⚠️ (De-duplicate Files)\n"
            f"  - <b>/backup_channel</b> 🚀 (Backup unique files)\n\n"
            f"<b>⭐️ Data Ingestion:</b> Forward files from `LIBRARY_CHANNEL` to ingest into all 3 nodes.\n"
            f"<b>⭐️ Mongo Index:</b> Dono Mongo DBs par 'title_text_index' banaya jaayega."
        )
    ...
    if is_member:
        welcome_text = (
            ...
            f"Example: <code>Kantara</code> ya <code>Katra</code>"
        )
...
async def help_command(message: types.Message, db_primary: Database):
    ...
    help_text = (
        ...
        "   Agar aap spelling galat likhte hain (e.g., <code>Mirjapur</code> ya <code>Katra</code>), toh bhi bot search kar lega.\n\n"
...
# =======================================================
# +++++ BOT HANDLERS: SEARCH & SEND (Aapka M+M+N Logic) +++++
# =======================================================

@dp.message(F.text & ~F.text.startswith("/") & (F.chat.type == "private"))
@handler_timeout(20)
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    # Yeh handler M+M+N architecture ke liye bilkul sahi hai.
    # Ise badalne ki zaroorat nahi.
    ...
    # --- Search Engine Waterfall (Preferred + Failover) ---
    
    m1_ready, n_ready, m2_ready = await asyncio.gather(
        safe_db_call(db_primary.is_ready(), default=False),
        safe_db_call(db_neon.is_ready(), default=False),
        safe_db_call(db_fallback.is_ready(), default=False)
    )
    ...
    # 1. Preferred mode try karein
    if preferred_mode == "mongo_1" and m1_ready:
        search_results = await safe_db_call(db_primary.mongo_primary_search(original_query, limit=20), default=[])
        if search_results: search_engine_used = "Mongo 1 (Preferred)"
            
    elif preferred_mode == "neon" and n_ready:
        search_results = await safe_db_call(db_neon.neondb_search(original_query, limit=20), default=[])
        if search_results: search_engine_used = "Neon (Preferred)"
            
    elif preferred_mode == "mongo_2" and m2_ready:
        search_results = await safe_db_call(db_fallback.mongo_fallback_search(original_query, limit=20), default=[])
        if search_results: search_engine_used = "Mongo 2 (Preferred)"

    # --- Failover Logic ---
    
    # 2. Agar fail hua, toh Neon (sabse pehla failover) try karein
    if not search_results and preferred_mode != "neon" and n_ready:
        logger.debug("Preferred fail, trying Neon (Failover)...")
        search_results = await safe_db_call(db_neon.neondb_search(original_query, limit=20), default=[])
        if search_results: search_engine_used = "Neon (Failover)"

    # 3. Agar ab bhi fail, toh Mongo 1 (doosra failover) try karein
    if not search_results and preferred_mode != "mongo_1" and m1_ready:
        logger.debug("Neon fail, trying Mongo 1 (Failover)...")
        search_results = await safe_db_call(db_primary.mongo_primary_search(original_query, limit=20), default=[])
        if search_results: search_engine_used = "Mongo 1 (Failover)"

    # 4. Agar ab bhi fail, toh Mongo 2 (aakhri failover) try karein
    if not search_results and preferred_mode != "mongo_2" and m2_ready:
        logger.debug("Mongo 1 fail, trying Mongo 2 (Failover)...")
        search_results = await safe_db_call(db_fallback.mongo_fallback_search(original_query, limit=20), default=[])
        if search_results: search_engine_used = "Mongo 2 (Failover)"
    ...
    for movie in search_results:
        # NeonDB se 'imdb_id' nahi aata (agar logic wahi hai), isliye check zaroori hai
        if movie.get('imdb_id') and movie['imdb_id'] not in unique_movies:
            unique_movies[movie['imdb_id']] = movie
        # Fallback agar imdb_id nahi hai (NeonDB ke case mein)
        elif movie.get('title') and not any(m['title'] == movie['title'] for m in unique_movies.values()):
             # Agar NeonDB se result aa raha hai, toh humein get_ callback ke liye ek unique key chahiye
             # Filhaal, hum NeonDB se mile results ko chhod rahe hain agar unmein imdb_id nahi hai
             # Agar NeonDB bhi imdb_id la raha hai, toh yeh 'if' kaafi hai.
             pass
    
    # Agar unique_movies abhi bhi khaali hai (sirf NeonDB se results mile)
    if not unique_movies and search_results:
         for movie in search_results[:max_buttons]:
            if movie.get('imdb_id') and movie['imdb_id'] not in unique_movies:
                 unique_movies[movie['imdb_id']] = movie
            # Agar NeonDB se results aa rahe hain, toh unhe handle karna hoga
            # Yahaan hum maan rahe hain ki sabhi search results mein 'imdb_id' hai
            
    
    for movie in list(unique_movies.values())[:max_buttons]:
    ...
    result_count = len(unique_movies)
    if result_count == 0 and search_results: # NeonDB fallback
...
async def get_movie_callback(callback: types.CallbackQuery, db_primary: Database, db_fallback: Database):
    ...
    # Preparatory message ko edit karein
...
async def stats_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    stats_msg = (
        ...
        f"  - ⚡️ Active ({ACTIVE_WINDOW_MINUTES}m): {concurrent_users:,} / {CURRENT_CONC_LIMIT}\n"
        f"  - 👤 Users (M1): {count_str(user_count)}\n"
        ...
        f"  - {status_icon(mongo_1_ready)} Node 1: Mongo-Primary\n"
        f"  - {status_icon(mongo_2_ready)} Node 2: Mongo-Fallback\n"
        f"  - {status_icon(neon_ready)} Node 3: Neon-Fuzzy\n\n"
        f"<b>Default Search: {search_status}</b>"
    )
...
@handler_timeout(10)
async def search_switch_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if CURRENT_SEARCH_MODE == "mongo_1":
        if await safe_db_call(db_primary.is_ready(), default=False): status = "🟢 (Ready)"
    elif CURRENT_SEARCH_MODE == "neon":
        if await safe_db_call(db_neon.is_ready(), default=False): status = "🟢 (Ready)"
    elif CURRENT_SEARCH_MODE == "mongo_2":
        if await safe_db_call(db_fallback.is_ready(), default=False): status = "🟢 (Ready)"

    logger.info(f"Admin ne default search mode change kiya: {CURRENT_SEARCH_MODE}")
    await safe_tg_call(message.answer(f"✅ Default search engine ab <b>{CURRENT_SEARCH_MODE.capitalize()}</b> par set hai.\nStatus: {status}"), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def sync_mongo_1_to_2_command(message: types.Message, db_primary: Database, db_fallback: Database):
    ...
    # Yeh function 'get_all_movies_for_neon_sync' jaisa hi hai,
    # lekin MongoDB -> MongoDB sync ke liye humein 'get_all_movies_for_neon_sync'
    # ke data format ka hi istemal karna hoga (kyonki 'get_all_movies_for_mongo_sync' 
    # aapke 'database.py' mein define nahi hai, lekin 'get_all_movies_for_neon_sync' hai)
    
    # Hum 'get_all_movies_for_neon_sync' ka istemal karenge kyonki ismein saari zaroori fields hain
    mongo_movies = await safe_db_call(db_primary.get_all_movies_for_neon_sync(), timeout=300)
    ...
    # Hum 'sync_from_mongo' (jo 'database.py' mein nahi hai) ke bajaye
    # 'add_movie' ko loop mein call karenge (kyonki 'sync_from_mongo_bulk' bhi define nahi hai)
    # Yeh 'database.py' ke functions ka istemal karega.
    
    processed_count = 0
    tasks = []
    
    # Mongo 2 (db_fallback) mein data daalne ke liye
    for movie in mongo_movies:
        processed_count += 1
        tasks.append(safe_db_call(db_fallback.add_movie(
            imdb_id=movie.get('imdb_id'),
            title=movie.get('title'),
            year=movie.get('year'), # Yeh field 'get_all_movies_for_neon_sync' se nahi aati, 'None' jayegi
            file_id=movie.get('file_id'),
            message_id=movie.get('message_id'),
            channel_id=movie.get('channel_id'),
            clean_title=clean_text_for_search(movie.get('title')), # Clean title generate karein
...
# --- NAYA COMMANDS (clean_title ke liye) ---
...
# --- YEH NAYA COMMAND HAI (DATA FIX KARNE KE LIYE) ---
...
async def rebuild_neon_vectors_command(message: types.Message, db_neon: NeonDB):
    msg = await safe_tg_call(message.answer("⏳ (NeonDB) mein `title_search_vector` (FTS Data) ko rebuild kiya ja raha hai... (Jo NULL hain)"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    updated_count = await safe_db_call(db_neon.rebuild_fts_vectors(), timeout=540, default=-1)
    if updated_count >= 0:
        await safe_tg_call(msg.edit_text(f"✅ (NeonDB) Rebuild Done!\n{updated_count:,} purane records ke FTS vectors fix kiye gaye."))
    else:
        await safe_tg_call(msg.edit_text("❌ (NeonDB) FTS vectors ko rebuild karne mein error aaya."))
# --- END NAYA COMMAND ---
