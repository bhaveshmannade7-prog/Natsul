...
async def search_movie_handler(message: types.Message):
    ...
    # --- Failovers (Logic Improved) ---
    if not search_results:
        # Try Secondary if preferred was NOT secondary and it IS ready
        ...
        # Try NeonDB if no results yet
        if not search_results and preferred_mode != "neondb" and await safe_db_call(neondb.is_ready(), default=False):
            search_results = await safe_db_call(neondb.neondb_search(original_query, limit=20), default=[])
            search_engine_used = "NeonDB (Failover)"
            
        # Try Primary MongoDB (Final Fallback)
        if not search_results and preferred_mode != "mongodb" and await safe_db_call(db.is_ready(), default=False):
            search_results = await safe_db_call(db.mongo_search_internal(original_query, limit=20), default=[])
            search_engine_used = "MongoDB (Fallback)"
