...
#
# ==================================================
# +++++ FIX: SMART FUZZY SEARCH (Better Accuracy) +++++
# ==================================================
#
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Improved Search Logic:
    1. Uses token_set_ratio: Handles 'Avegers' -> 'Avengers' and 'Katra' -> 'Kantara' better.
    2. High Cutoff (78): Removes junk like 'Himmatwar' for 'Avtar'.
    3. Start Bonus: Gives priority if movie name starts with query.
    """
    ...
    try:
        ...
        # --- FIX: Use token_set_ratio (Best for Typos & Partial Matches) ---
        results = process.extract(
            query,
            choices, 
            scorer=fuzz.token_set_ratio, 
            limit=20,  # Fetch more initially for filtering
            score_cutoff=78 # --- FIX: High cutoff removes junk results ---
        )
        ...
        for (matched_clean_title, score, index) in results:
            ...
            # --- FIX: Priority Boost Logic ---
            # 1. Exact match bonus
            if matched_clean_title == query:
                score += 20
            # 2. Starts-with bonus (e.g. "Avtar" -> "Avatar...")
            elif matched_clean_title.startswith(query):
                score += 10
