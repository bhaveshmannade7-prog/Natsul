...
async def main_polling():
    ...
    try:
        await redis_cache.init_cache() # NAYA
        await db_primary.init_db()
        await db_fallback.init_db()
        await db_neon.init_db()
    ...
    # NAYA: Primary bot ka istemal karein
    await token_manager.get_current_bot().delete_webhook(drop_pending_updates=True) 
    global monitor_task
    monitor_task = asyncio.create_task(monitor_event_loop())
    setup_signal_handlers()
    ...
    try:
        # NAYA: Dispatcher mein TokenManager aur RedisCache pass karein
        await dp.start_polling(
            token_manager.get_current_bot(),
            allowed_updates=dp.resolve_used_update_types(),
            db_primary=db_primary,
            db_fallback=db_fallback,
            db_neon=db_neon,
