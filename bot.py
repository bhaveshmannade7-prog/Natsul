...
from aiogram.dispatcher.middlewares.base import BaseMiddleware # NAYA MIDDLEWARE IMPORT
...
async def lifespan(app: FastAPI):
    ...
    # --- 3 DB Init ---
    # MongoDB 1 ko pehle init karein taaki NeonDB lock ke liye use ho
    try:
...
async def start_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        admin_message = (
            ...
            f"  - <b>/export_users</b> (Saare users ka JSON export)\n"
...
# =======================================================
# +++++ FEATURE 1: ADMIN EXPORT USERS (NEW) +++++
# =======================================================
...
async def export_users_command(message: types.Message, db_primary: Database, bot: Bot):
    msg = await safe_tg_call(message.answer("⏳ Saare active users ka data export kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return

    # DB se data fetch karein
    users_data = await safe_db_call(db_primary.export_all_users_for_json(), timeout=90, default=[])
    
    if not users_data:
        await safe_tg_call(msg.edit_text("❌ Export ke liye koi users nahi mile।"))
        return

    # JSON file banao
    file_name = f"active_users_export_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    
    # CPU-bound JSON serialization ko ThreadPoolExecutor mein chalayein
    loop = asyncio.get_running_loop()
    try:
        json_bytes = await loop.run_in_executor(executor, lambda: json.dumps(users_data, indent=4).encode('utf-8'))
        fio = io.BytesIO(json_bytes)
    except Exception as e:
        logger.error(f"JSON serialization error: {e}", exc_info=True)
        await safe_tg_call(msg.edit_text("❌ Data ko JSON format mein badalte waqt error।"))
        return

    await safe_tg_call(msg.edit_text(f"✅ {len(users_data):,} users ka data taiyar ho gaya hai. Ab file bheji ja rahi hai..."))
    
    # File ko Telegram Document ke roop mein bhejein
    input_file = BufferedInputFile(fio.read(), filename=file_name)
    
    send_result = await safe_tg_call(
        bot.send_document(
            chat_id=message.chat.id,
            document=input_file,
            caption=f"✅ User Export: {len(users_data):,} records\nDate: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}"
        ),
        semaphore=TELEGRAM_COPY_SEMAPHORE # Use a general semaphore
    )
    
    if send_result:
        # Puraane message ko delete kar dein ya update
        await safe_tg_call(bot.delete_message(message.chat.id, msg.message_id))
        logger.info(f"Admin {message.from_user.id} ko user export file bhej di gayi।")
    else:
        await safe_tg_call(msg.edit_text("❌ File bhejte waqt Telegram API error. Kripya logs check karein।"))

# =======================================================
# +++++ FEATURE 2: ADMIN BAN/UNBAN USER (NEW) +++++
# =======================================================
...
async def ban_user_command(message: types.Message, db_primary: Database):
    ...
    # 2. Command argument se ID nikalo
    if user_to_ban_id is None:
        args = message.text.split(maxsplit=2)
        if len(args) < 2 or not args[1].isdigit():
            await safe_tg_call(message.answer("❌ Istemal: /ban_user `USER_ID` [Reason] ya message ko reply karein।")); return
        user_to_ban_id = int(args[1])
        reason = args[2].strip() if len(args) > 2 else ""

    if user_to_ban_id == ADMIN_USER_ID:
        await safe_tg_call(message.answer("❌ Aap khud ko ban nahi kar sakte।")); return
    
    is_banned = await safe_db_call(db_primary.is_user_banned(user_to_ban_id), default=True)
    
    if is_banned:
        await safe_tg_call(message.answer(f"ℹ️ User <code>{user_to_ban_id}</code> pehle se hi banned hai।")); return

    ban_success = await safe_db_call(db_primary.ban_user(user_to_ban_id, message.from_user.id, reason), default=False)
    
    if ban_success:
        await safe_tg_call(message.answer(f"✅ User <code>{user_to_ban_id}</code> ko permanently ban kar diya gaya। Reason: {reason or 'N/A'}"));
        logger.warning(f"User {user_to_ban_id} banned by admin {message.from_user.id}")
    else:
        await safe_tg_call(message.answer("❌ Ban karte waqt database error aa gaya।"));
...
async def unban_user_command(message: types.Message, db_primary: Database):
    ...
    if user_to_unban_id is None:
        args = message.text.split(maxsplit=1)
        if len(args) < 2 or not args[1].isdigit():
            await safe_tg_call(message.answer("❌ Istemal: /unban_user `USER_ID` ya message ko reply karein।")); return
        user_to_unban_id = int(args[1])
    
    is_banned = await safe_db_call(db_primary.is_user_banned(user_to_unban_id), default=False)

    if not is_banned:
        await safe_tg_call(message.answer(f"ℹ️ User <code>{user_to_unban_id}</code> already banned nahi hai।")); return

    unban_success = await safe_db_call(db_primary.unban_user(user_to_unban_id), default=False)
    
    if unban_success:
        await safe_tg_call(message.answer(f"✅ User <code>{user_to_unban_id}</code> ko unban kar diya gaya। Ab woh bot istemal kar sakta hai।"));
        logger.warning(f"User {user_to_unban_id} unbanned by admin {message.from_user.id}")
    else:
        await safe_tg_call(message.answer("❌ Unban karte waqt database error aa gaya।"));


# =======================================================
# +++++ FEATURE 2: BAN CHECK MIDDLEWARE (NEW) +++++
# =======================================================

class BannedUserMiddleware(BaseMiddleware):
    def __init__(self, db_primary: Database, bot: Bot):
        super().__init__()
    ...
    async def __call__(
        ...
        # Message aur Callback Query dono ko handle karein
        ...
        if not user or user.id == self.admin_id:
            # Admin ya non-user updates ko skip karein
            return await handler(event, data)
        
        user_id = user.id
        
        # Database call for ban check
        is_banned = await safe_db_call(self.db_primary.is_user_banned(user_id), default=False)
        
        if is_banned:
            logger.warning(f"Banned user {user_id} tried to access the bot.")
            
            # User ko inform karein (sirf ek baar)
            error_message = "❌ Aapko is bot ka istemal karne se permanently ban kar diya gaya hai। Kripya Admin se sampark karein।"
            
            # Rate limit ko halke mein rakhte hue sirf message aur callback ke liye response karein
            if event.message:
                await safe_tg_call(
                    self.bot.send_message(user_id, error_message, disable_notification=True),
                    semaphore=TELEGRAM_COPY_SEMAPHORE
                )
            elif event.callback_query:
                await safe_tg_call(event.callback_query.answer(error_message, show_alert=True))
                
            # Request ko aage process hone se rokein
            return
        
        # Agar user banned nahi hai, toh aage badho
        return await handler(event, data)
...
# --- MIDDLEWARE REGISTRATION (Feature 2) ---
# Global level par check hona chahiye
banned_user_middleware = BannedUserMiddleware(db_primary=db_primary, bot=bot)
