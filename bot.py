...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Hybrid (Super-Score: Sequence > Fuzzy)")
...
def clean_text_for_search(text: str) -> str:
    ...
    text = re.sub(r"[^a-z0-9]+", "", text) # Space bhi hata do for Sequence check (Kantara vs Katra)
    return text
        ...
        try:
            ...
            if movies_list:
                for movie_dict in movies_list:
                    # Use 'clean_title' specifically for search, remove spaces for sequence check
                    orig_clean = movie_dict.get('clean_title', '')
                    if orig_clean:
                         # Store both tight version (for sequence) and spaced version (for fuzzy)
                         # Lekin memory bachane ke liye hum runtime pe process karenge
...
#
# ==================================================
# +++++ FIX: SUPER-SCORE SEQUENCE LOGIC ++++++++++++
# ==================================================
#
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Ultimate Search Logic (Fix for 'Katra' -> 'Kantara'):
    1. Clean inputs strictly (remove spaces).
    2. Iterate ALL titles.
    3. SCORING:
       - If Sequence Match (is_subsequence): Base Score = 200.
       - If NOT Sequence Match: Base Score = RapidFuzz Ratio (Max 100).
       - This guarantees Sequence Match >>> Fuzzy Match.
       - Penalty: Length difference (keeps 'Kantara' above 'Khatron Ke Khiladi').
    """
    if not fuzzy_movie_cache:
        return []

    try:
        # 1. Query Clean (Remove spaces for sequence check)
        q_tight = clean_text_for_search(query) # "katra"
        q_fuzzy = clean_text_for_fuzzy(query)  # "katra" (agar spaces hote toh rehte)
        ...
        # List of (title, data)
        # Optimization: Dictionary iteration is fast
        for title_key, data in fuzzy_movie_cache.items():
            # title_key is already cleaned/lowercased from DB, but might have spaces
            ...
            # --- A. SEQUENCE CHECK (The King) ---
            # Katra (5 chars) in Kantara (7 chars) -> True
            if is_subsequence(q_tight, t_tight):
                # BASE: 200 (Guaranteed winner over any pure fuzzy match)
                score = 200 
                
                # Bonus: Starts with same letter? (Almost always true for seq, but good practice)
                if t_tight.startswith(q_tight[0]):
                    score += 50
                
                # PENALTY: Length Difference
                # We want the "tightest" match.
                # Kantara (7) - Katra (5) = 2. Score = 250 - 2 = 248.
                # K...a...t...r...a... (Long) - Katra (5) = 20. Score = 230.
                # Result: Kantara wins.
                diff = len(t_tight) - len(q_tight)
                score -= diff # Simple subtraction penalty
                match_type = "sequence"
                
            else:
                # --- B. FUZZY FALLBACK ---
                # Only if strict sequence fails
                # Katra -> Kesari (No seq). Fuzzy check.
                fuz_score = fuzz.WRatio(q_fuzzy, t_fuzzy)
                if fuz_score > 50:
                    score = fuz_score # Max 100
                else:
...
async def health_check():
    ...
    return {
        ...
        "search_logic": "Hybrid (Super-Score Sequence > Fuzzy)",
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        search_status = f"⚡️ Hybrid (Super-Score Sequence > Fuzzy)"
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- NAYA HYBRID SEARCH (Sequence > Fuzzy) ---
    
    logger.info(f"User {user_id} searching for '{clean_query}'")
    
    # 1. Run Python Search (Sequence + Fuzzy) - Isme ab Super Score hai
    loop = asyncio.get_running_loop()
    ...
    # Priority Sort: Use the new 'score' key which is now Super-Scored
