...
from functools import wraps, lru_cache  # <--- LRU CACHE ADDED HERE
...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Optimized Hybrid (LRU Cache + Lazy Fuzzy + Length Filter)")
...
#
# ==================================================
# +++++ OPTIMIZED SEARCH & CACHE LOGIC +++++++++++++
# ==================================================
#

# --- 1. OPTIMIZED CACHE LOADING (Pre-processing) ---
                    ...
                    if orig_clean:
                         if orig_clean not in temp_cache:
                             ...
                             temp_cache[orig_clean] = {
                                 'data': movie_dict,
                                 # Pre-cleaning for Sequence Match (Space removal)
                                 't_tight': orig_clean.replace(" ", ""), 
                                 # Length pehle se calculate karke rakh lo
                                 't_len': len(orig_clean.replace(" ", "")) 
                             }
...
# --- 2. OPTIMIZED SEARCH FUNCTION (LRU CACHE + LENGTH FILTER + LAZY FUZZY) ---

@lru_cache(maxsize=2000) # <-- RESULT CACHING
def cached_search_logic(query: str):
    """
    Ye function result ko cache karega. 
    Agar same search dobara aayi to ye run nahi hoga, seedha RAM se result dega.
    """
    ...
    # --- Upgrade: Length Filter Range ---
    # Query 'Katra' (5) -> Check 3 to 15. Skip 'DDLJ' (20+).
    ...
    for title_key, item in fuzzy_movie_cache.items():
        ...
        # --- Upgrade: Lazy Fuzzy ---
        # High confidence (Score > 60) par Fuzzy skip.
        if seq_score > 60:
            ...
            continue # Next item pe jao, heavy fuzzy skip karo

        # --- STEP 3: FUZZY FALLBACK (Only if Sequence Failed) ---
        fuz_score = fuzz.WRatio(query, title_key) # Raw query vs Raw key (with spaces)
        if fuz_score > 65:
            candidates.append({
                'imdb_id': item['data']['imdb_id'],
                'title': item['data']['title'],
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Hybrid Search: Optimized Wrapper
    """
    try:
        # Wrapper ab cached logic ko call karega
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        search_status = f"⚡️ Optimized Hybrid (LRU + Lazy Fuzzy)"
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # --- NAYA HYBRID SEARCH (Cached Logic Call) ---
    
    logger.info(f"User {user_id} searching for '{clean_query}'")
    
    loop = asyncio.get_running_loop()
    # Executor ki zaroorat nahi hai agar LRU cache use kar rahe hain, par safe side ke liye rakh sakte hain
        ...
        async with FUZZY_CACHE_LOCK:
            if clean_title_val not in fuzzy_movie_cache:
                fuzzy_movie_cache[clean_title_val] = {
                    "data": {"imdb_id": imdb_id, "title": title, "year": year},
...
async def check_db_command(message: types.Message, db_primary: Database, db_neon: NeonDB):
    ...
    if fuzzy_movie_cache:
        ...
        try:
            ...
            fuzzy_cache_check = {"title": sample['data'].get('title'), "clean_title": first_key}
