...
# --- NEW IMPORTS ---
from core_utils import safe_tg_call, safe_db_call, DB_SEMAPHORE, TELEGRAM_DELETE_SEMAPHORE, TELEGRAM_COPY_SEMAPHORE, TELEGRAM_BROADCAST_SEMAPHORE, WEBHOOK_SEMAPHORE, TG_OP_TIMEOUT, DB_OP_TIMEOUT # FIX: Core utilities yahan se import honge
...
async def check_user_membership(user_id: int, current_bot: Bot) -> bool:
    ...
    def normalize_chat_id(identifier):
        ...
        # Agar username hai, toh @ sign hata kar string return karein
        return f"@{identifier}" if identifier.startswith('@') else f"@{identifier}" if identifier else None
...
def get_join_keyboard() -> InlineKeyboardMarkup | None:
    ...
    def is_numeric_id(identifier):
        return identifier and (identifier.startswith('-') and identifier[1:].isdigit() or identifier.isdigit())
...
async def no_url_join_callback(callback: types.CallbackQuery):
    await safe_tg_call(callback.answer("⚠️ Yeh ek Private Chat hai. Kripya Admin se link maang kar manually join karein, aur phir 'Verify' button dabayen।", show_alert=True))
