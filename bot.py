...
from functools import wraps, lru_cache # --- [ADDED] lru_cache import ---
...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Old Sequence (Accurate) + CPU Optimized (Cache)")
...
def clean_text_for_fuzzy(text: str) -> str:
    """Fuzzy ke liye basic cleaner (spaces rakhta hai)"""
    if not text: return ""
    text = text.lower()
...
#
# =======================================================
# +++++ [UPDATED] FIXED SEARCH LOGIC (SEQUENCE) +++++
# =======================================================
#

# --- 1. STRICT CLEANING FOR SEQUENCE ---
def clean_text_for_search(text: str) -> str:
    """
    Strict cleaning: Sab kuch lowercase, aur sirf a-z 0-9.
    Spaces bhi hata do taaki 'Katra' matches 'Kantara'.
    """
    if not text: return ""
    return re.sub(r"[^a-z0-9]", "", text.lower())

# --- 2. OPTIMIZED CACHE LOADING ---
async def load_fuzzy_cache(db: Database):
    """
    Optimized Loading: Pehle se hi Clean Title aur Length calculate
    karke RAM mein rakhta hai, taaki search fast ho.
    """
    global fuzzy_movie_cache
    async with FUZZY_CACHE_LOCK:
        logger.info("🔄 Loading Search Cache (Strict Sequence Optimized)...")
        try:
            movies_list = await safe_db_call(db.get_all_movies_for_fuzzy_cache(), timeout=300, default=[])
            temp_cache = {}
            if movies_list:
                for movie in movies_list:
                    raw_clean_title = movie.get('clean_title', '')
                    if not raw_clean_title: continue
                    
                    # Strict clean (no spaces) for sequence check
                    strict_clean = clean_text_for_search(raw_clean_title)
                    
                    if strict_clean not in temp_cache:
                        temp_cache[strict_clean] = {
                            'data': movie,
                            'clean': strict_clean,
                            'len': len(strict_clean) # Length pre-calculated
                        }
                
                fuzzy_movie_cache = temp_cache
                # Cache update hone par function ka cache clear karein
                cached_search_logic.cache_clear()
                logger.info(f"✅ Cache Loaded: {len(fuzzy_movie_cache):,} items.")
            else:
                fuzzy_movie_cache = {}
        except Exception as e:
...
# --- 3. THE OLD SEQUENCE MATCHER (ACCURATE) ---
def get_sequence_score(query: str, target: str) -> int:
    """
    Checks if chars of query appear in target IN ORDER.
    'katra' in 'kantara' -> Matches k, a, t, r, a in order -> High Score.
    'katra' in 'maaran' -> Fails (k not found) -> Score 0.
    """
    q_len = len(query)
    t_len = len(target)
    ...
    # Check if all chars exist in order
    idx_t = 0
    found_count = 0
    
    for char in query:
        idx_t = target.find(char, idx_t)
        if idx_t == -1:
            return 0 # Sequence toot gaya, match fail
        idx_t += 1
        found_count += 1
        
    # Agar yahan aaye, matlab poora sequence match ho gaya!
    # Score logic: 200 base + tightness bonus
    # Target jitna chhota, utna achha match
    
    base_score = 200
    length_diff = t_len - q_len
    penalty = length_diff * 2 # Har extra letter ka penalty
    
    final_score = max(100, base_score - penalty)
    return final_score

# --- 4. SEARCH WRAPPER (LRU CACHE + LENGTH FILTER) ---
@lru_cache(maxsize=2000) # Saves result of last 2000 queries
def cached_search_logic(query: str):
    ...
    # Length Filter: Don't compare 'Katra' (5) with 'Dilwale...' (20)
    min_len = max(1, int(q_len * 0.5))
    max_len = int(q_len * 4)

    for _, item in fuzzy_movie_cache.items():
        t_clean = item['clean']
        t_len = item['len']

        # 1. Length Filter (CPU Saver)
        if t_len < min_len or t_len > max_len:
            continue

        # 2. Sequence Match (Primary Strategy)
        seq_score = get_sequence_score(q_clean, t_clean)
        ...
        if seq_score > 0:
            ...
            # Agar Sequence mil gaya, to Fuzzy mat chalao (Save CPU)
            continue 

        # 3. Fuzzy Fallback (Secondary Strategy)
        # Sirf tab chalao jab spelling mistake ho (e.g. 'Avngers')
        fuzz_score = fuzz.ratio(q_clean, t_clean)
        if fuzz_score > 70: 
             candidates.append({
                'imdb_id': item['data']['imdb_id'],
                'title': item['data']['title'],
...
async def health_check():
    ...
    return {
        ...
        "search_logic": "Optimized Sequence (Strict)",
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        search_status = f"⚡️ Strict Sequence (Optimized)"
...
async def search_movie_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    for movie in final_results[:max_buttons]:
        ...
        score_icon = ""
        if 'score' in movie and movie['score'] > 100:
            score_icon = "⚡️" # Strict Sequence match
...
async def migration_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # Clean with strict function for cache consistency
    ...
    # --- [UPDATED] Cache Logic for Strict Structure ---
    if db1_res is True:
        async with FUZZY_CACHE_LOCK:
            if clean_title_val not in fuzzy_movie_cache:
                fuzzy_movie_cache[clean_title_val] = {
                    ...
                    "clean": clean_title_val,
                    "len": len(clean_title_val)
                }
...
async def search_switch_command(message: types.Message):
    await safe_tg_call(message.answer("ℹ️ Search switch ki ab zaroorat nahi hai.\nBot ab hamesha <b>Strict Sequence Optimized</b> ka istemal karta hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def check_db_command(message: types.Message, db_primary: Database, db_neon: NeonDB):
    ...
    # --- [UPDATED] Cache Check for Strict Structure ---
    ...
    reply_text = (
        ...
        f"<b>Cache: Python (Strict Sequence)</b>\n"
