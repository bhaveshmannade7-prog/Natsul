...
# --- Typesense Removed, SecondaryDB Added ---
...
def clean_text_for_search(text: str) -> str:
    ...
    UPDATED: Handles Web Series Season/Episode conversion.
    """
    if not text: return ""
    text = text.lower()
    
    # --- Web Series Logic (New) ---
    # Convert "Season 1" or "Season 01" or "S 1" to "s01"
    text = re.sub(r'\b(season|s)\s*(\d+)\b', lambda m: f"s{m.group(2).zfill(2)}", text)
    # Convert "Episode 1" or "E 1" to "e01"
    text = re.sub(r'\b(episode|ep|e)\s*(\d+)\b', lambda m: f"e{m.group(2).zfill(2)}", text)

    text = re.sub(r"[^a-z0-9\s]+", " ", text)
    # Remove standalone 's' or 'season' if followed by numbers (already handled above, but cleanup residue)
...
CURRENT_SEARCH_MODE_INDEX = 0 # Default is Secondary
...
async def shutdown_procedure(loop):
    ...
    try:
        ...
        if secondary_db:
            secondary_db.close()
            logger.info("Secondary DB connection closed.")
    except Exception as e:
...
async def lifespan(app: FastAPI):
    ...
    if secondary_db:
        try:
            ...
            logger.error(f"Secondary DB Init Failed: {e}")
...
async def health_check():
    ...
    sec_check_task = secondary_db.is_ready() if secondary_db else asyncio.sleep(0, result=False)
    ...
    if CURRENT_SEARCH_MODE == "secondary" and not sec_ok:
        logger.warning("Health: Search mode is 'secondary' but it is NOT ready.")
        status_msg = "degraded_secondary"
    elif CURRENT_SEARCH_MODE == "neondb" and not neondb_ok:
        logger.warning("Health: Search mode is 'neondb' but NeonDB is NOT ready.")
...
async def start_command(message: types.Message):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        sec_ready_task = secondary_db.is_ready() if secondary_db else asyncio.sleep(0, result=False)
    ...
    if is_member: # <--- LOGIC BRANCH 1
        welcome_text = (
            ...
            f"Search any movie or web series by typing its name.\n"
            f"Example: <code>Mirzapur Season 2</code> or <code>Stree 2</code>\n\n"
            f"⚡️ Fast & Free."
        )
...
async def search_movie_handler(message: types.Message):
    ...
    # --- NEW: Advanced Web Series Cleaning ---
    # Converts "Season 1" -> "s01" automatically
    ...
    searching_msg = await safe_tg_call(message.answer(f"🔍 <b>{original_query}</b> dhoond raha hu..."))
    ...
    if preferred_mode == "secondary":
        if secondary_db and await secondary_db.is_ready():
            search_results = await secondary_db.search_movies(query, limit=20)
            search_engine_used = "Secondary DB"
    ...
    if not search_results:
        ...
        # Try Secondary (if not preferred)
        if preferred_mode != "secondary" and secondary_db and await secondary_db.is_ready():
            search_results = await secondary_db.search_movies(query, limit=20)
            search_engine_used = "Secondary (Failover)"
        
        # Try NeonDB (if not preferred and Secondary fail)
    ...
    if not search_results:
        await safe_tg_call(searching_msg.edit_text(
            ...
            f"2. Year hata kar search karein.\n"
            f"3. 'Season 1' ko 'S01' likh kar dekhein."
        ))
        return
    ...
    await safe_tg_call(searching_msg.edit_text(
        f"✅ <b>{original_query}</b> - {result_count_text} results found ({search_engine_used}):",
...
async def get_movie_callback(callback: types.CallbackQuery):
    ...
    if not movie:
        ...
        if user.id == ADMIN_USER_ID:
            await safe_tg_call(callback.message.answer(f"ADMIN NOTE: Movie <code>{imdb_id}</code> missing in DBs."))
        return

    # --- UPDATED: COPY MODE (PRESERVE CAPTION) ---
    # Instead of "Sending", show "Checking" or just stay silent/answer callback
    # await safe_tg_call(callback.message.edit_text(f"✅ Sending <b>{movie['title']}</b>...")) # Optional: can clutter chat
    ...
    try:
        ...
        if is_valid_forward:
            logger.debug(f"Attempting copy_message for {imdb_id} from {movie['channel_id']}:{movie['message_id']}")
            # --- CRITICAL CHANGE: Use copy_message instead of forward_message ---
            # --- DO NOT pass 'caption' argument to preserve original caption ---
            ...
            else: error_detail = "Copy failed (timeout or API error)."
        else:
        ...
        if not success:
            ...
            if not movie.get("file_id"):
                ...
                caption_txt = f"🎬 <b>{movie['title']}</b> ({movie['year'] or 'N/A'})"
...
async def migration_handler(message: types.Message):
    ...
    # --- 3. Secondary DB mein Add/Update karein ---
    sec_status = ""
    if secondary_db:
        sec_res = await secondary_db.add_movie(imdb_id, title, year, message_id, channel_id, file_id, file_unique_id, clean_title_val)
...
async def search_switch_command(message: types.Message):
    ...
    if CURRENT_SEARCH_MODE == "secondary":
        if secondary_db and await secondary_db.is_ready(): status = "🟢 (Ready)"
...
async def import_json_command(message: types.Message):
    ...
    for i, item in enumerate(mlist):
        try:
            ...
            # Prepare batch data
            batch_item_full = {
                'imdb_id': imdb, 'title': title, 'year': year, 'clean_title': clean_title_val,
                'message_id': message_id, 'channel_id': channel_id, 'file_id': fid_str, 'file_unique_id': file_unique_id
            }
            
            neon_data = (message_id, channel_id, fid_str, file_unique_id, imdb, title)

            if db_res is True: a += 1; sec_batch.append(batch_item_full); neon_batch.append(neon_data)
            elif db_res == "updated": u += 1; sec_batch.append(batch_item_full); neon_batch.append(neon_data)
    ...
    neon_stat = ""
    if neon_batch:
        # neondb.sync_from_mongo ko raw data list chahiye (updated logic in neondb.py handles this better, 
        # but here we simulate standard structure if needed or pass raw tuples if function supports)
        # Assuming neondb.sync_from_mongo expects list of dicts
        neon_sync_data = [{
            "message_id": d[0], "channel_id": d[1], "file_id": d[2], 
            "file_unique_id": d[3], "imdb_id": d[4], "title": d[5]
        } for d in neon_batch]
        
        neon_res_count = await safe_db_call(neondb.sync_from_mongo(neon_sync_data), default=0)
...
async def remove_dead_movie_command(message: types.Message):
    ...
    sec_del = False
    if secondary_db:
        # Assuming secondary_db has remove method, using standard pymongo delete
        try:
            await secondary_db.movies.delete_many({"imdb_id": imdb_id})
            sec_del = True
        except: pass
    
    sec_stat = "✅ Secondary Removed." if sec_del else "ℹ️ Secondary Not Found/Error."
...
async def auto_index_handler(message: types.Message):
    ...
    # 2. NeonDB
    neon_res = await safe_db_call(neondb.add_movie(message.message_id, message.chat.id, file_id, file_unique_id, imdb_id, title, year))
    if neon_res: logger.info(f"{log_prefix} NeonDB Synced.")
    else: logger.error(f"{log_prefix} NeonDB Sync FAILED.")

    # 3. Secondary DB
    if secondary_db:
        sec_res = await secondary_db.add_movie(imdb_id, title, year, message.message_id, message.chat.id, file_id, file_unique_id, clean_title_val)
        if sec_res: logger.info(f"{log_prefix} Secondary DB Synced.")
