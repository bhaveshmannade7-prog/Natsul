...
from database import Database, mongo_search # Mongo search ko import karein
# --- FIX: `typesense_clear_index` ko import list se hata diya ---
...
DATABASE_URL = os.getenv("DATABASE_URL") # MongoDB Atlas URL
...
# --- Search Switch (NEW) ---
# Default search 'typesense' par set karein
CURRENT_SEARCH_MODE = "typesense" 
...
if LIBRARY_CHANNEL_ID == 0:
    logger.warning("LIBRARY_CHANNEL_ID is not set. Auto-indexing will not work.")
...
async def lifespan(app: FastAPI):
    ...
    # Initialize Typesense (Best effort)
    try:
        ts_ok = await initialize_typesense() # Typesense ko initialize karein
        if ts_ok: logger.info("Typesense initialization successful.")
        else: logger.critical("TYPESENSE INITIALIZATION FAILED. Search switch /search_switch ka istemal karein.")
...
async def health_check():
    ...
    # Search mode check
    if CURRENT_SEARCH_MODE == "typesense" and not ts_check:
        logger.warning("Health: Search mode is 'typesense' but Typesense is NOT ready. Switching to 'mongodb'.")
        CURRENT_SEARCH_MODE = "mongodb"
        status_msg = "degraded_typesense_failover"
    elif not db_ok:
        status_msg = "error_db"
        status_code = 503
    ...
    return {
        ...
        "typesense_connected": ts_check,
        "current_search_mode": CURRENT_SEARCH_MODE, # Naya status
        "uptime": get_uptime(),
...
async def start_command(message: types.Message):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        ts_status = "🟢 Connected" if ts_ready else "❌ NOT CONNECTED"
        
        # Search mode status (NEW)
        search_status = "⚡️ Typesense (Fast)" if CURRENT_SEARCH_MODE == "typesense" else "🗄️ MongoDB (Fallback)"
        if CURRENT_SEARCH_MODE == "typesense" and not ts_ready:
            search_status = "⚠️ TYPESENSE FAILED (Using Fallback)"
        ...
        admin_message = (
            ...
            f"⚡️ Typesense: <b>{ts_status}</b>\n"
            f"🔍 Search Mode: <b>{search_status}</b>\n"
            f"⏰ Uptime: {get_uptime()}\n\n"
            f"<b>Commands</b>\n"
            f"/stats | /health\n"
            f"<b>/search_switch</b> (Toggle Mongo/Typesense)\n"
            f"/broadcast (Reply)\n"
            f"/import_json (Reply)\n"
...
async def check_join_callback(callback: types.CallbackQuery):
    ...
    if is_member:
        ...
        # User not in channels, send alert on callback
        ...
        # Edit message to show button again if it disappeared
...
# =======================================================
# +++++ UNIVERSAL SEARCH HANDLER (Typesense + Mongo) +++++
# =======================================================
@dp.message(F.text & ~F.text.startswith("/") & (F.chat.type == "private"))
@handler_timeout(10)
async def search_movie_handler(message: types.Message):
    global CURRENT_SEARCH_MODE # Search mode ko access karein
    user = message.from_user
    if not user: return
    user_id = user.id

    # 1. Check Capacity & Update Activity
    if not await ensure_capacity_or_inform(message): return
    ...
    # 3. Check Search Mode and Health
    search_engine_name = "Typesense (Fast)"
    search_function = typesense_search # Default function
    
    # Agar mode 'typesense' hai lekin woh ready nahi hai, toh 'mongodb' par switch karein
    if CURRENT_SEARCH_MODE == "typesense":
        if not is_typesense_ready():
            logger.error(f"User {user_id} search: Typesense not ready! Failing over to MongoDB.")
            CURRENT_SEARCH_MODE = "mongodb" # Globally switch
            search_engine_name = "MongoDB (Fallback)"
            search_function = mongo_search
        else:
            # Typesense mode hai aur ready hai, sab aam hai
            search_engine_name = "Typesense (Fast)"
            search_function = typesense_search
            
    elif CURRENT_SEARCH_MODE == "mongodb":
        # Admin ne manually MongoDB set kiya hai
        search_engine_name = "MongoDB (Slow)"
        search_function = mongo_search

    # 4. Inform User & Perform Search
    searching_msg = await safe_tg_call(message.answer(f"⚡️ <b>{original_query}</b> search ho raha hai... ({search_engine_name})"))
    if not searching_msg: return

    # Search function ko call karein (ya toh typesense_search ya mongo_search)
    search_results = await search_function(original_query, limit=20)
...
async def stats_command(message: types.Message):
    ...
    movie_count_str = f"{movie_count_raw:,}" if movie_count_raw >= 0 else "DB Error"
    ts_status = "🟢 Connected" if ts_ready else "❌ NOT CONNECTED"
    search_status = "⚡️ Typesense" if CURRENT_SEARCH_MODE == "typesense" else "🗄️ MongoDB"
    if CURRENT_SEARCH_MODE == "typesense" and not ts_ready:
        search_status = "⚠️ TYPESENSE FAILED (Using Fallback)"
...
async def search_switch_command(message: types.Message):
    ...
    if CURRENT_SEARCH_MODE == "typesense":
        CURRENT_SEARCH_MODE = "mongodb"
        new_mode_text = "🗄️ MongoDB (Fallback)"
    else:
        # Check if typesense is ready before switching
        if not is_typesense_ready():
            await safe_tg_call(message.answer("❌ Typesense is not connected. Switch failed. /start karke status check karein."))
            return
            
        CURRENT_SEARCH_MODE = "typesense"
        new_mode_text = "⚡️ Typesense (Fast)"
        
    logger.info(f"Admin changed search mode to: {CURRENT_SEARCH_MODE}")
    await safe_tg_call(message.answer(f"✅ Search mode ab <b>{new_mode_text}</b> par set hai."))
...
async def import_json_command(message: types.Message):
    ...
    for i, item in enumerate(mlist):
        try:
            ...
            # Typesense ke liye data prepare karein
    ...
    ts_stat = ""
    if ts_batch:
        ts_res = await typesense_add_batch_movies(ts_batch) # Batch function
...
async def remove_dead_movie_command(message: types.Message):
    ...
    # Typesense se bhi delete karein
...
@dp.message(Command("sync_typesense"), AdminFilter()) # Command rename
@handler_timeout(1800)
async def sync_typesense_command(message: types.Message): # Function rename
    ...
    try:
        ...
        # Typesense sync function ko call karein
...
async def auto_index_handler(message: types.Message):
    ...
    if db_res in [True, "updated"]:
        ...
        # Typesense mein bhi sync karein
        algolia_data = {'imdb_id': imdb_id, 'title': title, 'year': year, 'clean_title': clean_title_val}
        ts_ok = await typesense_add_movie(algolia_data)
