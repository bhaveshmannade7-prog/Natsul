...
from database import Database # Sirf 'Database' class ko import karein
...
# Initialize Database
try:
    db = Database(DATABASE_URL) # 'db' instance create karein
...
async def lifespan(app: FastAPI):
    ...
    try:
        ts_ok = await initialize_typesense()
...
async def search_movie_handler(message: types.Message):
    ...
    if CURRENT_SEARCH_MODE == "typesense":
        if not is_typesense_ready():
            ...
            # --- FIX: 'db' instance se 'mongo_search_internal' ko call karein ---
...
async def export_csv_command(message: types.Message):
    ...
    try:
        if kind == "users":
            ...
            rows = await safe_db_call(db.export_movies(limit=limit), timeout=9Git 90, default=[])
            if not rows: await safe_tg_call(msg.edit_text(f"❌ No {kind} found.")); return
