...
# --- NAYA REDIS IMPORT (FIXED for Python 3.11 compatibility) ---
try:
    # aioredis ko hataakar ab sirf redis ka istemal karein
    import redis.asyncio as aioredis
except ImportError:
    logging.critical("--- redis library nahi mili! ---")
...
logging.getLogger("aioredis").setLevel(logging.WARNING) # Redis log level ko control karein
...
# NOTE: In semaphores ko ab BotManager control karega
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                if target_chat_id:
                    try: await safe_tg_call(bot.send_message(target_chat_id, "⚠️ Server request time out ho gayi. Kripya dobara koshish karein."))
...
async def check_user_membership(user_id: int) -> bool:
    ...
    try:
        tasks_to_run = []
        if check_channel:
            # BotManager ke through call karein
...
async def _process_update_safe(update_obj: Update, bot_instance: Bot):
    ...
    if update_obj.callback_query:
        ...
        if update_obj.message.forward_from_chat or (update_obj.message.video or update_obj.message.document): 
             priority = 'HIGH' # File ops & Migration
        elif update_obj.message.text and not update_obj.message.text.startswith('/'): 
             priority = 'MEDIUM' # Search queries
        elif update_obj.message.text and update_obj.message.text.startswith('/'): 
             priority = 'LOW' # Commands (menus/help)
        elif update_obj.message.chat.type == 'channel': 
             priority = 'HIGH' # Channel posts (auto index)
...
async def bot_webhook(update: dict, background_tasks: BackgroundTasks, request: Request):
    ...
    try:
        ...
        # Webhook se aaya update seedha primary bot instance (bot) ke saath queue mein daal diya jaata hai.
        # Worker loop khud BotManager se available bot instance utha leta hai.
...
async def get_movie_callback(callback: types.CallbackQuery, db_primary: Database, db_fallback: Database, db_cache_wrapper: DatabaseCacheWrapper):
    ...
    # NAYA: db_cache_wrapper se call karein (Cache hit/miss check hoga)
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if not doc.file_name or (not doc.file_name.lower().endswith(".json") and doc.mime_type != "application/json"):
        ...
        return
    msg = await safe_tg_call(message.answer(f"⏳ `{doc.file_name}` download ho رہا hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    try:
...
if __name__ == "__main__":
    ...
    if not WEBHOOK_URL:
        ...
        logger.error("Run karne ke liye: gunicorn bot:app -k uvicorn.workers.UvicornWorker --workers 3 --bind 0.0.0.0:8000")
