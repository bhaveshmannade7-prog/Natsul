...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Hybrid (Mongo1-Exact + Python-Sequence-Match)")
...
#
# ==================================================
# +++++ USER'S LOGIC: SEQUENCE MATCH SEARCH +++++
# ==================================================
#
def is_subsequence(query: str, target: str) -> bool:
    """
    Aapka bataya hua Logic:
    Check karta hai ki query ke letters target mein usi order mein hain ya nahi.
    Example: 'Katra' in 'Kantara' -> True (K..a..t..r..a mil gaya)
    """
    it = iter(target)
...
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Hybrid Logic: 
    1. Pehle WRatio (Cutoff 60) se milti-julti movies nikalo.
    2. Phir 'Subsequence Check' lagao (Jo aapne bola).
    Agar Subsequence match hua, toh usko TOP pe bhej do.
    """
    ...
    try:
        ...
        # Step 1: Pehle basic fuzzy search (Cutoff 60 rakha hai jaisa aapne kaha)
        # Limit badha kar 100 kar di hai taaki 'Kantara' list mein zaroor aaye
        ...
        for (matched_title, base_score, _) in candidates:
            movie_dict = fuzzy_movie_cache[matched_title]
            final_score = base_score
            
            # --- AAPKA LOGIC: SEQUENCE CHECK ---
            # Agar user ka likha hua word usi order mein movie naam mein hai
            # Toh usko bahot bada boost do (seedha top par)
            if is_subsequence(query, matched_title):
                final_score += 40  # Massive Bonus!
            
            # Startswith Bonus (Agar movie wahi se shuru hoti hai)
        ...
        # Final Score ke hisaab se sort karo
...
async def health_check():
    ...
    return {
        ...
        "search_logic": "Hybrid (Mongo1-Exact + Python-Sequence-Match)",
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        search_status = f"⚡️ Hybrid (Mongo-Exact + Python-Sequence-Match)"
        ...
        admin_message = (
            ...
            f"<b>⚙️ Search Configuration (SEQUENCE)</b>\n"
...
async def search_switch_command(message: types.Message):
    await safe_tg_call(message.answer("ℹ️ Search switch ki ab zaroorat nahi hai.\nBot ab hamesha <b>Hybrid (Mongo-Exact + Python-Sequence-Match)</b> ka istemal karta hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
