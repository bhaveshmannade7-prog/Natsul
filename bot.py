...
try:
    ...
    logging.critical("--- aioredis library nahi mili! ---")
    logging.critical("Kripya install karein: pip install aioredis")
    raise SystemExit("Missing dependency: aioredis")
...
# --- NAYA CORE SERVICES IMPORT ---
from core_services import BotManager, RedisCache, DatabaseCacheWrapper, PriorityDispatcher, priority_wrapper
...
try:
    ...
    # --- NAYA: Redis & Priority Queue Config ---
    REDIS_URL = os.getenv("REDIS_URL") # Optional
    QUEUE_CONCURRENT_LIMIT = int(os.getenv("QUEUE_CONCURRENT_LIMIT", "200")) # Feature D
...
# --- NAYA: Global Priority Dispatcher Instance ---
priority_dispatcher: PriorityDispatcher | None = None
# --- NAYA: Global Bot Manager Instance ---
...
# NOTE: Old TG semaphores will be managed by BotManager.safe_api_call
# with the underlying bot sessions, but we keep them for logic isolation
# in case BotManager is not used.
...
try:
    # --- NAYA: BotManager Initialize Karein (Feature A) ---
    bot_manager = BotManager(BOT_TOKEN, ALTERNATE_BOTS)
    bot = bot_manager.bots[0] # Primary bot instance for aiogram setup
    ...
    # --- NAYA: Redis Cache aur Wrapper Initialize Karein (Feature B) ---
    redis_cache = RedisCache(REDIS_URL)
    db_cache_wrapper = DatabaseCacheWrapper(db_primary, db_neon, redis_cache)
    ...
    dp = Dispatcher(
        ...
        db_neon=db_neon,
        db_cache_wrapper=db_cache_wrapper # Naya wrapper pass kiya
    )
    
    # --- NAYA: PriorityDispatcher Initialize Karein (Feature D) ---
    priority_dispatcher = PriorityDispatcher(dp, QUEUE_CONCURRENT_LIMIT)
    
    logger.info("BotManager (Multi-Token), Dispatcher, Database objects, aur PriorityDispatcher initialize ho gaye.")
...
async def shutdown_procedure():
    ...
    # --- NAYA: Priority Dispatcher Band Karein ---
    if priority_dispatcher:
        await priority_dispatcher.stop_workers()
    ...
    if WEBHOOK_URL:
        try:
            # Shutdown ke liye primary bot ka istemal karein
            await bot_manager.bots[0].delete_webhook(drop_pending_updates=True) 
    ...
    # --- NAYA: BotManager Sessions Close Karein ---
    if bot_manager:
    ...
    # --- NAYA: Redis Connection Close Karein ---
    if redis_cache:
...
def handler_timeout(timeout: int = HANDLER_TIMEOUT):
    def decorator(func: Callable):
        @wraps(func)
        @priority_wrapper(getattr(func, '__handler_priority__', 'LOW')) # NAYA: Priority maintain karein
        async def wrapper(*args, **kwargs):
            try:
                # Naya structure mein timeout abhi bhi zaroori hai
                ...
                if target_chat_id:
                    # TG call mein ab bot_manager ka istemal karein
                    try: await bot_manager.safe_api_call(bot.send_message(target_chat_id, "⚠️ Server request time out ho gayi. Kripya dobara koshish karein."))
...
async def safe_db_call(coro, timeout=DB_OP_TIMEOUT, default=None):
    # ... (No change in DB_SEMAPHORE logic)
    try:
...
# NOTE: Original safe_tg_call ko hata diya gaya hai. Ab sabhi TG calls 
# bot_manager.safe_api_call ke through jayenge.
async def safe_tg_call(coro, timeout=TG_OP_TIMEOUT, semaphore: asyncio.Semaphore | None = None):
    """
    Ab yeh helper function BotManager ko delegate karega.
    """
    if bot_manager is None: 
         logger.critical("BotManager not initialized."); return None
         
    return await bot_manager.safe_api_call(coro, timeout=timeout, semaphore=semaphore)
...
async def check_user_membership(user_id: int) -> bool:
    ...
    try:
        tasks_to_run = []
        # NOTE: bot.get_chat_member ko ab BotManager se use karna chahiye.
        # Lekin yahaan seedha call karne par bhi bot_manager.safe_api_call usse handle kar lega.
        if check_channel:
            tasks_to_run.append(bot.get_chat_member(chat_id=f"@{JOIN_CHANNEL_USERNAME}", user_id=user_id))
        if check_group:
            tasks_to_run.append(bot.get_chat_member(chat_id=f"@{USER_GROUP_USERNAME}", user_id=user_id))
        
        # NOTE: safe_tg_call ka istemal yahaan isliye nahi kiya jaa raha kyunki 
        # gather mein multiple coroutines ko wrap karna mushkil hai.
        # Hum seedha BotManager ka istemal karenge.
        
        # BotManager se call karein
        results = await asyncio.gather(
            *[safe_tg_call(task, timeout=5) for task in tasks_to_run]
        )
...
async def lifespan(app: FastAPI):
    ...
    # ... (ThreadPoolExecutor init - No change)
    ...
    # --- 3 DB Init ---
    # ... (DB initialization remains unchanged, using original objects)
    try:
    ...
    # --- NAYA: Redis Connect Karein ---
    await redis_cache.connect() 
    
    # --- NAYA: PriorityDispatcher Start Karein ---
    priority_dispatcher.start_workers() 
    
    # --- NAYA: Fuzzy Cache Load Karein ---
    # Fuzzy cache ab db_primary se load hoga, isliye db_cache_wrapper ka istemal nahi.
    ...
    if WEBHOOK_URL:
        try:
            ...
            primary_bot_instance = bot_manager.bots[0] 
            current_webhook = await primary_bot_instance.get_webhook_info()
            if not current_webhook or current_webhook.url != WEBHOOK_URL:
                 logger.info(f"Webhook set kiya ja raha hai: {WEBHOOK_URL}...")
                 await primary_bot_instance.set_webhook(
                     url=WEBHOOK_URL,
...
async def _process_update_safe(update_obj: Update, bot_instance: Bot):
    """
    Yeh function ab PriorityDispatcher ko delegate karega.
    """
    if priority_dispatcher is None:
        logger.critical("PriorityDispatcher is not initialized.")
        return

    priority = 'LOW' # Default priority
    
    if update_obj.callback_query:
        # File retrieval HIGH priority
        if update_obj.callback_query.data.startswith("get_"):
            priority = 'HIGH' 
        # Menu/Join checks LOW priority
        elif update_obj.callback_query.data in ["check_join"]: 
            priority = 'LOW'
            
    elif update_obj.message:
        # File forwarding/media is HIGH
        if update_obj.message.forward_from_chat: 
             priority = 'HIGH'
        # Search queries are MEDIUM
        elif update_obj.message.text and not update_obj.message.text.startswith('/'): 
             priority = 'MEDIUM'
        # Commands (menus/help) are LOW
        elif update_obj.message.text and update_obj.message.text.startswith('/'): 
             priority = 'LOW'
        # Channel posts (auto index) are HIGH
        elif update_obj.message.chat.type == 'channel': 
             priority = 'HIGH'

    # Original dependencies ko context mein daalein
    context = {
        'db_primary': db_primary, 
        'db_fallback': db_fallback, 
        'db_neon': db_neon,
        'db_cache_wrapper': db_cache_wrapper
    }

    # Dispatch to the priority queue
    await priority_dispatcher.dispatch(update_obj, bot_instance, priority, **context)
...
async def bot_webhook(update: dict, background_tasks: BackgroundTasks, request: Request):
    ...
    try:
        ...
        # --- NAYA: BotManager se next available bot instance chunein ---
        # NOTE: Agar telegram update mein koi token mention nahi hai, toh hum primary bot instance use karenge.
        # Lekin BotManager ke worker loop mein Bot instance ko update kiya jayega.
        # Yahaan hum sirf primary bot ka istemal karke update ko queue mein daal dete hain.
...
async def health_check():
    ...
    return {
        ...
        "redis_caching_enabled": redis_cache.is_enabled if redis_cache else False,
        "search_logic": "Hybrid (Smart Sequence > Fuzzy)",
        "fuzzy_cache_size": len(fuzzy_movie_cache),
        "priority_queue_limit": QUEUE_CONCURRENT_LIMIT,
        "uptime": get_uptime(),
...
async def ensure_capacity_or_inform(
    ...
    db_primary: Database # Yeh abhi bhi db_primary hai (user count Mongo 1 mein hai)
) -> bool:
    ...
    # User activity update ka call db_cache_wrapper se nahi ho sakta
    ...
    if active >= CURRENT_CONC_LIMIT:
        ...
        if isinstance(message_or_callback, types.CallbackQuery):
            await safe_tg_call(callback_query.answer("Server busy, kripya thodi der baad try karein.", show_alert=True))
        return False
...
@handler_timeout(15)
@priority_wrapper('LOW') # Menus are LOW priority
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB, db_cache_wrapper: DatabaseCacheWrapper):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        # NAYA: Cached count ka istemal karein
        mongo_1_count_task = safe_db_call(db_cache_wrapper.get_movie_count('primary'), default=-1) 
        mongo_2_count_task = safe_db_call(db_fallback.get_movie_count(), default=-1)
        neon_count_task = safe_db_call(db_cache_wrapper.get_movie_count('neon'), default=-1)
        ...
        admin_message = (
            ...
            f"  - 🧠 Priority Queue Limit: {QUEUE_CONCURRENT_LIMIT}\n\n" # NAYA
            ...
            f"  - {status_icon(redis_cache.is_enabled)} Redis Cache: {'Enabled' if redis_cache.is_enabled else 'Disabled'}\n\n" # NAYA
...
@handler_timeout(20)
@priority_wrapper('HIGH') # File transfer is HIGH priority
async def get_movie_callback(callback: types.CallbackQuery, db_primary: Database, db_fallback: Database, db_cache_wrapper: DatabaseCacheWrapper):
    user = callback.from_user
    if not user: 
    ...
    # NAYA: db_cache_wrapper se call karein
    movie = await safe_db_call(db_cache_wrapper.get_movie_by_imdb(imdb_id), timeout=DB_OP_TIMEOUT) 
    
    if not movie:
        logger.warning(f"Movie {imdb_id} not found in cache/db_primary, checking db_fallback...")
        # Fallback check mein abhi bhi original db_fallback ka istemal karein
    ...
    # ... (Rest of the file copying/sending logic remains unchanged, 
    # using safe_tg_call which is now routed through BotManager)
    ...
    try:
        ...
        if is_valid_for_copy:
            # bot.copy_message ka istemal karein (BotManager use karega)
        ...
        if not success:
            ...
            if not movie.get("file_id"):
                ...
                # bot.send_document ka istemal karein (BotManager use karega)
    ...
    if success:
        # Cache se remove karein (Agar future mein movie remove ho gayi ho)
        await redis_cache.delete(f"movie:imdb:{imdb_id}")
...
@handler_timeout(20)
@priority_wrapper('HIGH') # Migration/Indexing is HIGH priority
async def migration_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # NAYA: Cache invalidate karein
    await redis_cache.delete(f"movie:imdb:{imdb_id}")
    await redis_cache.delete("count:movies:primary")
    await redis_cache.delete("count:movies:neon")
...
@dp.channel_post()
@handler_timeout(20)
@priority_wrapper('HIGH') # Auto Indexing is HIGH priority
async def auto_index_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # ... (Rest of auto-index logic remains unchanged)
    info = extract_movie_info(message.caption or "") 
    if not info or not info.get("title"):
...
@handler_timeout(15)
@priority_wrapper('LOW') # Admin stats are LOW priority
async def stats_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB, db_cache_wrapper: DatabaseCacheWrapper):
...
@priority_wrapper('LOW') # Broadcast is LOW priority (to save HIGH/MEDIUM for user search)
async def broadcast_command(message: types.Message, db_primary: Database):
    ...
    async def send_to_user(user_id: int):
        ...
        # NOTE: bot.copy_to ka istemal karein (BotManager use karega)
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    # ... (Import logic remains largely unchanged)
    ...
    for i, item in enumerate(mlist):
        try:
            ...
            # NAYA: Cache invalidation (batch ke andar)
            await redis_cache.delete(f"movie:imdb:{imdb}")
    ...
    # NAYA: Count cache invalidate karein
    await redis_cache.delete("count:movies:primary")
    await redis_cache.delete("count:movies:neon")
...
async def remove_dead_movie_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # ... (Fuzzy cache update logic remains unchanged)
    if db1_del:
        async with FUZZY_CACHE_LOCK:
...
async def cleanup_mongo_1_command(message: types.Message, db_primary: Database):
    ...
    if deleted_count > 0:
        ...
        await redis_cache.delete("count:movies:primary") # NAYA: Count cache invalidate
    else:
...
async def cleanup_mongo_2_command(message: types.Message, db_fallback: Database):
    ...
    if deleted_count > 0:
        ...
        await redis_cache.delete("count:movies:fallback") # NAYA: Count cache invalidate
    else:
...
async def remove_library_duplicates_command(message: types.Message, db_neon: NeonDB):
    ...
    # ... (Rest of duplicate removal logic remains unchanged)
    ...
    async def delete_message(msg_id: int, chat_id: int):
        ...
        # NOTE: bot.delete_message ka istemal karein (BotManager use karega)
...
async def backup_channel_command(message: types.Message, db_neon: NeonDB):
    ...
    async def copy_file(msg_id: int, chat_id: int):
        ...
        # NOTE: bot.copy_message ka istemal karein (BotManager use karega)
...
async def sync_mongo_1_to_2_command(message: types.Message, db_primary: Database, db_fallback: Database):
    ...
    # ... (Bulk upsert logic remains unchanged)
    processed_count = 0
    tasks = []
    ...
    # NAYA: Count cache invalidate karein
    await redis_cache.delete("count:movies:fallback") 
...
async def force_rebuild_m1_command(message: types.Message, db_primary: Database):
    ...
    await safe_db_call(db_primary.force_rebuild_text_index()) # NAYA: Force text index rebuild
...
async def errors_handler(update: types.Update, exception: Exception):
    ...
    if target_chat_id:
        try: 
            # NOTE: bot.send_message ka istemal karein (BotManager use karega)
...
async def main_polling():
    ...
    try:
        ...
        await db_fallback.init_db()
        await db_neon.init_db()
        await redis_cache.connect() # NAYA: Redis connect karein
    ...
    primary_bot_instance = bot_manager.bots[0] # NAYA: Primary bot instance use karein
    await primary_bot_instance.delete_webhook(drop_pending_updates=True)
    global monitor_task
    monitor_task = asyncio.create_task(monitor_event_loop())
    setup_signal_handlers()

    try:
        # NOTE: Local polling mein PriorityDispatcher ka istemal nahi ho raha hai.
        # Sabhi updates seedha feed hote hain.
        await dp.start_polling(
            primary_bot_instance, # NAYA: Primary bot instance use karein
            allowed_updates=dp.resolve_used_update_types(),
            db_primary=db_primary,
            db_fallback=db_fallback,
            db_neon=db_neon,
