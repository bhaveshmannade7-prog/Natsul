...
# Helper functions (purane 'bot (2).py' se)
def clean_text_for_search(text: str) -> str:
    """Cleans text for search indexing."""
    if not text: return ""
    text = text.lower()
    # Punctuation hatayein
    text = re.sub(r"[^a-z0-9\s]+", " ", text)
    # Season numbers hatayein (search ke liye)
...
def extract_movie_info(caption: str | None) -> Dict[str, str] | None:
    ...
    # Series ke liye Season line jodein
...
def parse_filename(filename: str) -> Dict[str, str | None]:
    ...
    # . aur _ ko space se replace karein
    title = re.sub(r'[._]', ' ', title).strip()
    # Extra space hatayein
    title = re.sub(r"\s+", " ", title).strip()
    
    # Agar title poora clean ho gaya, toh fallback
    if not title:
        title = os.path.splitext(filename)[0].strip()
...
async def get_movie_callback(callback: types.CallbackQuery, db_primary: Database, db_fallback: Database):
    ...
    try:
        ...
        if is_valid_for_copy:
            # --- NAYA FIX: caption=None ---
            copy_result = await safe_tg_call(
                bot.copy_message(
                    chat_id=user.id,
                    from_chat_id=int(movie["channel_id"]),
                    message_id=movie["message_id"],
                    caption=None # Caption ko override karke None set karein
                ), 
        ...
        if not success:
            ...
            if not movie.get("file_id"):
                ...
                # --- NAYA FIX: Yahaan se bhi caption hata diya ---
                send_result = await safe_tg_call(bot.send_document(
                    chat_id=user.id,
                    document=movie["file_id"],
                    caption=None # Caption None set karein
                ), 
...
async def migration_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    info = extract_movie_info(message.caption or "") # <-- FIXED
    ...
    # 'clean_title_val' ko DB calls mein pass karein (8 arguments)
    ...
    # NeonDB ko clean title ki zaroorat nahi hai (woh fuzzy search karta hai)
...
async def auto_index_handler(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    # Tasks ko background mein run karein (auto-index ke liye response time zaroori nahi)
...
async def import_json_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    for i, item in enumerate(mlist):
        try:
            ...
            info = parse_filename(fname); # <-- FIXED
...
async def set_limit_command(message: types.Message):
    ...
    try:
        val = int(args[1]); assert 5 <= val <= 5000 # Limit badha di
