...
class BannedFilter(BaseFilter):
    async def __call__(self, message: types.Message, db_primary: Database) -> bool:
        ...
        if is_banned:
            ...
            try:
                await safe_tg_call(
                    message.answer("❌ Aapko bot istemal karne se rok diya gaya hai. Kripya admin se sampark karein।"),
...
async def ensure_capacity_or_inform(
    ...
    if active >= CURRENT_CONC_LIMIT:
        ...
        if isinstance(message_or_callback, types.CallbackQuery):
            await safe_tg_call(message_or_callback.answer("Server busy, kripya thodi देर baad try karein।", show_alert=False))
        return False
...
# FIX: BannedFilter ko /start par lagaya gaya
@dp.message(CommandStart(), BannedFilter())
async def banned_start_command_stub(message: types.Message):
    # This handler ensures the response from the BannedFilter is sent, and prevents further execution.
    pass

@dp.message(CommandStart(), AdminFilter())
@handler_timeout(15)
async def admin_start_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    """Admin ke liye /start command - seedha control matrix dikhaye."""
    user = message.from_user
    if not user: return
    
    # Admin commands ko sidha dikhane ke liye /stats command ka text istemal karein
    # Isse "complicated aur advance machine" wala feel aayega
    
    # *Note*: Hum yahaan stats_command ko call nahi kar rahe hain, balki iske message format ko copy kar rahe hain 
    # taaki user ko /stats ke bajaye /start par hi woh interface dikhe.
    
    # FIX: /stats handler mein jo logic hai use yahaan duplicate/call karne ke bajaye,
    # /start command ko AdminFilter ke saath alag se define kiya gaya hai
    # aur yeh seedha Control Matrix message bhejega.
    
    # Pehle stats data fetch karein
    ...
    # FIX: Admin message content ko /stats se alag banaya gaya hai
    admin_message = (
        f"🖥️ <b>{user.first_name}</b>, Welcome to **System Control Matrix**\n\n"
        f"<b>🚀 CORE ENGINE STATUS</b>\n"
        f"  - ⚡️ Active Users ({ACTIVE_WINDOW_MINUTES}m): {concurrent_users:,} / {CURRENT_CONC_LIMIT}\n"
        f"  - ⏱️ System Uptime: {get_uptime()}\n"
        f"  - 🔄 Queue Load: {priority_queue._queue.qsize()} (Workers: {QUEUE_CONCURRENCY})\n\n"
        f"<b>💾 DATABASE INTEGRITY CHECK (M+M+N+R)</b>\n"
        f"  - {status_icon(mongo_1_ready)} MongoDB Primary (M1): {count_str(mongo_1_count)} Docs\n"
        f"  - {status_icon(mongo_2_ready)} MongoDB Fallback (M2): {count_str(mongo_2_count)} Docs\n"
        f"  - {status_icon(neon_ready)} NeonDB Index (N3): {count_str(neon_count)} Vectors\n"
        f"  - {status_icon(redis_ready)} Redis Cache (R4): {len(fuzzy_movie_cache):,} Titles\n\n"
        f"<b>⚙️ COMMAND & MANAGEMENT SUITE</b>\n"
        f"  - <b>Telemetries:</b> /stats | /health\n"
        f"  - <b>Users:</b> /get_user | /export_users | /ban | /unban\n"
        f"  - <b>Sync & Maintenance:</b> /sync_mongo_1_to_2 | /sync_mongo_1_to_neon\n"
        f"  - <b>Rebuild:</b> /rebuild_clean_titles_m1 | /force_rebuild_m1\n"
        f"  - <b>Cleanup:</b> /remove_library_duplicates | /cleanup_users\n"
        f"  - <b>Broadcast:</b> /broadcast (Reply to message)\n"
    )
...
async def start_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    ...
    # --- NAYA: Regular User (Admin filter ke baad yeh sirf non-admin ke liye chlega) ---
...
async def check_join_callback(callback: types.CallbackQuery, bot: Bot, db_primary: Database, redis_cache: RedisCacheLayer):
    ...
    # --- NAYA FEATURE 2: Ban Check in Callback ---
    is_banned = await safe_db_call(db_primary.is_user_banned(user.id), default=False)
    if is_banned:
...
async def stats_command(message: types.Message, bot: Bot, db_primary: Database, db_fallback: Database, db_neon: NeonDB, redis_cache: RedisCacheLayer):
    """Admin ke liye /stats command - detailed system telemetry dikhaye."""
    ...
    admin_message = (
        ...
        f"  - <b>/export_users</b> 📄 (Export all active users to JSON)\n" 
        f"  - <b>/ban `ID/Reply`</b> | <b>/unban `ID`</b>\n\n" 
...
# --- NAYA FEATURE 1: Export Users Command ---
@dp.message(Command("export_users"), AdminFilter())
@handler_timeout(60)
async def export_users_command(message: types.Message, db_primary: Database):
    msg = await safe_tg_call(message.answer("⏳ Active users ka data Mongo 1 se fetch kiya ja raha hai..."), semaphore=TELEGRAM_COPY_SEMAPHORE)
    if not msg: return
    
    # Use the new DB method to get detailed user list
    user_data_list = await safe_db_call(db_primary.get_all_user_details(), timeout=50, default=[])
    
    if not user_data_list:
        await safe_tg_call(msg.edit_text("❌ Database mein koi active user details nahi mili।")); return
        
    # JSON serialization (Use ThreadPoolExecutor for CPU-bound task)
    loop = asyncio.get_running_loop()
    try:
        ...
        await safe_tg_call(msg.edit_text(f"❌ Data export mein error: {e}")); return
    ...
    try:
        await safe_tg_call(
            message.answer_document(
                ...
                caption=f"✅ {len(user_data_list):,} active users ka export safal."
            ),
            semaphore=TELEGRAM_COPY_SEMAPHORE
        )
        ...
        await safe_tg_call(msg.edit_text(f"❌ File bhejte waqt error: {e}"))
# --- END NAYA FEATURE 1 ---

# --- NAYA FEATURE 2: Ban/Unban Commands ---
...
async def ban_user_command(message: types.Message, db_primary: Database):
    ...
    if target_id is None:
        await safe_tg_call(message.answer("❌ Istemal: /ban `USER_ID` ya kisi user message ko reply karein।"), semaphore=TELEGRAM_COPY_SEMAPHORE); return
    
    if target_id == ADMIN_USER_ID:
        await safe_tg_call(message.answer("❌ Admin ko ban nahi kiya ja sakta।"), semaphore=TELEGRAM_COPY_SEMAPHORE); return
        
    # Extract reason if provided after ID
    # Note: If reply is used, message.text might just be '/ban'. Need to handle splitting carefully.
    text_parts = message.text.split(maxsplit=2)
    reason = None
    if len(text_parts) > 2:
        reason = text_parts[2]
    elif len(text_parts) == 2 and not text_parts[1].isdigit():
        # Case: /ban username/text
        reason = text_parts[1]
    ...
    if banned:
        await safe_tg_call(message.answer(f"✅ User <code>{target_id}</code> ko safaltapoorvak ban kar diya gaya hai। Reason: {reason}"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    else:
        await safe_tg_call(message.answer(f"❌ User <code>{target_id}</code> ko ban karne mein error aaya।"), semaphore=TELEGRAM_COPY_SEMAPHORE)
...
async def unban_user_command(message: types.Message, db_primary: Database):
    target_id = await _get_target_user_id(message)
    if target_id is None:
        await safe_tg_call(message.answer("❌ Istemal: /unban `USER_ID` ya kisi user message ko reply karein।"), semaphore=TELEGRAM_COPY_SEMAPHORE); return

    unbanned = await safe_db_call(db_primary.unban_user(target_id))
    
    if unbanned:
        await safe_tg_call(message.answer(f"✅ User <code>{target_id}</code> ko safaltapoorvak unban kar diya gaya hai।"), semaphore=TELEGRAM_COPY_SEMAPHORE)
    else:
        await safe_tg_call(message.answer(f"❌ User <code>{target_id}</code> ban list mein nahi hai ya unban karne mein error aaya।"), semaphore=TELEGRAM_COPY_SEMAPHORE)
# --- END NAYA FEATURE 2 ---
...
async def backup_channel_command(message: types.Message, db_neon: NeonDB):
    ...
    async def copy_file(msg_id: int, chat_id: int):
        ...
        # FIX 2: Flood limit fix: Add delay and use semaphore
