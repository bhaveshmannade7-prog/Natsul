...
import json
import hashlib
from datetime import datetime, timezone # `timezone` import zaroori hai
...
async def check_join_callback(callback: types.CallbackQuery):
    ...
    if is_member:
        ...
        # Failed verification
        await safe_tg_call(callback.answer("❌ Aapne abhi tak Channel/Group join nahi kiya hai. Please join karke dobara try karein.", show_alert=True))
...
async def search_movie_handler(message: types.Message):
    ...
    if not await check_user_membership(user_id):
        join_markup = get_join_keyboard()
        if join_markup:
            await safe_tg_call(message.answer("Access ke liye, kripya neeche diye gaye Channel aur Group join karein, phir '✅ Maine Join Kar Liya' button dabayen.", reply_markup=join_markup))
        return
    ...
    # Hum yahan user ki query ko clean nahi karte, Algolia ki settings (removeStopWords, etc.) use karte hain
...
async def broadcast_command(message: types.Message):
    ...
    st = datetime.now(timezone.utc) # Use timezone-aware datetime
    tasks = []
    processed_count = 0
...
async def add_movie_command(message: types.Message):
    ...
    if db_res in [True, "updated"]: # Only sync if added or updated in DB
        ag_data = {'objectID': imdb_id, 'imdb_id': imdb_id, 'title': title, 'year': year, 'clean_title': clean_title_val} # `clean_title` add karein
...
async def import_json_command(message: types.Message):
    ...
    total = len(mlist); a, u, s, fdb = 0, 0, 0, 0; ag_batch = []; st = datetime.now(timezone.utc) # Use timezone-aware datetime
    ...
    for i, item in enumerate(mlist):
        try:
            ...
            ag_data = {'objectID': imdb, 'imdb_id': imdb, 'title': title, 'year': year, 'clean_title': clean_title_val} # `clean_title` add karein
...
async def auto_index_handler(message: types.Message):
    ...
    if db_res in [True, "updated"]:
        ...
        algolia_data = {'objectID': imdb_id, 'imdb_id': imdb_id, 'title': title, 'year': year, 'clean_title': clean_title_val} # `clean_title` add karein
