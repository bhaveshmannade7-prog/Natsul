...
# --- NAYA SEARCH LOGIC ---
logger.info("Search Logic: Hybrid (Mongo1-Exact + Python-Smart-Fuzzy)")
...
#
# ==================================================
# +++++ FIX: BALANCED SMART FUZZY SEARCH +++++
# ==================================================
#
def python_fuzzy_search(query: str, limit: int = 10) -> List[Dict]:
    """
    Optimized Logic:
    1. Scorer: WRatio (Handles filenames and partial matches best)
    2. Cutoff: 60 (Low enough to catch 'Katra'/'Aveger', keeps 'Avtar')
    3. Sorting: Heavily boosts 'Start Matches' & 'Exact Matches' to push 'Avatar' above 'Himmatwar'.
    """
    ...
    try:
        ...
        # --- FIX: Revert to WRatio, but use Low Cutoff (60) ---
        results = process.extract(
            query,
            choices, 
            scorer=fuzz.WRatio, 
            limit=20,  # Fetch more to sort later
            score_cutoff=60 # --- FIX: 60 ensures typos like 'Katra' (Score ~71) are found ---
        )
        ...
        for (matched_clean_title, score, index) in results:
            ...
            # --- FIX: Priority Boost Logic (Crucial for sorting) ---
            # This pushes correct matches (Avatar) above accidental junk (Himmatwar)
            
            # 1. Exact match bonus
            if matched_clean_title == query:
                score += 25
            # 2. Starts-with bonus (e.g. "Avtar" -> "Avatar...")
            # Checking startswith handles "Avtar" -> "Avatar" better than "Avtar" -> "Himmatwar"
            elif matched_clean_title.startswith(query) or (len(query) > 3 and matched_clean_title.startswith(query[:3])):
                score += 15
        ...
        # Re-sort after boosting to ensure best match is #1
...
async def health_check():
    ...
    return {
        ...
        "search_logic": "Hybrid (Mongo1-Exact + Python-Smart-Fuzzy)",
...
async def start_command(message: types.Message, db_primary: Database, db_fallback: Database, db_neon: NeonDB):
    ...
    if user_id == ADMIN_USER_ID:
        ...
        search_status = f"⚡️ Hybrid (Mongo-Exact + Python-Smart-Fuzzy)"
...
async def search_switch_command(message: types.Message):
    await safe_tg_call(message.answer("ℹ️ Search switch ki ab zaroorat nahi hai.\nBot ab hamesha <b>Hybrid (Mongo-Exact + Python-Smart-Fuzzy)</b> ka istemal karta hai."), semaphore=TELEGRAM_COPY_SEMAPHORE)
