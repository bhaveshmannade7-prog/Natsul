...
async def initialize_algolia():
    ...
    try:
        ...
        # --- Settings to apply ---
        settings_to_apply = {
            # ✅ FIX: Use clean_title with underscore
        ...
        await client.set_settings(index_name=ALGOLIA_INDEX_NAME, index_settings=settings_to_apply)
...
def is_algolia_ready():
    ...
    return is_ready and client is not None
...
async def algolia_search(query: str, limit: int = 20) -> List[Dict]:
    ...
    try:
        ...
        # ✅ FIX: Use imdb_id with underscore
        return [
            {
                "imdb_id": hit.get("objectID") or hit.get("imdb_id"),
                "title": hit.get("title") or "Title Missing",
                "year": hit.get("year")
            }
            for hit in hits if hit.get("objectID") or hit.get("imdb_id")
        ]
    except Exception as e:
...
async def algolia_add_movie(movie_data: dict) -> bool:
    ...
    if "objectID" not in movie_data or not movie_data["objectID"]:
        # ✅ FIX: Check for imdb_id with underscore
        if "imdb_id" in movie_data and movie_data["imdb_id"]:
            ...
            return False
    try:
        await client.save_object(index_name=ALGOLIA_INDEX_NAME, body=movie_data)
        return True
    except Exception as e:
...
async def algolia_add_batch_movies(movies_list: List[dict]) -> bool:
    ...
    if not valid_movies:
        ...
        return False
    try:
        await client.save_objects(index_name=ALGOLIA_INDEX_NAME, objects=valid_movies)
...
async def algolia_remove_movie(imdb_id: str) -> bool:
    ...
    if not imdb_id:
        return False
    try:
        await client.delete_object(index_name=ALGOLIA_INDEX_NAME, object_id=imdb_id)
