...
# --- Helper function (Yeh Python query ko clean karne ke liye zaroori hai) ---
...
class NeonDB:
    ...
    async def init_db(self):
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                # --- CRASH FIX (1/4): Purane table mein 'clean_title' column jodein ---
                try:
                ...
                # --- CRASH FIX (2/4): Python logic ko SQL FUNCTION ke roop mein banayein ---
                await conn.execute("""
                    ...
                    BEGIN
                        ...
                        -- 1. Remove non-alphanumeric/space
                        cleaned_text := regexp_replace(cleaned_text, '[^a-z0-9\s]+', ' ', 'g');
                        -- 2. Remove "s1", "season 02" etc. (using \y for word boundary)
                ...
                """)
                logger.info("SQL function 'f_clean_text_for_search' created/verified.")
                ...
                # --- CRASH FIX (3/4): Trigger ko naye SQL FUNCTION ka istemal karayein ---
                await conn.execute("""
                    CREATE OR REPLACE FUNCTION update_movie_search_vector()
                    RETURNS TRIGGER AS $$
                    BEGIN
                        -- Naye SQL function ko call karein
    ...
    async def add_movie(self, message_id: int, channel_id: int, file_id: str, file_unique_id: str, imdb_id: str, title: str) -> bool:
        ...
        # 'clean_title' ab trigger (SQL function) ke zariye database mein generate hoga
    ...
    async def neondb_search(self, query: str, limit: int = 10) -> List[Dict]:
        ...
        # Python function yahaan user ki query ko clean karne ke liye istemal hoga
    ...
    # --- CRASH FIX (4/4): Rebuild query ko naye SQL FUNCTION ka istemal karayein ---
    async def rebuild_fts_vectors(self) -> int:
        ...
        # Yeh query ab Python function 'clean_text_for_search' ke bajaye
        # SQL function 'f_clean_text_for_search' ka istemal karegi.
        query = """
            UPDATE movies
            SET 
    ...
    # ... (baaki sabhi functions 'find_and_delete_duplicates', 'get_unique_movies_for_backup', 'sync_from_mongo' same rahenge) ...
