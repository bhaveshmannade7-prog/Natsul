...
class NeonDB:
    ...
    # --- YEH FUNCTION REPLACE KIYA GAYA HAI (FINAL FIX 2.0) ---
    async def neondb_search(self, query: str, limit: int = 20) -> List[Dict]:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                # --- SABSE ZAROORI FIX ---
                # Hum is connection ke liye threshold 0.2 par set kar rahe hain
                await conn.execute("SET pg_trgm.similarity_threshold = 0.2;")
                # --- END FIX ---

                # --- NAYA FIX: asyncio.gather (jo error de raha tha) ko hata diya gaya hai ---
                # Ab hum dono queries ko ek ke baad ek (sequentially) chalayenge
                # Isse "operation is in progress" error nahi aayega
                
                try:
                    fts_rows = await conn.fetch(search_query_fts, query, limit)
                except Exception as e:
                    logger.error(f"NeonDB FTS search fail hua '{query}': {e}", exc_info=False)

                try:
                    trg_rows = await conn.fetch(trigram_query, query, limit)
                except Exception as e:
                    logger.error(f"NeonDB trigram search fail hua '{query}': {e}", exc_info=False)
                # --- END NAYA FIX ---

        except Exception as e:
            logger.error(f"NeonDB connection pool (acquire) fail hua '{query}': {e}", exc_info=True)
            return []
    ...
    async def rebuild_fts_vectors(self) -> int:
        """
        Purane data ke liye 'title_search_vector' column ko update karta hai jo NULL hain.
        """
        if not await self.is_ready():
        ...
        query = """
            UPDATE movies
            SET title_search_vector = setweight(to_tsvector('simple', COALESCE(title, '')), 'A')
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                logger.info(f"NeonDB: {updated_count} NULL FTS vectors ko rebuild kiya.")
