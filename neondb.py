...
class NeonDB:
    ...
    async def sync_from_mongo(self, mongo_movies: List[Dict]) -> int:
        ...
        for movie in mongo_movies:
            ...
            # --- YEH FIXED LOGIC HAI ---
            # Ab hum check karte hain ki key 'None' hai ya nahi
            # Isse '0' (zero) waali ID bhi pass ho jaayengi
            if not all([
                unique_id_for_db,
                movie.get('file_id'),
                movie.get('message_id') is not None,  # Allow 0 or 9090909090
                movie.get('channel_id') is not None   # Allow 0
            ]):
